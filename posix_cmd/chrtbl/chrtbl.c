 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: chrtbl.c,v 1.2.1.2 90/05/10 04:06:27 wje Exp $"
...
#define CHRCLASS	257
#define LC_CTYPE	258
#define LC_NUMERIC	259
#define	NUMERIC_BASE	260
#define DECIMAL_POINT	260
#define THOUSANDS_SEP	261
...
int	chrclass = 0;			/* set if LC_TYPE is defined */
int	numclass = 0;			/* set if LC_NUMERIC is defined */
...
int	ctype[SIZE];			/* character class and character
					 * conversion table */
int	numeric_fmt[FMT_SIZE];		/* numeric string format table */
int	range = 0;			/* set when the range character '-'
					 * follows the string */
char	tablename[24];			/* save data file name */  
char	fmtname[24];			/* save numeric format file name */  
 ...
 * of the environment variable CHRCLASS (or LC_CTYPE).
 */
...
/* create3() produces a data file containing the numeric format array
 * elements. The name of the file is the same as the value
 * of the environment variable LC_NUMERIC.
 */
...
{
	register   i=0;
	if (freopen(fmtname, "w", stdout) == NULL)
		perror(fmtname), exit(1);
	for (i=0; i< FMT_SIZE; i++)
		printf("%c", numeric_fmt[i]);
}
...
int	type;
{
	...
	*ep-- = '\0';  	/* delete newline */
	...
	if (strlen(p) > 1)
		return(0);
	numeric_fmt[type - NUMERIC_BASE] = (int)*p;
	return(1);	
}
