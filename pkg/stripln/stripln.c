 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: stripln.c,v 1.1.2.2 90/05/10 04:05:08 wje Exp $"
/*
**    stripln package bomfile ...
*/

#define MAIN 1
#include "stripln.h"

static char rcs[]="$Header: stripln.c,v 1.1.2.2 90/05/10 04:05:08 wje Exp $";
...
char *argv[];
{
   ...
   /* This stuff is here in case we ever define any options */

   while ((c = getopt(argc, argv, OPTIONS)) != EOF)
     switch (c)
       {
	 case 't': rmtime=atol(optarg); break;
	 default : usage(); break;
       }
   ...
   while (optind < argc) /* all arguments are bomfile names.. */
      {
  ...
  while (fgets(bomrec, MAXPRESREC, bomfil) != NULL)
    {
		...
		switch (sbuf.st_mode & S_IFMT)
		  {
		    ...
		    case S_IFLNK:
		      ...
		      if (unlink(targetfile) == -1)
		        {
			  sprintf(emsg, "cannot unlink %s, errno = %d",
				targetfile, errno);
		    ...
		    case S_IFDIR:
		      printf ("rmdir %s\n", targetfile);
		      if (rmdir(targetfile) == -1)
		        {
			  sprintf(emsg, "cannot rmdir %s, errno = %d",
				targetfile, errno);
			  error(ERRTYPE_WARN, emsg);
			}
		      break;
	  ...
	  /* case for symlinks that change to something else */

	  if (((sbuf.st_mode & S_IFLNK) == S_IFLNK) &&
	      ((mode & S_IFLNK) != S_IFLNK))
	    {
	      /* nuke it */
	      char rmcmd[MAXFILPATH+16];
	      sprintf(rmcmd, "rm -f %s", targetfile);
	      printf("%s\n", rmcmd);
	      system(rmcmd);
	    }

	  /* case for master plain file links, so as not to disturb old
	     links that are left around */
