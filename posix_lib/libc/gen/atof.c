 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: atof.c,v 1.1.1.2 90/05/10 04:12:24 wje Exp $"
...
#endif
{
	 ...
	 * is outrageously slow, so the accumulation is done using double's */
		...
		register unsigned char decpt_char = _numeric[DECIMAL_POINT];

		while (isdigit(c = *p++) || c == decpt_char && !decpt++) {
			if (c == decpt_char)
				continue;
			GOT_DIGIT;
