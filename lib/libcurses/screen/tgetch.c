 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: tgetch.c,v 1.2.1.3.1.1.1.2 91/01/04 17:51:29 beacker Exp $"
...
int	interpret;
{
	...
	if (*chars_onQ == 0)
	{
		 ...
		 * a key that starts with ERR.  Also, we don't want to
		 * access funckeystarter[ERR].
		 */
		interpret = FALSE;
	    }
	}
 ...
 * quickly if there is nothing waiting, returning ERR.
 ...
 * -2 is returned if we time out, ERR is returned if interrupted, and the
