 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: newscreen.c,v 1.2.1.3 90/05/10 02:16:33 wje Exp $"
...
FILE	*outfptr, *infptr;
{
...
#ifdef	_IOFBF
    (void) setvbuf(outfptr, 0, (char *) _IOFBF, 0);
#else	/* _IOFBF */
    ...
    PROGTTY.c_oflag &= ~OCRNL;
    change_bsd43_bits(0,RAW | CRMOD | CBREAK);
#else RISCOS
#ifdef	SYSV
    PROGTTY.c_lflag &= ~ECHO;
