 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: setupterm.c,v 1.1.1.3 90/05/10 02:33:19 wje Exp $"
	...
	*errret = 1;
	 ...
	 */
	{
		...
		if (ioctl(0, TIOCGWINSZ, &win) != -1 &&
			...
			cur_nums->_lines = win.ws_row;
			cur_nums->_columns = win.ws_col;
#ifdef DUMPTI
...
next:
	...
	for (n = 0; n < sizeof(offsets); n++)
	{
		...
		((n > 0) && off_sequences[0] != NULL &&
			(strcmp(off_sequences[n], off_sequences[0]) == 0)) ||
		((n == 2) && exit_attribute_mode != NULL &&
			(strcmp(exit_attribute_mode, off_sequences[n]) == 0)))
	    {
...
#ifdef RISCOS
/*
 *	Change current apparent settings of RAW, CBREAK, and CRMOD
 */
change_bsd43_bits(set_flags,reset_flags)
	int	set_flags;
	int	reset_flags;
{
	int	bsd43_bits;
	...
	set_flags &= ~bsd43_bits;
	old_icanon = PROGTTY.c_lflag & ICANON;
	if (reset_flags & RAW) {
		PROGTTY.c_oflag |= (SHELLTTY.c_oflag & OPOST);
		PROGTTY.c_iflag |= (SHELLTTY.c_iflag & ICRNL);
		PROGTTY.c_iflag |= (SHELLTTY.c_iflag & IXON);
		PROGTTY.c_lflag |= (ISIG | ICANON);
	};
	if (reset_flags & CBREAK) {
		PROGTTY.c_iflag |= (SHELLTTY.c_iflag & ICRNL);
		PROGTTY.c_lflag |= (ISIG | ICANON);
	};
	if (set_flags & CBREAK) {
	...
	if (set_flags & RAW) {
		...
		PROGTTY.c_iflag &= ~(ICRNL | IXON);
		PROGTTY.c_lflag &= ~(ISIG | ICANON);
	};
	if (reset_flags & CRMOD) {
	...
	if (old_icanon != 0 &&
	    (PROGTTY.c_lflag & ICANON) == 0) {
		PROGTTY.c_cc[VMIN] = 1;
		PROGTTY.c_cc[VTIME] = 0; 
	};
	if (old_icanon == 0 &&
	    (PROGTTY.c_lflag & ICANON) != 0) {
		if ((SHELLTTY.c_lflag & ICANON) != 0) {
			PROGTTY.c_cc[VEOF] = SHELLTTY.c_cc[VEOF];
			PROGTTY.c_cc[VEOL] = SHELLTTY.c_cc[VEOL];
		} else {
			PROGTTY.c_cc[VEOF] = CEOF;
			PROGTTY.c_cc[VEOL] = CNUL;
		};
	};
}
...
/*
 *	Return current apparent settings of RAW, CBREAK, and CRMOD
 */
get_bsd43_bits()
{
	int	bsd43_bits;
	...
	bsd43_bits = 0;
	if ((PROGTTY.c_lflag & ICANON) == 0 &&
	    (PROGTTY.c_lflag & ISIG) == 0 &&
	    (PROGTTY.c_iflag & IXON) == 0 &&
	    (PROGTTY.c_iflag & ICRNL) == 0)
		bsd43_bits |= RAW;
	else if ((PROGTTY.c_lflag & ICANON) == 0 &&
		 (PROGTTY.c_lflag & ISIG) != 0)
		bsd43_bits |= CBREAK;
	if ((PROGTTY.c_oflag & ONLCR) != 0)
		bsd43_bits |= CRMOD;
	return(bsd43_bits);
}
#endif RISCOS
