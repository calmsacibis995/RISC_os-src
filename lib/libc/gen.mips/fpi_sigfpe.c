 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: fpi_sigfpe.c,v 1.5.2.2 90/05/10 01:25:09 wje Exp $"
...
fpi()
{
	...
	if(sysmips(MIPS_FPSIGINTR, 1) == -1){
	    fprintf(stderr,
	   "fpi(): Error: system call sysmips(MIPS_FPSIGINTR,1) failed (%s)\n",
		   sys_errlist[errno]);
	    exit(1);
	}
}
...
/* routines to execute single instructions */
...
struct sigcontext *scp;
{
	...
	if(sysmips(MIPS_FPSIGINTR, 0) == -1){
	    fprintf(stderr,
	   "fpi(): Error: system call sysmips(MIPS_FPSIGINTR,0) failed (%s)\n",
		   sys_errlist[errno]);
	    exit(1);
	}
	...
	if(fpc_irr.fi_struct.implementation == IMPLEMENTATION_R2360){
	    if(sysmips(MIPS_FPSIGINTR, 1) == -1){
		fprintf(stderr, "fpi(): Error: system call sysmips(MIPS_FPSIGINTR,1) failed (%s)\n", sys_errlist[errno]);
		exit(1);
	    }
	 ...
	 */
	    ...
	    if(sysmips(MIPS_FPSIGINTR, 2) == -1){
		fprintf(stderr, "fpi(): Error: system call sysmips(MIPS_FPSIGINTR,2) failed (%s)\n", sys_errlist[errno]);
		exit(1);
	    }
	}
}
