 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: doprnt.c,v 1.15.1.5.1.1.1.3 90/11/16 09:11:27 beacker Exp $"
...
#ifdef	SYSTYPE_POSIX
#include "../gen/_locale.h"
#define	DPCHAR	_numeric[DECIMAL_POINT]
#else
#define DPCHAR '.'
#endif

#ifndef _NFILE
#define _NFILE 20		/* System V */
#endif
	...
	int *fp_classp, *is_zerop;
{
	...
	*is_zerop = (value == 0.0);
	...
	if (!f_flag) {
		if (ndigits > 17) {
			register char *p, *e;
			*decpt = _dtoa (buf, 17, value, 0) + 1;
			for (p = buf+18,
			     e = buf + 1 + (ndigits > NDIG ? NDIG : ndigits);
			     p != e; ) *p++ = '0';
			*p++ = '\0';
		}
		else if (ndigits <= 0) {
			*decpt = _dtoa (buf, 1, value, 0) + 1;
			buf[1] = '\0';
		}
		else {
			*decpt = _dtoa (buf, ndigits, value, 0) + 1;
		}
		*sign = buf[0] == '-';
		return buf+1;
		...
		*decpt = _dtoa (buf, ndigits, value, 1) + 1;
		*sign = buf[0] == '-';
		return buf+1;
	}
}
...
#ifdef	SYSTYPE_POSIX
#define PUT(p, n)     { register int nn, cc, rr; \
			for (nn = 0; nn < n; nn++) { \
				cc = *(p+nn); \
				if ( (rr = putc(cc, iop)) < 0) { \
					return(rr); \
