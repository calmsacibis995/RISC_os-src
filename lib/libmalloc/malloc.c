 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: malloc.c,v 1.7.2.3 90/05/10 02:50:02 wje Exp $"
...
unsigned nbytes;
{
	...
	/* Wait until we get here before checking that nbytes is valid.
	 * We want to make sure that the initialization code gets done
	 * the first time through.
	 */
	if (nbytes == 0) return NULL;
#ifdef RISCOS
	if (nbytes > 0x7FFFFFFF) return NULL;
#endif /* RISCOS */
	if (nbytes <= maxfast)  {
		/*
