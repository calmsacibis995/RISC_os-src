#ident "$Header: common_scsi.c,v 1.8 90/11/06 19:46:54 chungc Exp $"
/* $Copyright: |
 * |-----------------------------------------------------------|
...
#define		pblkno		bp->av_back
#define		residual	bp->b_dkspare
...
struct sense_key {
	...
	{ BLANK_CHK, "no-data condition encountered on tape or tape is QIC-120 and drive is QIC-24" },
...
static struct int_errors {
	...
	/* Hardware Status */
	{ PARITY, "SCSI Parity Error Unrecoverable" },
...
int mode;
{
    ...
    if (machine_type == BRDTYPE_R3030 && !un->un_tmpbuf) {
#ifdef STANDALONE
        un->un_tmpbuf = (u_char*)align_malloc(64,align);
        un->un_buf_64 = un->un_tmpbuf;
#else STANDALONE
	un->un_tmpbuf = (u_char*)kern_malloc(64);
	temp = (align - 1) & (u_long)un->un_tmpbuf;
	if (temp) { /* aligned? */
	    kern_free(un->un_tmpbuf); /* no, then free and alloc extra */
	    un->un_tmpbuf = (u_char*)kern_malloc(64 + align);
	    temp = (align - 1) & (u_long)un->un_tmpbuf;
...
#else STANDALONE
	un->un_vh = (struct volume_header*)kern_malloc(512);
	temp = (align - 1) & (u_long)un->un_vh;
	if (temp) { /* aligned? */
	    kern_free(un->un_vh); /* no, then free and alloc extra */
	    un->un_vh = (struct volume_header*)kern_malloc(512 + align);
	    temp = (align - 1) & (u_long)un->un_vh;
	    /* round up to address alignment */
	    un->un_vh += (align - temp);
	}
    ...
    if (showconfig) {
	common_scsi_string(un,inq_string);
	...
	if (is_a_tape && ((status == SCSI_CHECK && sense->key == NOT_RDY) ||
		status == SCSI_BUSY))
	    goto success;
	else if (is_a_disk && status==SCSI_CHECK && sense->key==MEDIUM_ERR) {
...
int mode;
{
	...
	if (common_scsi_spcmd(un,C0_STARTSTOP,1,1,0,mode)) {
	    ...
	    (void)timeout(motortimeout, 0, 10 * HZ);
	}
...
int mode;
{
    ...
    if (common_scsi_spcmd(un,C1_READCAP,0,sizeof(SCSI_READCAP),addr,mode))
        if (scsiexterr)
            cmn_err(CE_CONT,"SCSI %dL%d: Cannot read capacity\n",
                    un->un_target,un->un_lun);
    addr = K0_TO_K1(&un->un_mserror);
    if (common_scsi_spcmd(un,C0_MSENSE,PD_ERROR,
			sizeof(SCSI_MS_ERROR),addr,mode))
        if (scsiexterr)
	    cmn_err(CE_CONT,
	...
	addr = K0_TO_K1(&un->un_msrdahd);
	if (common_scsi_spcmd(un,C0_MSENSE,PD_RDAHD,
			sizeof(SCSI_MS_RDAHD),addr,mode))
	    if (scsiexterr)
		cmn_err(CE_CONT,
		"SCSI %dL%d: cannot get cache control parameters\n",
	        un->un_target,un->un_lun);
	if (((SCSI_MS_RDAHD *)addr)->rcd == 1)
	    cmn_err(CE_WARN,"SCSI %dL%d: Disk cache not enabled\n",
	        un->un_target,un->un_lun);
    } else {
	addr = K0_TO_K1(&un->un_mscache);
	if (common_scsi_spcmd(un,C0_MSENSE,PD_CACHE,
			sizeof(SCSI_MS_CACHE),addr,mode))
	    if (scsiexterr)
		cmn_err(CE_CONT,
		"SCSI %dL%d: cannot get cache control parameters\n",
	        un->un_target,un->un_lun);
	if (((SCSI_MS_CACHE *)addr)->ce == 0)
	    cmn_err(CE_WARN,"SCSI %dL%d: Disk cache not enabled\n",
	        un->un_target,un->un_lun);
    }
    (void)readvh(un,mode);
...
register struct scsi_unit *un;
{
	...
	if (ip->scsi_hwstatus) {
	    cmn_err(CE_CONT,"SCSI %dL%d: scsi hardware ERROR status: %x %s\n",
...
errcomp:
	    ...
	    if (ip->cmd_blk.cdb_0.cdb_0_cmd != C0_REQSENSE)
		++un->un_hardcount;
	    cmn_err(CE_CONT,"SCSI %dL%d: retrying(%d)...\n",
		un->un_target,un->un_lun,bp->b_error);
	    temp = bp->b_error;
	    devp = &vh->vh_dp;
	    ...
	    while (bp) { /* need to handle combined reads/writes! */
		     ...
		     */
		    pblkno  = (struct buf*)(bp->b_blkno + pt->pt_firstlbn);
		    bp->b_cylin = (u_int)pblkno / temp1;
...
normcomp1:
	...
	do {
	     ...
	     * is b_resid valid for any SPL commands??? XXX
	     */
	    savedbp = bp;
		...
		pblkno  = (struct buf*)(bn + pt->pt_firstlbn);
		bp->b_cylin = (u_int)pblkno / temp;
		disksort(dp, bp, un->un_lastcyl);
		bp = NULL; /* force out of loop! */
	    } else {
normal:
		/*
...
register struct buf *bp;
{
    ...
    cbn = (u_int)pblkno; /* (b_count - b_resid) handled in common_scsi_intr */
    pblkno = (struct buf*)0; /* now we can zero out bp->av_back for use below */
    un->un_sgentries = 0;	 /* reset */

    if ((amount = bp->b_bcount % un->un_sectsize) ||
	((long)bp->b_dmaaddr & un->un_dmastartmask)) {
	if (amount)
	    amount = (un->un_sectsize - amount); /* round up */
	count = sizeof(struct align_info) + amount + bp->b_bcount
		+ un->un_dmaalign;
	if ((al = (struct align *)kern_malloc(count)) == NULL) {
	    cmn_err(CE_PANIC,"SCSI %dL%d: Could not kern_malloc %d bytes\n",
	...
	if (un->un_dmaalign)
	    al->al_tbuff = (char *)((u_long)(al->al_tbuff + un->un_dmaalign - 1)
			& ~(un->un_dmaalign - 1));
	bp->av_back = (struct buf *)al;
	bp->b_dmaaddr = (caddr_t)al->al_tbuff;
	bp->b_bcount += amount;
	bp->b_resid += amount;
	if (!(bp->b_flags & B_READ))
	    bcopy(al->al_uaddr,al->al_tbuff,al->al_size);
	abnormal = 1;
    }
    /*
...
lbpl:
	...
	(*LOW_SCSI(bp->b_dev,setupdma))(un,SG,r_w,amount,physaddr,
		un->un_sgentries);
	bp->b_resid -= amount;
	...
	if (nbp) {
	   if((!bp->b_error) &&			/* retry? */
	     ...
	     ((cbn + (bp->b_bcount >> SCTRSHFT)) == (u_int)nbp->av_back) &&
...
dev_t dev;
{
    ...
    else if (!physck(pt->pt_nblks, B_READ))
	return;
...
dev_t dev;
{
    ...
    else if (!physck(pt->pt_nblks, B_WRITE))
	return;
...
int dev,count,addr,block,mode;
{
	...
	if (IS_KSEG0(dmaaddr))
	    ...
	    phys = dmaaddr;
	(*LOW_SCSI(dev,setupdma))(un,NOSG,r_w,amount,phys,entry++);
	dmaaddr += amount;
	total -= amount;
    }
...
register caddr_t arg;
{
...
#ifndef STANDALONE
    if ((un->un_flags & INT_TAPE) && !(cmd == GIOCPRSTR || cmd == DIOCREMOVE)) {
	...
	case GIOCPRSTR:
	    ...
	    common_scsi_string(un, gioctl.gi_ident);
	...
	case DIOCNOECC:
	    ...
	    if (common_scsi_spcmd(un,C0_MSELECT,PD_ERROR,
				sizeof(SCSI_MS_ERROR),err,SP_WAIT))
		status = DIOC_OPERR;
	    break;
	...
	case DIOCRDEFECTS:
	    ...
	    /* round up size */
	    if (un->un_dmaalign)
		i = (io_arg.datasz + un->un_dmaalign - 1) 
			& ~(un->un_dmaalign - 1);
#ifdef STANDALONE
            addr = (u_int)align_malloc(i + un->un_dmaalign,un->un_dmaalign);
#else STANDALONE
            addr = (u_int)kern_malloc(i + un->un_dmaalign);
#endif STANDALONE
	...
	case GIOCGETVAL:
	    ...
	    i += un->un_dmaalign;
#ifdef STANDALONE
            addr = (u_int)align_malloc(i,un->un_dmaalign);
#else STANDALONE
            addr = (u_int)kern_malloc(i);
#endif STANDALONE
	    cp = (u_short *)K2_TO_K1(addr);
	    if (un->un_dmaalign)
		cp = (u_short *)(((u_long)cp + un->un_dmaalign - 1)
	...
	case GIOCSETVAL:
	    ...
	    if ((err->msense.hdr.blk_desc_len != 
				sizeof(SCSI_MS_BLK_DESC)) ||
		...
		switch (gioctl.gi_page) {
		    case PD_ERROR:
			bcopy(cp, &un->un_mserror, gioctl.gi_size);
			break;
		    case PD_DISREC:
...
register char *pp;
{
	...
	case TYPE_TAPE:
		rb = (SCSI_RDBLKLIM *)K0_TO_K1(&un->un_blklim);
                (void)common_scsi_spcmd(un,C0_RDBLOCK,0,6,rb,POLLED);
		if (rb->maxlen != rb->minlen)
			q = "VARIABLE TAPE";
		else
			q = "FIXED TAPE";
	...
	if (len > ifp->offset)
	    ...
	    j = 0;
	if (!ifp->offset) j = 15;	/* special case for device type */
	q = ifp->dp;
...
register struct fmt_map_info *fmi;
{
	...
	case FMI_MAP_TRACK:
	    ...
	    i = fmi->fmi_intrlv + un->un_dmaalign;
#ifdef STANDALONE
	    addr = (int *)align_malloc(i,un->un_dmaalign);
#else STANDALONE
	    addr = (int *)kern_malloc(i);
#endif STANDALONE
