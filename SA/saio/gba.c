 ...
 * |-----------------------------------------------------------|
 */
/* $Header: gba.c,v 1.9 90/10/24 13:19:29 hawkes Exp $ */
...
{
  int GBAid;
  unsigned long ioa_base_addr, ioa_bsize;
#ifdef	R6000_FAKEIO  
  ...
  int GBAid;
{
  ...
  vme_addr_reg =gbaPtr->ioa->mem.gba_misc_ctl[gbaPtr->gbaNum].vme_addr_comp;
  if (vme_addr_reg.word != 0x0000FFFF)
	cmn_err(CE_WARN, "vme_addr_comp register: 0x%x\n", vme_addr_reg );
  /*
   * Can't currently read vme_addr_comp register.  HW has hard coded PAL.
   */
/****  vme_addr_reg.word = 0x0000ffff;	*****/
...
retry:  
  ...
  while (!(*(volatile uint *)CSR_IVECTSET
    ...
    if (i++ > 1000) {
      ...
      cmn_err(CE_WARN,
	      "GBA flush op did NOT complete, flush_addr: 0x%x flush_op: 0x%x",
	      flush_addr, i);
      flush_addr = 0;
  ...
  if (i > imax) {
    imax = i;
    cmn_err( CE_CONT, "New cache wait max: %d\n", imax );
  }
  *(volatile uint *)CSR_IVECTCLR =
    CSR_IVECTSET_IOA1_GBA0_LOP | CSR_IVECTSET_IOA1_GBA1_LOP;
  ...
  if (!flush_addr)
    ...
    /* Following check is to make sure HW really performed the flush */
...

{
  ...
  oldPri = splhigh();		/* START: critical section on CS queue */
...

{
  ...
  if (!(LCS->userGBAflags & GBA_CS_AUTOEXP))
...

{
  ...
  oldPri = splhigh();		/* START: critical section on SR queue */
  do {
    PCS = LCS;
    while (PCS && needPages) {
 ...
 * GBA cache.
 *
 * If GBA_CS_AUTOEXP is set in the GBAflags, then gba_map_spage will
...
{
  int		GBAid;		/* GBA which contains the device */
  gba_sw_info  *gbaPtr;		/* pointer to GBA info area */
...

{
  ...
  if (fakeio != fakeio_reported) {
    if (fakeio)
      ...
      cmn_err(CE_WARN, "Using REAL I/O");
  ...
  oldPri = splhigh();		/* START: critical section: flush done bits */
 ...
 *
 * Special Flags:
 *	GBA_CONTIG_ADDR	I/O address space must be contiguous
 *	GBA_NOPART_MAP	No partial map allowed if insufficient registers
 *
...

{
  ...
  *smapArea = 0;
  ...
  if ((pages > 1) && (flags & GBA_CONTIG_ADDR))
...

{
  ...
  oldPri = splhigh();		/* START: critical section on SP queue */
...

{
  ...
  if ((kaddr != 0) && (len !=0)) {
	...
	if (nextSregInfo->mapEntry.bits.pp_num == ++prevPPnum)
	  cmn_err(CE_CONT, "Fakeio OK, found contiguous physical page\n");
	else
...
{
  return( vme_release_iomap( csh ));
}  

boolean 
gba_reserve_lcs( dev_no, dev_ctlspace, minPages, csh, userGBAflags )
...
{
return(vme_reserve_iomap( dev_no, dev_ctlspace, minPages, csh, userGBAflags));
}

boolean
gba_kseg_flush( smapArea, kaddr, len )

sah_type smapArea;		/* map area descriptor */
kaddr_t  kaddr;			/* system address (k0,k1,k2) */
long len;			/* length of area (bytes) */
{
  return(vme_ioflush(smapArea, kaddr, len));
}  
...
{
  return (vme_iomap_flush(csh));
}  


int
gba_kseg_map( csh, kaddr, len, smapArea, flags)
...

{
  ...
  return(vme_iomap(csh, kaddr, len, flags, smapArea, &VMEaddress));
}

boolean
gba_kseg_unmap( smapArea )
     sah_type smapArea;	/* map area descriptor */

{
  return(vme_iounmap( smapArea ));
}  

boolean
ka_to_ioaddr( smapArea, kaddr, GBAaddress )
...
{
  return( ka_to_vmeaddr( smapArea, kaddr, GBAaddress ));
}
