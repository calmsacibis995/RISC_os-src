#ident "$Header: mono.c,v 1.3 90/05/23 15:20:54 huang Exp $"
/*	%Q%	%I%	%M%	*/
/* $Copyright: |
 * |-----------------------------------------------------------|
...
#define svirtophys(x)	((u_int)(x) & ~0xE0000000)
#else
...
mono_init ()
{
	...
	/*
	 * the standalone version is at the end of the function --
	 * changes have to be reflected there too.
	 */
	if (do_mono) {
	 ...
	 */
	sp = ipcget (GBMONCH, IPC_CREAT|0666, shmem, shminfo.shmmni,
		 sizeof (*sp), &status);
	spc = ipcget (GBMNUNC, IPC_CREAT|0666, shmem, shminfo.shmmni,
		 sizeof (*sp), &status);
	ASSERT(sp);
	ASSERT(spc);
	ASSERT(status);
	...
	for (i = 0; i < ctos(end); i++) {
		...
		while (--size >= 0) {
			pt->pgi.pg_pde = mkpde(PG_N|PG_M|PG_VR|PG_G|PG_LOCK|PG_SV|PG_NDREF, gp);
			pdetodbd(pt)->dbd_type = DBD_NONE;
			pt++;
			ptc->pgi.pg_pde = mkpde(PG_M|PG_VR|PG_G|PG_LOCK|PG_SV|PG_NDREF, gp++);
...
int clr;
{
	...
	init_r3030_mono(K0_TO_K1(frm_addr), clr);
	ram_ch_p->dma_mode = FLUSH_RAMBO_FIFO | CLR_DMA_ERR;
	ram_ch_p->dma_laddr = svirtophys(frm_addr);
	ram_ch_p->dma_block_s = (MONO_FRAME_SIZE >>BLOCK_SHIFT);
	ram_ch_p->dma_mode = CHANNEL_EN | AUTO_RELOAD;
