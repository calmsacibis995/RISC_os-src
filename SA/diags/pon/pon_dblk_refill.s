#ident "$Header: pon_dblk_refill.s,v 1.10.3.1 90/07/18 14:30:27 huang Exp $"
/* $Copyright: |
# |-----------------------------------------------------------|
...
/*
 *	R3000 data block refill function.
 *	A 0 is returned upon success.
 *
 *	Functional Description:
 *
 *	Verifies that the R3000 on the R3200 CPU board is doing block refill of
 *	the defined block size for the data cache.  The block size is jumper
 *	configurable on the R3200 but there is no means for this program to
 *	read that information, so the block size this program determines via
...
#define	DBLKSIZEM180		4
#define	DBLKSIZEM20		8
#define	DBLKSIZE		16
#define	MINBLKSIZE		4
...
3:
		...
		b	Fail
2:
		li	a1,DBLKSIZE		# expected block refill size
		beq	sp,BRDTYPE_R3200,5f
		beq	sp,BRDTYPE_RB3125,5f

		li	a1,DBLKSIZEM180		# expected block refill size
		beq	sp,BRDTYPE_M180,5f

		li	a1,DBLKSIZEM20		# expected block refill size
5:
		beq	a1,s1,4f		# check block refill size found
...
begintest:
		.asciiz "Data Cache Block Refill Test..."
