 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: if_egl.c,v 1.21.1.17.1.2.1.5 90/12/20 19:21:09 beacker Exp $"
/*
...
#define NEGL	8	/* Maximum number of eagles supported */
...
struct  egl_softc {
	...
	csh_type	egl_csh_xmit;	/*GBA cache section descriptor (xmit)*/
	csh_type	egl_csh_rcv;	/* GBA cache section descriptor (rcv)*/
	EGL_CSTB	egl_cstb;	/* Firmware info		*/
...
#else
int egl_rxblock;	/* relies on BSS being 0'ed */
int egl_txblock;	/* relies on BSS being 0'ed */
int egl_txdone;		/* relies on BSS being 0'ed */
#endif  STANDALONE
...
int read_op;
{
  ...
  if (iomap->io_sah) {		/* Buffer is already setup */
    ...
    writeback_virtual_data( mtod(mbuffer, ULONG), mbuffer->m_len );
#else
    ...
    if (!vme_iomap( (read_op) ? egl->egl_csh_rcv : egl->egl_csh_xmit,
		   PHYS_TO_K0(iomap->physical),  mbuffer->m_len,
#ifdef	R6000_FAKEIO
...
#ifndef STANDALONE
  if (read_op)
    invalidate_virtual_data( mtod(iomap->virtual, ULONG),
			    iomap->virtual->m_len);
#else
...
#endif STANDALONE
{
...
#ifdef	R6000
	addr = (EGL_SHIO*)edtp->e_base;
	if (!(addr = (EGL_SHIO*)find_r6000_controller
	      (edtp, (int)&addr->sh_MCSB - (int)addr, sizeof(short)))) {
#else
	...
	if (!vme_reserve_iomap( unit, addr, 32, &egl->egl_csh_xmit, GBA_CS_AUTOEXP))
	  cmn_err( CE_PANIC, "Couldn't allocate xmit cache section in GBA!\n");
	if (!vme_reserve_iomap( unit, addr, 32, &egl->egl_csh_rcv, GBA_CS_AUTOEXP))
	  cmn_err( CE_PANIC, "Couldn't allocate rcv cache section in GBA!\n");
	eglsetup(unit);
#ifndef STANDALONE
...
eglsetup(unit)
{
	...
	bcopy(&shio->sh_CSTB, &egl->egl_cstb, sizeof(EGL_CSTB));
	if (!egl->laf_set) {
		egl->egl_cstb.cstb_FILT[0] = 0x00;
		egl->egl_cstb.cstb_FILT[1] = 0x00;
		egl->egl_cstb.cstb_FILT[2] = 0x00;
		egl->egl_cstb.cstb_FILT[3] = 0x00;
		egl->egl_cstb.cstb_FILT[4] = 0x00;
		egl->egl_cstb.cstb_FILT[5] = 0x00;
		egl->egl_cstb.cstb_FILT[6] = 0x00;
		egl->egl_cstb.cstb_FILT[7] = 0x00;
	}
...
#endif  STANDALONE
	if (!egl->enaddr_set)
		bcopy(egl->egl_cstb.cstb_PHY, egl->egl_enaddr, sizeof(egl->egl_cstb.cstb_PHY));
#ifndef	STANDALONE	
...
/*
 * Stop controller and clear pending operations.
 */
egl_stop(unit)				/* So Lboot does not grab it */
	int unit;
{
	register struct egl_softc *egl = &egl_softc[unit];
	volatile  EGL_SHIO *shio = egl->egl_shio;
	...
	int unit;
{
	...
	/* Do we REALLY want to blindly set IFF_UP ???  */
	...
	int unit;
{
	...
	for(i = 0;i < EGL_RX_NBUFF;i++) {
		...
		if (IS_M2000_ARCH || IS_R6300) /* use block mode transfers */
	...
	int unit;
{
...
#ifdef	R6000
	/* RC62x0 series can handle mod 2 fine, since the EBA is capable
	 * of responding to the 16-bit block-mode Address Modifier which
	 * may occur for the final transfer.
	 */
	iopb->iopb_LENGTH = (len + 1) & ~0x1; 
	...
	if (!(buff_addr & 0x2))
	  ...
	  W(iopb->iopb_TOPT) = EGL_TOPT | DIR_WRITE;
#else
	/* If egl_noroundup is set, then we can't round up to multiple of
	 * of four bytes because some other machine can't handle the padded
	 * transmit.  In this case we will end up using non-block mode
	 * packet VME transfer to get the packet.
	 */
	if (egl_noroundup)
	  iopb->iopb_LENGTH = (len + 1) & ~0x1; 
	else
	...
	if (IS_M2000_ARCH) {
	  ...
	  /* we must be aligned on a long-word boundary */
	  if ((buff_addr & 0x3) || (iopb->iopb_LENGTH & 0x03))
	    W(iopb->iopb_TOPT) = EGL_TOPT | DIR_WRITE;
	  else   /* block mode OK */
	    W(iopb->iopb_TOPT) = EGL_TOPT_BLK | DIR_WRITE;
	}
	...
	int itype;
{
	...
	bcopy(egl->egl_cstb.cstb_PHY, cib->cib_PHY, 
		sizeof(egl->egl_cstb.cstb_PHY));/* Default Physical Address */
	bcopy(egl->egl_cstb.cstb_FILT, cib->cib_FILT,
	 sizeof(egl->egl_cstb.cstb_FILT)); /* Default Logical Address Filter */
	...
	register int unit;
{
	...
	egl->egl_quecnt--;
	egl->egl_qfull = 0;		/* at least one slot open now */
	/* Stop current transmit timeout.  Start new timeout if still
	 * have transmit requests queued on controller.
	 */
	if (egl->egl_tx_timerid)
	...
	register int unit;
{
	...
	if(tiopb.iopb_LAN1 & LANCE_TERR) {
		...
		if (tiopb.iopb_LAN3 & LAN3_BUFF)
		  log(BSD43_LOG_ERR," buffer error " );
		if (tiopb.iopb_LAN3 & LAN3_UFLO)
		  log(BSD43_LOG_ERR," underflow " );
		if (tiopb.iopb_LAN3 & LAN3_LCOL)
		  log(BSD43_LOG_ERR," late collision " );
		if (tiopb.iopb_LAN3 & LAN3_LCAR)
		  log(BSD43_LOG_ERR," carrier loss " );
		if (tiopb.iopb_LAN3 & LAN3_RTRY)
		  log(BSD43_LOG_ERR," retry error " );
		log(BSD43_LOG_ERR,"\n" );
	...
	register int unit;
{
	...
	volatile EGL_IOPB tiopb;
	struct mbuf *rbuf;
	register int real_errors;
	
	tiopb = shio->sh_RET_IOPB;
	if (tiopb.iopb_LAN1 & LANCE_RERR) {
	  	/* First, check for bogus LANCE errors.  Call normal	*/
	  	/* completion code if no "real" errors.			*/
	  	real_errors = tiopb.iopb_LAN1;
		if ((tiopb.iopb_LAN1 & LAN1_OFLO) &&
		    (tiopb.iopb_LAN1 & LANCE_RENP))
		    real_errors ^= LAN1_OFLO;	/* Turn off bogus OFLO */

		if ((!(tiopb.iopb_LAN1 & LANCE_RENP)) ||
		    (tiopb.iopb_LAN1 & LAN1_OFLO))
		  real_errors &= (0xffff - (LAN1_FRAM | LAN1_CRC));

		if (!(real_errors & 
		      (LAN1_FRAM | LAN1_OFLO | LAN1_CRC | LAN1_BUFF))) {
		  /* Error was bogus.  Treat like normal receive int.  */
		  eglrint(unit);
		  return;
		}
		/* Looks like we had a real receive error */
		log(BSD43_LOG_ERR,"egl%d: rx error, LAN1 = %x LAN3 = 0x%x",
			unit,tiopb.iopb_LAN1,tiopb.iopb_LAN3);
		if (real_errors & LAN1_FRAM)
		  log(BSD43_LOG_ERR," framing error " );
		if (real_errors & LAN1_OFLO)
		  log(BSD43_LOG_ERR," overflow " );
		if (real_errors & LAN1_CRC)
		  log(BSD43_LOG_ERR," crc error " );
		if (real_errors & LAN1_BUFF)
		  log(BSD43_LOG_ERR," buffer error " );
		log(BSD43_LOG_ERR,"\n" );
	...
	struct sockaddr *dst;
{
	...
	int can_trail;
	u_char	edst[6];
	register struct mbuf *mcopy = (struct mbuf *) 0;
	...
	caddr_t data;
{
	...
	switch (cmd) {
		...
		case AF_UNSPEC:
			...
			bcopy((caddr_t)egl->egl_enaddr, (caddr_t)egl->egl_cstb.cstb_PHY, sizeof (egl->egl_cstb.cstb_PHY));
			egl_init(ifp->if_unit);
			break;
		default:
			egl_init(ifp->if_unit);
	...
	case SIOCSIFFLAGS:
		if ( ((ifp->if_flags & IFF_UP) == 0) &&
			...
			egl_stop(ifp->if_unit);		/* reset controller */
        ...
        case SIOCSLAF:
		egl->laf_set = 1;
		bcopy (ifr->ifr_data, (caddr_t)egl->egl_cstb.cstb_FILT, 8);
		egl_init(ifp->if_unit);
		break;

        case SIOCGLAF:
		bcopy ((caddr_t)egl->egl_cstb.cstb_FILT, ifr->ifr_data, 8);
		break;
#ifdef notdef
	case SIOCGIFSTATS:
		{
 ...
 *
 *	egl0 - Interphase 4207 Eagle - Revision XXX - xx:xx:xx:xx:xx:xx
 */
eglgetctlr(ifp, buf)
struct ifnet *ifp;
char *buf;
{
	register int unit = ifp->if_unit;
	struct egl_softc *egl = &egl_softc[unit];
	...
	strcat( buf, " (");
	btoa( egl->egl_cstb.cstb_FREV[0], &buf[strlen(buf)], 16);
	strcat(buf, ".");
	btoa( egl->egl_cstb.cstb_FREV[1], &buf[strlen(buf)], 16);
	strcat(buf, ".");
	btoa( egl->egl_cstb.cstb_FREV[2], &buf[strlen(buf)], 16);
	strcat(buf, ") Date ");
	strncat( buf,  &egl->egl_cstb.cstb_FDATE[3], 2);
	strcat( buf, "/");
	strcat( buf,  &egl->egl_cstb.cstb_FDATE[6]);
	strcat( buf, "/");
	strncat( buf,  &egl->egl_cstb.cstb_FDATE[0], 2);
	strcat( buf, " - ");
	btoa( egl->egl_enaddr[0], &buf[strlen(buf)], 16);
	strcat( buf, ":");
	btoa( egl->egl_enaddr[1], &buf[strlen(buf)], 16);
	strcat( buf, ":");
	btoa( egl->egl_enaddr[2], &buf[strlen(buf)], 16);
	strcat( buf, ":");
	btoa( egl->egl_enaddr[3], &buf[strlen(buf)], 16);
	strcat( buf, ":");
	btoa( egl->egl_enaddr[4], &buf[strlen(buf)], 16);
	strcat( buf, ":");
	btoa( egl->egl_enaddr[5], &buf[strlen(buf)], 16);
	return;
