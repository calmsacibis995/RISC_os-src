 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: uipc_socket.c,v 1.2.1.4 90/05/10 04:35:01 wje Exp $"
/*
 * Copyright (c) 1982, 1986 Regents of the University of California.
 * All rights reserved.  The Berkeley software License Agreement
 * specifies the terms and conditions for redistribution.
 *
 * @(#)uipc_socket.c 2.1 88/05/18 4.0NFSSRC SMI
 * @(#)uipc_socket.c 7.1 86/06/05 UCB
 */

/* originally included  param.h systm.h user.h proc.h file.h vnode.h
 * buf.h mbuf.h un.h domain.h protosw.h socket.h socketvar.h stat.h
 * ioctl.h uio.h ../net/route.h ../netinet/in.h ../net/if.h
 */

#include "../tcp-param.h"       /* KLUDGE to get extra symbols */
#include "sys/types.h"
...
#ifdef	RISCOS
#define	CLTHRESHOLD	512		/* threshold for using clusters */
...
#ifdef RISCOS
	struct mbuf *p; /* ADD register later */
#endif
	register int space;
...
#ifdef RISCOS
			   (uio->uio_resid >= CLTHRESHOLD &&
			     space < CLTHRESHOLD &&
			   so->so_snd.sb_cc >= CLTHRESHOLD &&
			   (so->so_state & SS_NBIO) == 0 
			    && (uio->uio_fmode & FNDELAY) == 0)
#else
...
#ifdef	RISCOS
				/* SYSV and POSIX programs using sockets
				 * won't have SS_NBIO set if O_NDELAY was
				 * set using an fcntl(), so we must check the
				 * file mode bits ; the PRE4_5 stuff
				 * is to preserve binary (bug) compatibility
				 * with systems prior to RISC/os 4.50
				 */
				if ((uio->uio_fmode & FNDELAY) &&
				    (uio->uio_fmode & FNDELAY_PRE4_5) == 0) {
					if (first)
						error = EAGAIN;
					splx(s);
					goto release;
...
#ifdef RISCOS
			if (uio->uio_resid >= CLTHRESHOLD &&
			    space >= CLTHRESHOLD) {
				if (!mclget(m)) { /* failed to get cluster */
					goto nopages;
				}
				len = MIN(MIN(MCLBYTES, uio->uio_resid),
					  space);
				space -= len;
#else
...
#define	rcverr(errno)	{ error = errno; splx(s); goto release; }
	if (so->so_rcv.sb_cc == 0) {
		...
		if (so->so_state & SS_NBIO)
			rcverr(EWOULDBLOCK);
#ifdef	RISCOS
		/* If this is a SYSV or POSIX program using sockets, then
		 * SS_NBIO isn't set so we must look at the mode.  The
		 * PRE4_5 stuff is to preserve binary (actually bug)
		 * compatibility with systems prior to RISC/os 4.50
		 */
		if ((uio->uio_fmode & FNDELAY) &&
		    (uio->uio_fmode & FNDELAY_PRE4_5) == 0) {
			rcverr(EAGAIN);
	...
	register struct socket *so;
{
	...
	else if (so->so_pgrp > 0 && (p = bsd43_pfind(so->so_pgrp)) != 0)
		psignal(p, SIGURG);
#else
	if (so->so_pgrp < 0)
