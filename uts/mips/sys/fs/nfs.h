 ...
 * |-----------------------------------------------------------|
 */
/* $Header: nfs.h,v 1.14.1.4 90/05/10 06:16:20 wje Exp $ */

/*	@(#)nfs.h	2.2 88/05/20 NFSSRC4.0 from  2.36 88/02/08 SMI 	*/

#ifndef _SYS_FS_NFS_
#define _SYS_FS_NFS_	1
...
enum nfsstat {
	NFS_OK = 0,			/* no error */
	NFSERR_PERM=1,                  /* EPERM => Not owner */
	NFSERR_NOENT=2,			/* ENOENT No such file or directory */
	NFSERR_IO=5,			/* EIO I/O error */
	NFSERR_NXIO=6,                  /* ENXIO No such device or address */
	NFSERR_ACCES=13,		/* EACCES Permission denied */
	NFSERR_EXIST=17,		/* EEXIST File exists */
	NFSERR_NODEV=19,		/* ENODEV No such device */
	NFSERR_NOTDIR=20,		/* ENOTDIR Not a directory */
	NFSERR_ISDIR=21,		/* EISDIR Is a directory */
	NFSERR_FBIG=27,  		/* EFBIG File too large */
	NFSERR_NOSPC=28,		/* ENOSPC No space left on device */
	NFSERR_ROFS=30, 		/* EROFS Read-only file system */

	NFSERR_OPNOTSUPP=45, 		/* EOPNOTSUPP operation NOT supported */
	                                /* Unofficial NFS Connect-athon added */
...
#ifdef RISCOS
#define	puterrno(error)		(((enum nfsstat)nfs_puterrno(error)))
#define	geterrno(status)	((int)nfs_geterrno(status))
#else
...
#ifdef RISCOS
#define NFS_FIFO_MODE	BSD43_S_IFCHR
#else
...
#define NA_SETFIFO(NA)	{ \
			...
			(NA)->na_rdev = NFS_FIFO_DEV; \
			(NA)->na_mode = ((NA)->na_mode&~BSD43_S_IFMT)|NFS_FIFO_MODE; \
			}
#else
#define NA_SETFIFO(NA)	{ \
			(NA)->na_type = NFS_FIFO_TYPE; \
...
#ifdef SYSTYPE_SYSV
	struct bsd43_dirent *rdok_entries;	/* variable number of entries */
#else SYSTYPE_SYSV
