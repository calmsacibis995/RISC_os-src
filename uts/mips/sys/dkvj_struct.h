 ...
 * |-----------------------------------------------------------|
 */
/* $Header: dkvj_struct.h,v 1.5.1.3.1.3 90/07/12 10:45:03 hawkes Exp $ */
/*
 ...
 *
 */
#define NUM_CQE         22	/* is this the maximum we can fit on */
#define NUM_CQE_OFF     40	/* this is the maximum allowed (off-board) */
#define MAX_IOPB        64
...
typedef struct iqar {               /* Interrupt on Queue Available Reg    */
    union {
        struct {
            ...
            Bit     IQHE : 1;       /* Interrupt on Queue Half Empty Enable*/
            Bit          : 3;       /* Reserved bits                       */
...
/*  UWORD   cib_WQ0_TIMEOUT_msw;    /* Work Q 0 timeout in 256ms ticks     */
/*  UWORD   cib_WQ0_TIMEOUT_lsw;    /* Work Q 0 timeout in 256ms ticks     */
    UWORD   cib_RESELECT_msw;       /* REselection timeout in milli-second */
    UWORD   cib_RESELECT_lsw;       /* REselection timeout in milli-second */
    UWORD   cib_VME_TIMEOUT_msw;    /* VME timeout: 0 == 100ms timeout     */
    UWORD   cib_VME_TIMEOUT_lsw;    /* VME timeout: 0 == 100ms timeout     */
    UWORD   cib_PAGE_SIZE_msw;      /* Page size on controller in bytes    */
    UWORD   cib_PAGE_SIZE_lsw;      /* Page size on controller in bytes    */
    VJ_ADR  cib_CRB_ADRMOD;         /* Off board CRB memtype, AM, xfer type*/
    UWORD   cib_CRB_ADDRESS_msw;    /* CRB off-board address               */
    UWORD   cib_CRB_ADDRESS_lsw;    /* CRB off-board address               */
    UWORD   cib_RES0;    	    /* Reserved word 	                   */
    UWORD   cib_RES1;    	    /* Reserved word 	                   */
} VJ_CIB;
...
typedef struct qecr {               /* Queue Entry Control Register        */
    union {
        struct {
            ...
            Bit          : 3;       /* Reserved bits                       */
            Bit     FOB  : 1;       /* Fetch Off-board CQE/IOPB pair       */
            Bit     FA   : 1;       /* What is this for?                   */
...
typedef struct crsw {               /* Command Response Status Word        */
    union {
        struct {
            Bit           : 8;      /* Reserved bits                       */
            Bit     SC    : 1;      /* Controller Error occured (ERT)      */
...
typedef struct crb {                /* Command Response Block              */
    ...
    UBYTE	crb_IOPB_TYPE;	    /* nuff said			   */
    UBYTE       crb_RES0;           /* Reserved byte                       */
...
typedef struct csb {                /* Configuration Status Block 120 bytes*/
    ...
    UWORD   csb_RES8[2];            /* Reserved 4 bytes                    */
    UBYTE   csb_PID;                /* Primary Bus ID                      */
    UBYTE   csb_SID;                /* Secondary Bus ID                    */
    UBYTE   csb_PRI_SLCTD; /* (7ac) Last device selected on primary Bus    */
    UBYTE   csb_SEC_SLCTD;          /* Last device selected on second Bus  */
    UBYTE   csb_PRI_PSNS;  /* (7ae) Primary port phase sense shadow        */
    UBYTE   csb_SEC_PSNS;           /* Secondary port phase sense shadow   */
    UBYTE   csb_RSRVD9;	   /* (7b0) Reserved byte			   */
    UBYTE   csb_DB_ID;		    /* Daughter board ID		   */
    UBYTE   csb_RSRVD10;   /* (7b2) Reserved byte			   */
    UBYTE   csb_DIP_SW;		    /* Software Dip switch setting 	   */
    ULONG   csb_FRZ_BITS;  /* (7b4) Queue Frozen status's		   */
/*  UBYTE   csb_RES9[72];           /* Reserved bytes                      */
    UBYTE   csb_RES9[64];           /* Reserved bytes                      */
    /* The following are mips special debugging hacks			   */
    UWORD   csb_VEC_FLAG;           /* Vector flag, normal 1: cib 2 (7f8)  */
    UWORD   csb_TIMER_VEC;          /* Timer Vector	            (7fa)  */
    UWORD   csb_IOPB_VEC;           /* Iopb  Vector	            (7fc)  */
    UWORD   csb_POST_VEC;           /* Post  Vector	            (7fe)  */
} VJ_CSB;
...
typedef struct option {             /* IOPB Option word                    */
    union {
        struct {
            ...
            Bit           : 4;      /* Reserved bits                       */
            Bit     FAST  : 1;      /* Fast Scatter/Gather                 */
            Bit     NOSYNC: 1;      /* Supress Syncronous transfers (debug)*/
            Bit     SGI   : 1;      /* Scatter/Gather                      */
...
#define M_OPT_NOSYNC    0x0004
#define M_OPT_FAST      0x0008
#define B_OPT_DIR(x)    ((x).U.b.DIR)
#define B_OPT_IE(x)     ((x).U.b.IE)
#define B_OPT_SG(x)     ((x).U.b.SGI)
#define B_OPT_NOSYNC(x) ((x).U.b.NOSYNC)
#define B_OPT_FAST(x)   ((x).U.b.FAST)
