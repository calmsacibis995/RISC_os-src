 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: stream.c,v 1.18.1.2.1.2.1.4 91/01/18 15:12:24 beacker Exp $"
...
int queueflag;		/* set iff inside queuerun() */
int queueflag_missed;	/* set iff queuerun() found queueflag set */
...
long arg;
{
	...
	if (!(sep = sealloc(SE_NOSLP))) {
		cmn_err(CE_CONT, "bufcall: could not allocate stream event\n");
		return(0);
	}
	dbp = &dballoc[class];
	s = splstr();
...
/*
 * Return sum of msgdsize() for all messages on a queue
 */
int
qmsgdsize(q)
     	queue_t *q;
{
	...
	s = splstr();
	totalsize = 0;
	for (nbp = q->q_first; nbp != NULL; nbp = nbp->b_next)
		totalsize += msgdsize(nbp);
	splx(s);
	return(totalsize);
}
...
#define STRMEDFRAC strmedfrac
	...
	if (size == 0
		...
		cmn_err(CE_CONT, "strinit: cannot allocate stream data blocks\n");
		return;
        }
...
allocq()
{
	...
	if (!qp) {
		cmn_err(CE_CONT, "allocq: out of queues\n");
		strst.queue.fail++;
	} else {
...
/* 
 * This is a generic routine used by all of the character i/o
 * drivers so it is included here.
 */
tcgeta(wq,bp, p)
queue_t *wq;
register mblk_t *bp;
