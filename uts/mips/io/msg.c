 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: msg.c,v 1.6.1.2 90/05/10 05:24:10 wje Exp $"
...
#include	"sys/pcb.h"
#include	"sys/user.h"
#if !defined(u3b5) && !defined(u3b2) && !defined(mips)
#include	"sys/seg.h"
#endif
#include	"sys/proc.h"
#include	"sys/buf.h"
...
#endif
#if u3b || u3b2
	if ((msg = (paddr_t)kseg(btoc(msginfo.msgseg * msginfo.msgssz))) ==
		NULL) {
#endif
#if mips
	n = btoc(msginfo.msgseg * msginfo.msgssz);
	memlock();
	if ((msg = (paddr_t)phystokv(ctob(contmemall(n, 0)))) == NULL) {
#endif
