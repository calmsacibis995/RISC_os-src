 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: nfs_dump.c,v 1.2.1.4 90/05/10 05:03:34 wje Exp $"
/*
 * @(#)nfs_dump.c 1.2  88/05/23 NFSSRC4.0 Copyright Sun Microsystems, Inc.
 * @(#)nfs_dump.c 1.10 88/02/08           Copyright Sun Microsystems, Inc.
 *
...
resend:
		 ...
		 */
#ifdef RISCOS
		m = (struct mbuf *)mclgetx(noop, 0, noop, 0,
					   header, HDR_SIZE, M_WAIT);
#else
		m = (struct mbuf *)mclgetx(noop, 0,
					   header, HDR_SIZE, M_WAIT);
#endif
		if (m == NULL) {
...
#ifdef RISCOS
		m->m_next = (struct mbuf *) mclgetx(noop, 0, noop, 0,
				addr, count, M_DONTWAIT);
#else
...
int
nfs_dumpstub(dev, flag, bn, physaddr, count)
dev_t dev;
int flag;
daddr_t bn;
caddr_t physaddr;
int count;
{
	nfs_dump(dumpfile.bo_vp,physaddr,bn,count);
}
