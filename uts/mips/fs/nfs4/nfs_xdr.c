 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: nfs_xdr.c,v 1.2.1.4 90/05/22 21:45:03 wje Exp $"
...
#include "sys/vnode.h"
#include "sys/file.h"
#include "bsd43/sys/dirent.h"   /* "sys/dirent.h" */
...
#ifdef RISCOS
#define	nextdp(dp)	((struct bsd43_dirent *)((int)(dp) + (dp)->d_reclen))
#define MINDIRSIZ(dp)	(sizeof(struct bsd43_dirent) - MAXNAMLEN + (dp)->d_namlen)
#else
	...
	struct nfsrddirres *rd;
{
	...
	for (;;) {
		...
		if (valid) {
			if (!xdr_u_long(xdrs, &t_dirent.d_fileno) ||
			    !xdr_u_short(xdrs, &t_dirent.d_namlen) ||
			    t_dirent.d_namlen > MAXNAMLEN)
				return (FALSE);

			if (DIRSIZ(&t_dirent) > size) {
				/* 
				 * no room:  skip rest of entries
				 */
				for (;;) {
					if (!xdr_opaque(xdrs, t_dirent.d_name,
						(u_int)t_dirent.d_namlen) ||
					    !xdr_u_long(xdrs,
						(u_long *) &t_dirent.d_off) )
						return (FALSE);
				...
				/*
				 * we are not at eof, since we just tossed
				 * at least one entry
				 */
				if (!xdr_bool(xdrs, &rd->rd_eof)) {
					return (FALSE);
			...
			/*
			 * get rest of this entry
			 */
			dp->d_fileno = t_dirent.d_fileno;
			dp->d_namlen = t_dirent.d_namlen;
