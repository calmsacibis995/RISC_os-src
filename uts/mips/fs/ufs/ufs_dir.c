 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ufs_dir.c,v 1.4.1.7.1.2 90/08/03 11:20:56 hawkes Exp $"

/* "@(#)ufs_dir.c	2.6 88/06/24 4.0NFSSRC Copyr 1988 Sun Micro"; */
...
/* Originally included param.h systm.h user.h proc.h vfs.h vfs_stat.h vnode.h
 * buf.h uio.h dnlc.h ../ufs/inode.h ../ufs/fs.h ../ufs/mount.h ../ufs/fsdir.h 
	...
	struct inode **ipp;		/* return entered inode (locked) here */
{
	 ...
	 */
	for (ptr = namep, namlen = 0; *ptr; ptr++, namlen++)
		if (*ptr == '/')
			return(EACCES);
#else
	namlen = strlen(namep);
	 ...
	 * and return EEXIST.  Rename or link TO "." is forbidden.
	 * Rename of ".." is forbidden, but link must be able to do this
	 * (in order to implement /etc/mvdir).
	 */
	if (namep[0] == '.' &&
	    (namlen == 1 || (namlen == 2 && namep[1] == '.' && op != DE_LINK))){
		if (op == DE_RENAME) {
			return (ENOTEMPTY);
		}
		if (ipp) {
	...
	if (tip) {
...
#if RISCOS
		/*
		 * SVID requires mkdir to return EMLINK if making the
		 * directory will push the parent over the MAXLINK limit.
		 */
		if (op == DE_CREATE && SYSV_SYSCALL &&
		    tdp->i_nlink >= MAXLINK && vap->va_type == VDIR) {
			err = EMLINK;
			goto out;
		}
#endif
		...
		err = diraddentry(tdp, namep, namlen, &slot, sip, sdp, op);
		if (err) {
			if (op == DE_CREATE) {
				/*
	...
	struct slot *slotp;		/* slot for entry */
{
	...
	if (doingdirectory) {
		...
		tdp->i_flag |= ICHG;
		if (sdp != tdp) {
			err = dirfixdotdot(sip, sdp, tdp, 0);
			if (err) {
				return (err);
			}
		}
 ...
 */
static
dirfixdotdot(dp, opdp, npdp, linkdir)
	...
	register struct inode *npdp;	/* new parent directory */
{
	...
	/* Only adjust . and .. if we are truly creating a new directory
	 * and not if we are just creating a hard link to a directory
	 */
	if (!linkdir) {
		/*
		 * Increment the link count in the new parent inode and 
		 * force it out.
		 */
		npdp->i_nlink++;
		npdp->i_flag |= ICHG;
 ...
 */
static
diraddentry(tdp, namep, namlen, slotp, sip, sdp, op)
	struct inode *tdp;
	char *namep;
	int namlen;
	struct slot *slotp;
	...
	enum de_op op;			/* entry operation */
{
	...
	if ((sip->i_mode & IFMT) == IFDIR) {
		err = dirfixdotdot(sip, sdp, tdp, (op == DE_LINK));
		if (err) {
			goto bad;
		}
	}
	...
	register struct vattr *vap;
{
	...
	ip->i_uid = u.u_uid;
#if RISCOS
	ip->i_gid = (_riscos_group_parent == 0 ||
		     ITOV(tdp)->v_vfsp->vfs_flag & VFS_GRPID ||
		     tdp->i_mode & ISGID) ? tdp->i_gid : u.u_gid;
	/* this is already done for the NFS case and can, therefore,
	 * be done at a higher level for the local case; but will do
	 * it here just in case we talk to an NFS client which doesn't
	...
	if ((ip->i_mode & ISGID) && !groupmember(ip->i_gid)) {
		ip->i_mode &= ~ISGID;
#if RISCOS
		/* So we send a consistent value back to higher levels.
		 * We deem this not to be an error.
		 */
		vap->va_mode &= ~ISGID;
#endif
	}
...
#if RISCOS
			if (rmdir)
				return (SYSV_SYSCALL ? EEXIST : ENOTEMPTY);
#else
			return (ENOTEMPTY);
#endif /* RISCOS */
	}
...
#if RISCOS
	/*
	 * SVID demands an EINVAL if we try to rmdir() the current dir.
	 *
	 * SVID demands that ETXTBUSY be returned on last unlink of
	 * a busy text file.
	 */
	if (SYSV_SYSCALL) {
	    if (rmdir) {
...
#if RISCOS
			err = (SYSV_SYSCALL ? EEXIST : ENOTEMPTY);
#else
			err = ENOTEMPTY;
#endif /* RISCOS */
			goto out;
