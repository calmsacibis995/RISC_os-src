 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ufs_alloc.c,v 1.3.1.7 90/05/10 20:31:52 wje Exp $"
/* @(#)ufs_alloc.c 2.1 88/05/20 4.0NFSSRC SMI */
/* @(#)ufs_alloc.c 7.1 86/06/05 UCB */
/*
...
/* Original include: param.h systm.h buf.h vfs.h user.h kernel.h vnode.h */
	...
	int osize, nsize;
{
	...
	if (bno != 0) {
		...
		} while (brealloc(bp, nsize, 0 /* don't flush B_DELWRI buffers if growing */) == 0);
		bp->b_flags |= B_DONE;
	...
	if (bno > 0) {
		 ...
		 * 
		 * The NFS4.0 code uses disk-relative blkno's for page cache
		 * identification, whereas we use file-relative blkno's.
		 * This routine only gets disk-relative blkno's as addresses,
		 * but it is only called to grow a frag, and frags can oonly
		 * exist at the end of the file.  Hence, we can determine out
		 * file-relative blkno by looking at the size of the file.
		 */
		{
		  int base_db = btodb(ip->i_size & fs->fs_bmask);
		  u_long nodeid = ip->i_number;
		  for (i = 0; i < count; i += btodb(NBPC_I))
			punhash(ITOV(ip), (base_db+i), nodeid);
		}
#else
		s = splimp();
		for (i = 0; i < count; i += CLBYTES / DEV_BSIZE)
			if (mfind(ip->i_devvp, bn + i))
				munhash(ip->i_devvp, bn + i);
		splx(s);
#endif
		bzero(bp->b_un.b_addr + osize, (unsigned)nsize - osize);
	...
	int mode;
{
	...
	if (ip->i_mode) {
		...
		bp1 = bread(ip->i_devvp, fsbtodb(fs, itod(fs, ino)),
				(int)fs->fs_bsize);
		dp = bp1->b_un.b_dino;
		dp += itoo(fs, ino);
		printf("incore version = %d, disk version = %d\n",
			ip->i_gen, dp->di_gen);
...
#if NOTYET
#if RISCOS
	btoss(ip->i_dev, fsbtodb(fs, bno), size);
#endif
#endif
	bno = dtogd(fs, bno);
	if (size == fs->fs_bsize) {
		...
		for (i = 0; i < frags; i++) {
			if (isset(cgp->cg_free, bno + i)) {
				ino = ip->i_number;
				bp1 = bread(ip->i_devvp,
					    fsbtodb(fs, itod(fs, ino)),
					    (int)fs->fs_bsize);
				dp = bp1->b_un.b_dino;
				dp += itoo(fs, ino);
				printf("incore version = %d, disk version = %d\n",
