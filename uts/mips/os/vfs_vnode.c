 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: vfs_vnode.c,v 1.5.1.5.1.4 90/08/03 11:26:54 hawkes Exp $"

/*	@(#)vfs_vnode.c	2.3 88/08/09 4.0NFSSRC SMI;  from SMI 2.16 86/12/05	*/

/* Originally included param.h user.h uio.h file.h pathname.h vfs.h vnode.h */
...
/*
 * release a vnode; decrements reference count and calls VOP_INACTIVE when zero.
 */
void
vn_rele(vp)
	register struct vnode *vp;
{
	...
	if (vp->v_count == 0)
		panic("vn_rele");
	ASSERT(vp->v_dnlcref < vp->v_count);
	if (--vp->v_count == 0) {
	...
	struct vnode **vpp;
{
	...
	*vpp = (struct vnode *)0;
	...
	if (error)
		return (error);
#ifdef 	RISCOS
	/* POSIX and SYSV expect to get ENOENT if passed a NULL string.
	 * BSD programs convert it to the current directory.
	 */
	else if (pn.pn_pathlen == 0 && (POSIX_SYSCALL || SYSV_SYSCALL)) {
		pn_free(&pn);
		return(ENOENT);
	}
#endif
	...
	int seg;
{
	 ...
	 * BSD programs convert it to the current directory.
	 */
	if (pn.pn_pathlen == 0 && (POSIX_SYSCALL || SYSV_SYSCALL)) {
	        pn_free(&pn);
		return(ENOENT);
	}

	followlink = (POSIX_SYSCALL || SYSV_SYSCALL) ? NO_FOLLOW : FOLLOW_LINK;
	error = lookupname(from_p, seg, followlink, (struct vnode **)0, &fvp);
#else
	...
	int seg;
{
	 ...
	 * BSD programs convert it to the current directory.
	 */
	else if (fpn.pn_pathlen == 0 && (POSIX_SYSCALL || SYSV_SYSCALL)) {
		pn_free(&fpn);
		return(ENOENT);
	}
#endif
	 ...
	 * BSD programs convert it to the current directory.
	 */
	else if (tpn.pn_pathlen == 0 && (POSIX_SYSCALL || SYSV_SYSCALL)) {
		pn_free(&fpn);
		return(ENOENT);
	}
#endif
...
#if RISCOS
	/*
	 * lookuppn returns EEXIST if (vp == dvp), which should only happen
	 * if we are removing ".".   The proper return for trying to 
	 * rmdir(".") is EINVAL.
	 */
	if (error == EEXIST && dirflag == DIRECTORY)
		error = EINVAL;
#endif /* RISCOS */
	if (error) {
		pn_free(&pn);
...
#if RISCOS
	/*
	 * I believe this to be a NFS4.0 bug, but it is possible
	 * that they are emulating BSD behavior, so I won't change
	 * it.  We really should be checking dvp's filesystem to
	 * see if it is RDONLY, since dvp is being modified, not vp.
	 * If we have a path which ends in
	 * "..", then dvp and vp will be somewhat reversed, but
	 * they will be in the same filesystem.  If we have a path
	 * which ends with the last component crossing a mount point,
	 * then the SVID demands that we return EBUSY (below),
	 * and dvp is the correct filesystem to check anyway.
	 * Otherwise, they are in the same filesystem, anyway.
	 */
	if (SYSV_SYSCALL) {
		if (dvp->v_vfsp->vfs_flag & VFS_RDONLY) {
			error = EROFS;
...
#ifdef RISCOS
	/*
	 * While releasing the vnode, we call VOP_SETATTR to
	 * set the file attributes.  This may fail while attempting
	 * to set the file modification time returning EPERM.
	 * We ignore this since we are going to unlink the file.
	 */
	if (u.u_error == EPERM)
		u.u_error = 0;
#endif /* RISCOS */
