/* $Header: flock.c,v 1.9.1.1 89/11/28 12:09:21 wje Exp $ */
/* ------------------------------------------------------------------ */
 ...
 *      "l_len" because the end, not length, of the record lock is stored
 *      internally.
 */
#include "sys/param.h"
#include "sys/errno.h"
...
/*
 * Initialize file lock free list
 */
flckinit()
{
	register i;
 ...
 * already existing locked section. There are five regions:
 *
 ...
 *
 * When the list is sufficiently scanned and the new lock is not an unlock,
 * the new lock is inserted into the appropriate position in the list.
 */
...
/*
 * blocked checks whether a new lock (lckdat) would be blocked by a
 * previously set lock owned by another process. Insrt is set to point
 * to the lock where lock list updating should begin to place the new lock.
 */
struct filock *
 ...
 *
 * if cmd equals (F_SETLK || F_SETLKW) then obtain a lock with or w/o sleeping.
 * If sleepwait is non-zero then sleep if a blocking lock already exists.
 *
 * Note: In some cases cmd equals F_SETLK but sleepwait is TRUE depending on
 * the f_mode F_NDELAY bit setting.
 */
reclock(vp, lckdat, cmd, offset, filesize, sleepwait)
struct	vnode  *vp;
struct	flock *lckdat;
int	cmd;
off_t	offset;
int	filesize, sleepwait;
{
	...
	if (retval = convoff(lckdat, 0, offset, filesize))
		return (retval);
	...
	lock_list = (struct filock **)&vp->v_filocks;
	...
	convoff(lckdat, whence, offset, filesize);
	return(retval);
}

/*
 * convoff converts the given data (start, whence) to the given whence.
 * Note: Consult the SVID for the whence definitions/semantics.
 */
int
convoff(lckdat, whence, offset, filesize)
struct flock *lckdat;
