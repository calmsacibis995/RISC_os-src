 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: vfs_dnlc.c,v 1.3.1.2.1.3 90/07/12 10:43:37 hawkes Exp $"
...
/* Originally included  param.h user.h systm.h vnode.h dnlc.h */
 ...
 */
#if RISCOS
#define	INS_HASH(ncp,nch)	_insque((caddr_t)ncp, (caddr_t)nch)
#define	RM_HASH(ncp)		_remque((caddr_t)ncp)
#else
	...
	struct ucred *cred;
{
	...
	VN_HOLD(dp);
	dp->v_dnlcref++;
	ASSERT(dp->v_dnlcref != 0  && dp->v_dnlcref <= dp->v_count);
	ncp->vp = vp;
	VN_HOLD(vp);
	vp->v_dnlcref++;
	ASSERT(vp->v_dnlcref != 0 && vp->v_dnlcref <= vp->v_count);
	ncp->namlen = namlen;
