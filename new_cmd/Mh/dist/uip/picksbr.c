...
struct tws *tws_parse (), *tws_special ();
...
struct	nexus *parse (), *exp1 (), *exp2 (), *exp3 (), *newnexus ();

int	ORaction (), ANDaction (), NOTaction (), GREPaction (), TWSaction ();
...
static struct nexus *exp3 () {
	...
	case PRTO: 
	case PRSUBJ: 
	    dp = *brkstring (parswit[i].sw, " ", NULLCP);
    header: ;
...
int     isafter;
{
    ...
    (void) sprintf (buffer, "%02d %s %02d %s",
	    ts -> tw_mday, MOTY(ts -> tw_mon), ts -> tw_year, ap);
    if ((tw = dparsetime (buffer)) != NULL)
	return tw;

    (void) sprintf (buffer, "%02d %s %02d %s %s",
	    ts -> tw_mday, MOTY(ts -> tw_mon), ts -> tw_year,
	    ap, dtwszone (ts));
    if ((tw = dparsetime (buffer)) != NULL)
	return tw;
