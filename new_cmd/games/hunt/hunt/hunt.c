 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: hunt.c,v 1.2.2.3 90/05/10 03:12:04 wje Exp $"
...
# endif BROADCAST
	...
	if (Sock_host != NULL) {
		...
		bcopy(hp->h_addr, &Daemon.sin_addr, sizeof(Daemon.sin_addr));
		if (!Query_driver)
			return;
	}
...
# endif BROADCAST
		...
		bcopy(hp->h_addr, &local_address, sizeof(Daemon.sin_addr));
...
# else BROADCAST
	...
	while (hp = gethostent()) {
		...
		bcopy(hp->h_addr, &taddr, sizeof(taddr));
