 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: crib.c,v 1.1.2.2 90/05/10 03:04:24 wje Exp $"
...
# define	INSTRCMD	"more -f -s /usr/games/lib/crib.instr"
...
char	*argv[];
{
	...
	if (!quiet) {
		...
		save_handler = signal(SIGTSTP, SIG_DFL);
		endwin();
		fflush(stdout);
		system(INSTRCMD);
		signal(SIGTSTP, save_handler);
		crmode();
		noecho();
		clear();
		refresh();
