 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: subs.c,v 1.1.2.2 90/05/10 02:57:26 wje Exp $"
...

{
	...
	while (s[0] && s[0][0] == '-') {
		...
		case 's':
			...
			recover (s[0]);
			break;
		/* Fast mode - turn off pauses */
		case 'f':
			pause = 0;
			break;

		default:
			fprintf(stderr, "Usage: backgammon [-f] [-n] [-b] [-r] [-w] [-p[brw]] [-t[term]] [-sfile]\n");
			exit(1);
			break;
		}
		s++;
