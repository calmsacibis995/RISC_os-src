 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: com4.c,v 1.1.2.2 90/05/10 02:59:15 wje Exp $"
	...
	while(wordnumber <= wordcount){
		...
		switch(value){
			...
			case MANGO:
				...
				printf("%s:\n",objsht[value]);
				if (testbit(inven,value) && curtime > ate - CYCLE && testbit(inven,KNIFE)){
					clearbit(inven,value);
					carrying -= objwt[value];
					encumber -= objcumber[value];
					ate = max(curtime,ate) + CYCLE/3;
					snooze += CYCLE/10;
					curtime++;
					puts("Eaten.  You can explore a little longer now.");
				}
				else if (curtime < ate - CYCLE)
					puts("You're stuffed.");
