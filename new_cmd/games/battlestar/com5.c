 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: com5.c,v 1.1.2.2 90/05/10 02:59:22 wje Exp $"
...
zzz()
{
	...
	oldtime = curtime;
	if ((snooze - curtime) < (0.75 * CYCLE)){
		curtime += 0.75 * CYCLE - (snooze - curtime);
		printf("<zzz>");
		for (n = 0; n < curtime - oldtime; n++)
			printf(".");
		printf("\n");
		snooze += 3 * (curtime - oldtime);
		if (notes[LAUNCHED]){
			fuel -= (curtime - oldtime);
...
chime()
{
	if ((curtime / CYCLE + 1) % 2 && OUTSIDE)
		switch((curtime % CYCLE)/(CYCLE / 7)){
			case 0:
		...
		puts("I can't tell the curtime in here.");
}
