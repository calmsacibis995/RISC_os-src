 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: canfield.c,v 1.2.2.2 90/05/10 03:03:12 wje Exp $"

/*
 * The canfield program
 *
 * Usage: canfield [-a] [-b] [-c] [-n] [-x]
 *
 *	-a	Abbreviated input mode (no newlines)
 *	-b	Betting box
 *	-c	Card counting on
 *	-n	Don't ask if user wants instructions
 *	-x	Clear box
 *
 * The environment variable CANFIELD is read first for options.
 *
 * Authors:
	...
	char *cp;
{
	...
	if (Input == I_OLD) {
		old_getcmd(row, col, cp);
	} else {
		new_getcmd(row, col, cp);
	}
}
	...
	char *argv[];
{
	...
	set_from_env();
	while ((c = getopt(argc, argv, "abcnx")) != EOF) {
		switch (c) {
		...
		default:
			fprintf(stderr, "usage: canfield [-a] [-b] [-c] [-n] [-x]\n");
			exit(1);
...
/*
 * The subroutine set_from_env() reads the enviroment variable CANFIELD
 * to set options. The variable is parsed for option letters. Any option
 * letter found sets the option. All others are silently ignored.
 */
...
/*
 * procedure to get a command - old style.
 */
...
/*
 * procedure to get a command - new style. This is also called abbreviated
 * style. The first character must be one of the following or a bell sounds:
 *
 *	s S t T 1 2 3 4
 *	c C b B q Q x X i I y Y n N
 *	h H 
 *
 * The variable srcpile is set to this character. In the case of the second
 * set, return is immediate with the variable destpile set to null. The third
 * set is just like the second except that destpile is set to 't'. In the
 * other cases, the second character must be from the set:
 *
 *	1 2 3 4 f
 *
 * which is used to set destpile.
 *
 * If a backspace or line kill is typed in the second position, control goes
 * back to getting the first character. If a formfeed (^L) is typed at
 * either position, the screen is redrawn. If a suspend (^Z) is typed at
 * either position, the program is suspended.
 *
 * For convenience, upper case is translated to lower case.
 */

#define POS1_DEL	"st1234"
#define POS1_IMM	"hcbqxiyn"
#define POS2		"1234f"
	...
	char *cp;
{
	...
	int position = 1;	/* Start in first position */
	int ch;
	...
	while (position <= 2) {
		...
		if (position == 2) {
			...
			if (ch == _tty.sg_erase || ch == _tty.sg_kill) {
				printw("\b \b");
				position--;
				col--;
				continue;
			}
			if (index(POS2, ch) != NULL) {
				addch(ch);
				destpile = ch;
				col++;
				position++;
