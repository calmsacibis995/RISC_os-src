/* $Header: intrp.c,v 1.1.1.1 89/11/28 01:02:36 wje Exp $
 *
 * $Log:	intrp.c,v $
 * Revision 1.1.1.1  89/11/28  01:02:36  wje
 * Branch RISCOS_4_50_FIXES off of trunk.
 * 
 * Revision 1.1  89/05/26  20:55:22  kris
 * Initial revision
 * 
...
register int destsize;
{
	...
	if (*src != '<') {
		...
		*dest = *dot++ = '\0';
		sprintf(tmpbuf,"%s@mips.COM",dot,beg);
		len = strlen(tmpbuf);
		if (destsize > len) {
		    strcpy(dest,tmpbuf);
		    dest = dest + len;
