/* $Header: term.c,v 1.1.1.1 89/11/28 01:09:01 wje Exp $
 *
 * $Log:	term.c,v $
 * Revision 1.1.1.1  89/11/28  01:09:01  wje
 * Branch RISCOS_4_50_FIXES off of trunk.
 * 
 * Revision 1.1  89/05/26  20:57:30  kris
 * Initial revision
 * 
...
get_windowsize()
{
    ...
    signal(SIGWINCH,get_windowsize);
    ioctl(0,TIOCGWINSZ,&winsizebuf);
    if (winsizebuf.ws_row)
	LINES = winsizebuf.ws_row;
    if (winsizebuf.ws_col)
	COLS = winsizebuf.ws_col;
}
