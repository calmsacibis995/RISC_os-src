#ident "$Header: top.c,v 1.5.1.2.1.2 90/07/11 18:19:39 hawkes Exp $"
 ...
 *  Pyramid C compiler).
 *
 *  The Symmetric 375 needs various goodies as well.  "scs" is defined in cc.
 *
...

{
	...
	case 16384:	pte_to_pfn_shift = 10;
			...
			Usize = 1;
			break;
	default:	fprintf(stderr,"top: cannot handle %d byte page size\n",
				pagesize);
			exit(1);
	} /* switch */
#else RISCOS
...
#ifndef RISCOS
	getkval(nlst[X_PROC].n_value,  &procp,  sizeof(int),
		nlst[X_PROC].n_name);
	...
	getkval(nlst[X_HZ].n_value,    &hz,    sizeof(int),
		nlst[X_HZ].n_name);
#endif RISCOS
	getkval(nlst[X_CCPU].n_value,  &ccpu,  sizeof ccpu,
		nlst[X_CCPU].n_name);
#ifndef RISCOS
#if defined(scs)
	getkval(nlst[X_SPT].n_value,   &spt,   sizeof(struct spt *),
		nlst[X_SPT].n_name);
#endif scs
#else RISCOS
	procp = (struct proc *) nlst[X_PROC].n_value;
	getkval(nlst[X_V].n_value, &v, sizeof(v),
		nlst[X_V].n_name);
	nproc = v.v_proc;
	hz = HZ;
#endif RISCOS
	...
	swaptab_offset = nlst[X_SWAPTAB].n_value;
	availsmem_offset = nlst[X_AVAILSMEM].n_value;
#endif RISCOS
...
#ifdef RISCOS
    old_sigint = sigset(SIGINT,SIG_HOLD);
    old_sigquit = sigset(SIGQUIT,SIG_HOLD);
    old_sigtstp = sigset(SIGTSTP,SIG_HOLD);
#else RISCOS
#ifndef FOUR_ONE
	...
	getkval(procp, pbase, bytes, "proc array");
	...
	/* get sysinfo */
	getkval(sysinfo_offset, &sysinfo, sizeof(sysinfo), "sysinfo");
	for (i = 0; i < CPUSTATES; i++)
		cp_time[i] = sysinfo.cpu[i];
	getkval(maxmem_offset, &maxmem, sizeof(maxmem), "maxmem");
	getkval(freemem_offset, &freemem, sizeof(freemem), "freemem");

	/* get swaptab */
	getkval(swaptab_offset, swaptab, sizeof(swaptab), "swaptab");
	swapmem = 0;
	for (i = 0; i < MSFILES ; i++ ) 
		if (swaptab[i].st_ucnt != NULL)
			swapmem += swaptab[i].st_npgs;
	getkval(availsmem_offset, &availsmem, sizeof(availsmem), "availsmem");
#endif RISCOS
...
#else RISCOS
	(*d_memory)(
		pagetok(maxmem - freemem),
		pagetok(maxmem - freemem),
		pagetok(swapmem - availsmem),
		pagetok(swapmem - availsmem),
		pagetok(freemem));
#endif RISCOS
...
unsigned char sorted_state[] =
{
    ...
    2,	/* stop			*/
    5,	/* start		*/
    7,	/* onproc (being scheduled) */
    4	/* xbrk (being xswapped) */
#endif RISCOS
};
