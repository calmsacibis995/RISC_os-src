 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: wall.c,v 1.2.1.3 90/05/07 19:50:08 wje Exp $"
...
#ifndef lint
static char sccsid[] = "@(#)wall.c	5.4 (Berkeley) 10/22/87";
#endif not lint
...
char *argv[];
{
	...
	for (i=0; i<c; i++) {
		p = &utmp[i];
#if defined(RISCOS)
		/* The following change was made due to the SysV
		 * format for /etc/utmp.  This is how they do it.
		 */
