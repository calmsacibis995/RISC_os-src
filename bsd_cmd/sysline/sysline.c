 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sysline.c,v 1.3.2.2 90/05/07 19:29:46 wje Exp $"
 ...
 * etc. are gleaned from /etc/termcap.
...
#ifdef mips
fix fix_avenrun[3];	/* Converted to double for above */
#endif
#ifdef RISCOS
struct var v;
#endif RISCOS;
...
/*
 * Tempbuf exists in order to fix problems with doing stringprt() on
 * machines that pass variables differently than a Vax.
 */
...
int ws_cap = 0;		/* set to 1 if termcap 'ws' capability used */
...
#if defined(pdp11) || defined(RISCOS)
	nlist("/unix", nl);
#else
	nlist("/vmunix", nl);
#endif
...
initprocread()
{
	...
	if (nl[NL_PROC].n_value == 0)
		return;
#ifdef RISCOS
	lseek(kmem, (long)nl[NL_V].n_value, 0);
	read(kmem,&v,sizeof(v));
	nproc = v.v_proc;
	if (nproc > 10000 ||
	    nproc <= 0) {
		fprintf(stderr, "/unix does not match running system\n");
		exit(1);
	};
#else RISCOS
#ifdef VMUNIX
...
prtinfo()
{
	...
	if (nl[NL_AVEN].n_value != 0) {
...
#ifdef mips
		read(kmem, fix_avenrun, sizeof(fix_avenrun));
		avenrun[0] = FIX_TO_DBL(fix_avenrun[0]);
		avenrun[1] = FIX_TO_DBL(fix_avenrun[1]);
		avenrun[2] = FIX_TO_DBL(fix_avenrun[2]);
#else
		...
		if ((diff = avenrun[0] - avenrun[1]) < 0.0) {
			sprintf(tempbuf, "%.1f %.1f", avenrun[0], diff);
			stringcat(tempbuf, -1);
		} else {
			sprintf(tempbuf, "%.1f +%.1f", avenrun[0], diff);
			stringcat(tempbuf, -1);
...
mailseen()
{
	...
	while ((n = readline(mfd, lbuf, sizeof lbuf)) >= 0 && !isfrom(lbuf)) {
		;
	}
	if (n < 0) {
	 ...
	 * and print it. Note that this line may be From or >From
	 * if the mail system used is MH.
	 */
	from = lbuf;
	while (*from != ' ') {
		from++;
...
/*
 * Getwinsize gets the window size for the current window. This is avoided
 * if the columns were obtained by using the 'ws' termcap value, so that
 * it doesn't get improperly reset.
 */
...
/*
 * The subroutine isfrom() returns 1 if the buffer begins with the word
 * From followed by a space and optionally preceded by >.
 */
