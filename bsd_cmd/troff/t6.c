 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: t6.c,v 1.1.2.2 90/05/07 19:45:25 wje Exp $"
...
/*
 * The subroutine read_fontfile() takes a file descriptor and a table pointer,
 * and fills the table with 224 (256-32) bytes of information from the file.
 *
 * Each line of the file is either a number of a comment (first character is
 * a '#'), or a decimal number from 0-255, which is stored in the font table
 * for that position (i.e., if the 54th non-comment line in the file is '118',
 * table[53] will be set to 118).
 */

#define FTABSIZE	(256 - 32)

static int get_character();

static void
read_fontfile(fd, table)
int fd;
char *table;
{

	int cur_pos;		/* Current table position		*/
	int accum;		/* Accumulator for current line		*/
	int c;			/* Character from input			*/

	bzero(table, FTABSIZE);
	...
	for ( ; ; ) {
		...
		while (c != '\n') {
			if (c < '0' || c > '9') {
				prstr("Non-numeric character in font file ");
				prstr(fontfile);
				prstr(" (may be non-ASCII font file)\n");
				done(-1);
			}
...
/*
 * The subroutine get_character() reads and returns an ASCII character from
 * the given file descriptor. If the character read is null or non-ASCII, a
 * message is printed and the program exits. The return value is 0 on EOF.
 */
...
int fd;
{
	...
	if ((c & 0x80) || c == 0) {
		prstr("Null or non-ASCII character in font file ");
		prstr(fontfile);
		prstr("\n");
		done(-1);
	}
