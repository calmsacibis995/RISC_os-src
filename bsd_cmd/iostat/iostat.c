 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: iostat.c,v 1.3.1.2 90/05/07 18:42:17 wje Exp $"
	...
	char *argv[];
{
	...
	nlist("/unix", nl);
	if(nl[X_ID].n_type == 0) {
		fprintf(stderr, "%s not found in /unix namelist\n",
			nl[X_ID].n_name);
		exit(1);
	}
#else RISCOS
...
#define	allocate(e, t) \
	...
	for (arg = buf, i = 0; i < dk_ndrive; i++) {
		dr_name[i] = arg;
#ifdef RISCOS
		strncpy(dr_name[i],DevName + strlen(DevName) - 3,2);
		dr_name[i][2] = 0;
...
#ifdef RISCOS
	idstat = (struct iotime *) calloc(dk_ndrive,sizeof(struct iotime));
	lseek(mf, nl[X_V].n_value, L_SET);
	read(mf, &v, sizeof(struct var));
#else RISCOS
	read_names();
...
loop:
...
#ifdef RISCOS
	{
		...
		if (nl[X_ID].n_type == 0)
			return;
		lseek(mf, nl[X_ID].n_value, L_SET);
		read(mf,idstat,sizeof(struct iotime) * dk_ndrive);
		for (i = 0; i < dk_ndrive; i++) {
