 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: tip.c,v 1.1.2.9 90/05/28 17:53:00 wje Exp $"
...
cucommon:
	...
	if (DU)
		change_host_clocal(0);
			 /* turn off CLOCAL, so we die on disconnect */
	if (vtable[LOCAL].v_access & CHANGED) {
		...
		switch (clocal) {
		case 0:
		  	vassign(vtable + LOCAL,"!");
			break;
		case 1:
			vassign(vtable + LOCAL," ");
			break;
		default:
			vassign(vtable + LOCAL,(DU ? "!" : " "));
			break;
		};
#endif RISCOS
...
child_disconnected()
{
	...
	if (ioctl(FD, TIOCGETD, (char *)&ldisc) == -1) {
...
#ifdef RISCOS
	signal(SIGEMT,child_disconnected);
#endif RISCOS
	/*
	 * Kinda klugey here...
	...
	int speed;
{
	...
	struct	sgttyb new_arg;

#ifdef RISCOS
	init_termio(FD);
#endif RISCOS
	new_arg = arg;
	new_arg.sg_ispeed = new_arg.sg_ospeed = speed;
	new_arg.sg_flags = RAW;
	if (boolean(value(TAND)))
		new_arg.sg_flags |= TANDEM;
	ioctl(FD, TIOCSETP, (char *)&new_arg);
