 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sh.lex.c,v 1.3.1.2 90/05/07 18:16:35 wje Exp $"
...
#ifdef FILEC
	char ttyline[BUFSIZ];
	register int numleft = 0;
#endif
	register int roomleft;
#ifdef PREPROCESS
	int pp_style;
#define STYLE_NONE	0
#define STYLE_FILEC	1
#define STYLE_TSH	2
#endif
	...
	if (cantell) {
		...
		if (fseekp == feobp) {
			fbobp = feobp;
#ifdef  TSH
/* If this is a TTY, call our line editor routines to get input. */
			if (intty && tsh_line_edit) {
		      /* The ignoring of interrupts is accomplished at
			 a lower level */
			    c = tsh_read(SHIN, SHOUT, fbuf[0], BUFSIZ);
			}
			else {
			    do
...
#ifdef TSH
			case STYLE_TSH:
				c = tsh_read(SHIN, SHOUT, fbuf[buf] + off, roomleft);
				break;
#endif
