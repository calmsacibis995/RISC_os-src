 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sh.func.c,v 1.7.1.3.1.1.1.2 90/10/17 18:13:45 beacker Exp $"
	...
	char **v;
{
	...
	/*
	 * No longer exit if /bin/login failed.
...
retry:
...
#ifdef TERMIO
	{
		...
		(void) ioctl(FSHTTY, TCGETA, (char *)&termio);
		oldisc = termio.c_line;
		if (oldisc != LDISC_NEW) {
			printf("Switching to new tty driver...\n");
			termio.c_line = LDISC_NEW;
			(void) ioctl(FSHTTY, TCSETAW, (char *)&termio);
		}
	}
#else TERMIO
