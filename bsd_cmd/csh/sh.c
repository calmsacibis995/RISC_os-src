 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sh.c,v 1.4.1.2 90/05/07 18:13:31 wje Exp $"
...
#ifdef  TSH
/*
 * This must precede .login so users can set shell variables in their .login
 * files and thereby affect the line editor.
 */
	tsh_init();
#endif
...
/*
 * There is code here to prevent from automatically source'ing $home/.cshrc
 * and $home/.login if you don't own them. This probably isn't a good
 * idea in two cases:
 *
 *	1. A program may exist whose sole purpose is to change your uid
 *	   and groups without changing you to that user (for example, su -e
 *	   in some versions of Unix).
 *
 *	2. You may be sharing a home directory with someone else. This was
 *	   the case with a group I worked in once.
 */
...
/*
 * The subroutine printprompt() prints the prompt. The prompt is a vector,
 * and each element is printed in rotation. When a prompt is printed, it
 * may contain the special items !, and \@x. A ! is replaced by the
 * current history number. A \@x is replaced
 * by the time defined by timepr(x) (see that routine for more info).
 */
