 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sh.set.c,v 1.3.1.2 90/05/07 18:18:08 wje Exp $"
...
#if RISCOS
		} else if (eq(vp, "logname")) {
			setenv("LOGNAME", value(vp));
#endif
		} else if (eq(vp, "term")) {
...
#ifdef  TSH
		} else if (eq(vp, "lineeditmin")) {
		/* Let the "lineeditmin" shell variable specify the minimum
		   length line which the editor will use for ^N/^P */
			int temp;
			char *ctemp = value(vp);
			if ((!ctemp) ||
			   (!isdigit(*ctemp))
			   || ((temp = atoi(ctemp)) < 1))
			   {
			   bferr("lineeditmin treated as 1");
			   temp = 1;
			   }
			tsh_set_min(temp);
			setenv("LINEEDITMIN", value(vp));
		} else if (eq(vp, "lineeditchars")) {
		/* Let the "lineeditchars" shell variable specify the set
		   of characters used by the line editor for commands */
			tsh_set_edit_chars( value(vp) );
			setenv("LINEEDITCHARS", value(vp));
		} else if (eq(vp, "lineedit")) {
			tsh_line_edit = 1;
			tsh_set_crt_chars( value(vp) );
 			setenv("LINEEDIT", value(vp));
#endif
#ifdef FILEC
		} else if (eq(vp, "filec")) {
			filec = 1;
		} else if (eq(vp, "fastlist")) {
			fastlist = 1;
#endif
		}
	} while (p = *v++);
	...
	char *v[];
{
	...
	if (adrof("lineedit") == 0) {
		tsh_line_edit = 0;
		tsh_set_crt_chars( (char *) 0 );
		unsetenv("LINEEDIT");
	}
	if (adrof("lineeditmin") == 0) {
		tsh_set_min(1);
		unsetenv("LINEEDITMIN");
	}
	if (adrof("lineeditchars") == 0) {
		tsh_set_edit_chars((char *) 0);
		unsetenv("LINEEDITCHARS");
	}
#endif
#ifdef FILEC
	if (adrof("filec") == 0) {
