 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: errorinput.c,v 1.1.2.2 90/05/07 18:30:16 wje Exp $"
	...
	Eptr	**r_errorv;
{
	...
	if ( wordc > 0 &&
	   ...
	   || (( errorclass = mipsgeneric() )!= C_UNKNOWN)
...
/*
 * MIPS generic error message. Format is:
 *
 *	{component}: {tag}: {filename}, line {number}: {error message}
 *
 */
...
mipsgeneric()
{
	...
	if ((lastchar(wordv[1]) != ':') ||
	    (lastchar(wordv[2]) != ':') ||
	    (lastchar(wordv[3]) != ',') ||
	    (strcmp(wordv[4], "line") != 0) ||
	    (!isdigit(firstchar(wordv[5]))) ||
	    (lastchar(wordv[5]) != ':')) {
		return C_UNKNOWN;
	}
	...
	if (strcmp(wordv[1], "cpp") == 0) {
		language = INCPP;
	} else if (strcmp(wordv[1], "as0") == 0 ||
		   strcmp(wordv[1], "as1") == 0) {
		language = INAS;
	} else if (strcmp(wordv[1], "ccom") == 0) {
		language = INCC;
	} else if (strcmp(wordv[1], "fcom") == 0) {
		language = INF77;
	}

	/*
	 * Rebuild the line. The format must become:
	 *
	 *	filename linenumber message
	 *
	 */
