 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: run.c,v 1.1.2.3 90/05/07 18:05:58 wje Exp $"
#ifndef lint
static char sccsid[] = "@(#)run.c	4.8 3/29/88";
#endif
...
char *format(s,a) char *s; node *a;
{
	...
	if (p == NULL)
		error(FATAL, "format unable to malloc buffer");
	while (*s) {
		if (*s != '%') {
			*p++ = *s++;
			continue;
