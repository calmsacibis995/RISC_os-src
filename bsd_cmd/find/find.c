 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: find.c,v 1.3.1.2.1.1.1.3 90/12/20 19:02:27 beacker Exp $"
...
char *pfstype;		/* parent's fstype */
{
	...
	if (follow) {
	  	lstat(fname, &statf);
		if ((statf.st_mode&S_IFMT) == S_IFLNK)
		  	getwd(prev_wd);
	}
	...
	c1 = endofname;
	*c1 = '\0';
	if(cdval == -1 || (follow && ((statf.st_mode&S_IFMT)==S_IFLNK) ?
			   chdir(prev_wd) : chdir("..")) == -1) {
	...
	char *file;
{
	...
	while ((mnt = getmntent(mntp)) != 0) {
		...
		if (stat(mnt->mnt_dir, &dirstat) < 0) {
			fprintf(stderr, "find: can't stat %s: ", mnt->mnt_dir);
			perror("");
			endmntent(mntp);
			return(NULL);
		}
...
#ifdef RISCOS
	if (filestat.st_dev != -1)
	  	fprintf(stderr, "Couldn't find mount point (file system type) for %s\n", file);
	return (NULL);
#else
