 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: svcauth_des.c,v 1.3.1.2 90/05/07 21:03:18 wje Exp $"
/*
 * @(#)svcauth_des.c 1.9  88/08/02 4.0NFSSRC Copyr 1988 Sun Micro
 * @(#)svcauth_des.c 1.15 88/02/08 SMI
 *
 ...
 *
 * It is up to the server to determine if the window size is too small.
 */
...
#ifdef KERNEL
#include "sys/systm.h" /* "sys/kernel.h" */
#include <bsd43/sys/syslog.h>
#define debug(msg)	 log(BSD43_LOG_ERR,"svcauth_des: %s\n", msg)
...
#endif
#define debug(msg)	 syslog(LOG_ERR,"svcauth_des: %s\n", msg)
#endif KERNEL
...
#if (defined(sun) || defined(vax) || mips)
