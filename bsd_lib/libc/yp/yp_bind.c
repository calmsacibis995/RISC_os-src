 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: yp_bind.c,v 1.3.1.4 90/05/07 21:29:04 wje Exp $"
...
/*#include <sys/file.h>*/  /* kludge for the moment */
#ifdef RISCOS
#ifdef SYSTYPE_BSD43
#include <syslog.h>
#else SYSTYPE_BSD43
...
#ifdef SYSTYPE_SYSV
/* this stuff is to let yp_bind use an flock() system call in both
 * sysv and bsd land.  This assumes that flock is p/o libbsd and libsun
 */
#include <bsd43/fcntl.h>
/* bsd defines L_SET sysv doesn't, its a 0 */
#define L_SET 0
#endif
	...
	struct dom_binding **binding;	/* if result == 0, ptr to dom_binding */
{
...
#endif
#ifdef SYSTYPE_SYSV
			if (flock(bf,BSD43_LOCK_EX+BSD43_LOCK_NB) == 0) {
#endif
				close(bf);
	 ...
	 */
#ifdef RISCOS
        /* We couldn't afford to loop forever if there is a probelm.
         * Here is one of the case:
         *   1) start ypbind in local hosts.
         *   2) stop ypbind manually.
         *   3) try to call this procedure.
         *   4) we will loop forever here.
         * We shall try 3 times in outer "for" loop and 2 times in inner 
	 * "for" loop.  
         */
#endif
	for (bound = FALSE, vers = YPBINDVERS; !bound; ) {
                ...
                j++;
                if ( j == 3 ) {
		/* if we can't do it then return with error */
                        syslog(LOG_ERR, "yp_bind: _yp_dobind problem\n");
                        return(YPERR_YPBIND);
                }
#endif
		if (binder_port) {
...
#ifdef RISCOS
		/* Again, we can't afford to loop forever */
		    binder_port && (loopctl != BS_OK) && tries < 3; tries++) {
#else
