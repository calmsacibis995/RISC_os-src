 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: co.c,v 1.6.1.4 90/05/10 00:26:09 wje Exp $"
...
char * argv[];
{
	...
	catchints();
        cmdid = "co";
        cmdusage = "command format:\nco -b[rev] -l[rev] -p[rev] -q[rev] -r[rev] -ddate -sstate -w[login] -jjoinlist file ...";
	...
	branchflag = false;
		...
		case 'b':
			...
			goto revno;
				...
				faterror("missing date for -d option");
			}
                        /* process date/time */
				...
				faterror("missing joinlist for -j option");
			}
                        break;
				...
				faterror("missing state for -s option");
			}
                        break;
