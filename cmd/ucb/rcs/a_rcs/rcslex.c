 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: rcslex.c,v 1.3.2.2 90/05/10 00:23:07 wje Exp $"
...
{       nerror++;
	if (in_faterror)
	  exit(3);
	in_faterror++;
        VOID fprintf_final(stderr,"%s error: ",cmdid);
        VOID fprintf_final(stderr,e, e1, e2, e3, e4, e5);
        VOID fprintf_final(stderr,"\n%s aborted\n",cmdid);
        cleanup();
        exit(1);
}
...
fprintf_final(iop, fmt, va_alist)
FILE *iop;
char *fmt;
va_dcl
/* Function: formatted output. Same as fprintf in stdio,
 * but ignores error (for use by faterror()).
 */
{
	va_list ap;
