 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: rcsrev.c,v 1.3.2.2 90/05/10 00:23:51 wje Exp $"
...
int branchflag = 0;	/* -b: follow branch to end */
                ...
                if ((next==nil) ||
                    (cmpnumfld(branchnum,next->num,1)!=0))/*overshot*/ {
                        ...
                        *store++ = next;
                        ...
                        /* Check to see if there is a branch and follow */
                        /* it if there is.                              */
                        if (branchflag && next->branches != nil) {
				char *place;
                                strcpy (t, next->branches->hsh->num);
				place = (char *) rindex(t, '.');
				if (place) {
					*place = '\0';
					length = countnumflds(t);
					return genbranch(next,t,length,date,
						 author,state,store);
				}
                        }
                }
...
int argc; char * argv[];
{
        ...
        do {
                ...
                target=genrevs(numricrevno,*date=='\0'?(char *)nil:date, *author=='\0'?nil:author,
