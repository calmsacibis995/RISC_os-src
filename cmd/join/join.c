 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: join.c,v 1.8.2.2 90/05/09 16:17:40 wje Exp $"
	...
	while (argc > 1 && argv[1][0] == '-') {
		...
		case 'a':
			...
			case '2':
				...
				break;
			default:
				error("Invalid file number (%s) for -a",
					&(argv[1][2]));
			}
			break;
		case 'e':
		...
		case 'o':
			for (no = 0; no < 2*NFLD; no++) {
				if (argv[2][0] == '1' && argv[2][1] == '.') {
					...
					error("Invalid file number (%s) for -o",
						argv[2]);
				}
				if (olist[no] < 1) {
					error("Invalid field number (%s) for -o",
						&(argv[2][2]));
				}
				argc--;
				argv++;
		...
		case 'j':
			if (argv[1][2] == '1')
				...
				error("Invalid file number (%s) for -j",
					&(argv[1][2]));
			}
			if (j1 < 1 || j2 < 1) {
				error("Invalid field number (%s) for -j",
					argv[2]);
			}
			argc--;
			argv++;
...
usage()
{
	fprintf(stderr, "join: usage: join [-an] [-e s] [-jn m] [-tc] [-o list] file1 file2\n");
}
