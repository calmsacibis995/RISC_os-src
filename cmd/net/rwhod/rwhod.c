 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: rwhod.c,v 1.14.2.3 90/05/09 17:23:03 wje Exp $"
...
main()
{
	...
	for (;;) {
		 ...
		 */
#if RISCOS
		(void) chmod(path, 0644);	/* just in case */
#endif
...
onalrm()
{
	...
	for (i = 0; i < 3; i++)
#if RISCOS
		mywd.wd_loadav[i] = FIX_TO_DBL(avenrun[i]) * 100;
#else
...
loop:
	...
	if (check_kernel_id (kmemf,
			(long)nl[NL_ID_STRING].n_value, "/unix") > 0) {
		syslog(LOG_WARNING,"/unix does not match /dev/kmem");
		close(kmemf);
		kmemf = -1;
		sleep(300);
		goto loop;
