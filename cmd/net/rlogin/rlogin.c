 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: rlogin.c,v 1.17.2.2 90/05/09 17:16:43 wje Exp $"
...
/* These are not defined in sysv land */
/* From bsd/net/soioctl.h or bsd43/sys/ioctl.h */
#define	SIOCATMARK	_IOR(s,  7, int)		/* at oob mark? */
/* From bsd43/sys/ioctl.h */
...
#define t_suspc c_cc[V_SUSP]
#define t_dsuspc c_cc[V_DSUSP]
	...
	for (;;) {
		...
		if (bol) {
			...
			if (c == '!') {
				...
				ioctl(0, TCGETA, (char*)&tb);
				ioctl(0, TCSETA, (char*)&def_termio);
				if ((shellpid = fork()) < 0) {
					prf("\007rlogin: cannot fork: %d",
					    errno);
				} else if (!shellpid) {
					register int i;
					...
					register int pid = wait((int*)0);
					if (pid == shellpid)
						break;
					if (pid == child)
						child = -1;
				}
				ioctl(0, TCSETA, (char*)&tb);
...
oob()
{
	...
	if (!eight && (mark & TIOCPKT_NOSTOP)) {
		...
		if (def_termio.c_iflag & IXON) {
			(void) ioctl(0, TCGETA, (char *)&sb);
			sb.c_iflag &= ~IXON;
			(void) ioctl(0, TCSETA, (char *)&sb);
		}
#else
		(void) ioctl(0, TIOCGETP, (char *)&sb);
		sb.sg_flags &= ~CBREAK;
