 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: rt_init.c,v 1.2.1.3 90/05/09 17:02:23 wje Exp $"
/*
...
#ifndef	lint
static char *rcsid = "$Header: rt_init.c,v 1.2.1.3 90/05/09 17:02:23 wje Exp $";
#endif	not lint
...
rt_readkernel()
{
  ...
  { 
	...
	for (i=0;i<sizeof(nl)/sizeof(struct nlist);i++)
		if (nl[i].n_name[0] == '_')
			for (j=0;nl[i].n_name[j]!='\0';j++)
				nl[i].n_name[j]=nl[i].n_name[j+1];
  }
  (void) nlist("/unix", nl);
  { 
  	int i;
	for (i=0;i<sizeof(nl)/sizeof(struct nlist);i++)
		nl[i].n_value &= 0x7fffffff;
  }
#else
...
#define lseek(kmem,offset,how)	klseek(offset)
  ...
  if (check_kernel_id (kmem,
		(long)nl[N_ID_STRING].n_value, "/unix") > 0) {
	p_error("/unix does not match /dev/kmem\n");
	quit();
  };
#endif RISCOS
