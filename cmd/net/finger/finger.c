 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: finger.c,v 1.1.2.3 90/05/09 16:54:51 wje Exp $"
 ...
 * of the user, and, if it exists, a copy of the file .plan in the users
...
#if RISCOS
#define HMAX 3			/* a nice, small number; see its usage below */
#else
	...
	char **argv;
{
	...
	while (read(uf, (char *)&user, sizeof user) == sizeof user) {
#if RISCOS
		if (*user.ut_name == 0 || user.ut_type != USER_PROCESS)
#else
		if (*user.ut_name == 0)
#endif
			continue;
	...
	if (*pers->tty) {
		if (pers->tty[0] == 't' && pers->tty[1] == 't' &&
			...
			if (pers->tty[3] == 'd' && pers->loggedin)
				dialup = 1;
			printf("%-6.6s ", pers->tty + 3);
		} else
			printf("%-6.6s ", pers->tty);
	} else
		printf("   ");
...
#if RISCOS
		; /* ignore error; rest of code "does the right thing" */
#else
