 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: bfs.c,v 1.6.2.4 90/05/10 20:15:34 wje Exp $"
...
char file[];
{
	...
	if((txtfd=open(file,0)) < 0) return(err(1,"can't open input file"));
...
getc()
{
	if(!peeked) {
		while((!(infildes == oldfd && flag)) && (!flag1) && (!readc(infildes,&charbuf))) {
			...
			if((infildes=pop(fstack)) == -1) quit(0);
