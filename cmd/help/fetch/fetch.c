 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: fetch.c,v 1.7.2.2 90/05/09 16:10:32 wje Exp $"
...
#ifdef BSD42
#define TEMPLATE_BSD42	"/tmp/arXXXXXX"
#endif BSD42
...
char	*tempnam();
#else
	char *mktemp(), _temp_buf[sizeof(TEMPLATE_BSD42)];
#endif
char	*ctime();
long	time();
...
{
#ifdef BSD42
	strcpy (_temp_buf, TEMPLATE_BSD42);
	tfnam = mktemp (_temp_buf);
#else	
...
bamatch()
{
	register f;
#ifdef BSD42
	char *mktemp(), _tempfile_buf[sizeof(TEMPLATE_BSD42)];
#endif 	
	...
	case 2:
		bastate = 0;
#ifdef BSD42
		strcpy (_tempfile_buf, TEMPLATE_BSD42);
		tf1nam = mktemp (TEMPLATE_BSD42);
#else		
