 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: interact.c,v 1.6.2.2 90/05/09 16:12:35 wje Exp $"
...
{
#ifdef notdef
	static int	(*oldhup)(), (*oldquit)(), (*savint)();
#else
	int	(*oldhup)(), (*oldquit)(), (*savint)();
#endif	
	static int	pid, rpid, retcode;

	oldhup = (int (*)())signal(SIGHUP, SIG_IGN);
	oldquit = (int (*)())signal(SIGQUIT, SIG_IGN);
	resetterm();
	if((pid = fork()) == 0) {
