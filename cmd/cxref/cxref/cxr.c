 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: cxr.c,v 1.5.2.3 90/05/09 15:35:11 wje Exp $"
...
int	sawone = 0;   /* did we process any files? */

/* compiler environment variable */
char *systype = 0;
char *comp_target_root = 0;
char include_base[300];
char include_default[300];

main(argc,argv)
	int argc;
	char *argv[];
{
	...
	while (--argc > 0) {
		++argv;
		sblk = 0;
		/* special case for -systype xxx */
		if (strcmp(*argv,"-systype") == 0)
		{
		    if ( --argc > 0 )
		    {
			if (systype != 0)
			{
			    fprintf(stderr,
			    "cxref: only one -systype option allowed\n");
			    dexit(1);
			}
			systype = *++argv;
		    }
		    ...
		    {
			fprintf(stderr,
			"cxref: missing argument for -systype\n");
			dexit(1);
		    }
		}
		if (**argv == '-') {
			switch (*++*argv) {
				...
				case 'I' :
					...
					{
					    fprintf(stderr,
					    "cxref: bad -I argument (%s)\n",
					    --*argv);
					    dexit(1);
					}
					if (incs < MAXINC) {
/* later we'll add in COMP_TARGET_ROOT and systype, so get just the name
 * skipping by the "-I"
 */
						incdirs[incs++] = ++*argv;
					}
				...
				case 'o':
					...
					continue;
#ifdef DEBUG
/* get the xcpp or xref from non-std places */
				case 'j':
				    xref = *++argv;
				    argc--;
				    continue;
...
#endif
				default:
					fprintf(stderr, "cxref: invalid option '%c' - ignored\n", **argv);
			};
		}
		else if (strcmp(*argv + strlen(*argv) - 2, ".c") != 0)
		{
			fprintf(stderr,"cxref: %s is not a C file, ignored\n",*argv);
			...
			{
			    fprintf(stderr,
			    "cxref: no C input files specified\n");
			    dexit(1);
			}
			cfile = *argv;
			if (silent == NO)
	...
	if ( sawone == 0)
	{
	    fprintf(stderr,"cxref: no C input files specified\n");
	    dexit(1);
	}
	if (cflg == YES) {
		sortfile();	/* sorts temp file */
...
doxref()
{
	...
	strcpy(include_base,"-I");
	if ( comp_target_root != 0)
	{
	    strcat(include_base, "/");
	    strcat(include_base,comp_target_root);
	}
	if ( systype != 0)
	{
	    strcat(include_base, "/");
	    strcat(include_base,systype);
	}
	...
	arv[8] = "-Dhost_mips";
	arv[9] = "-DMIPSEB";
	for (n = 10, i = 0; i < defs; i++) {
		arv[n++] = predefs[i];
	};
	if (systype != 0)
	{
	    /* generate -DSYSTYPE_xxx */
	    p = systype;
	    while (*p)
		*p++ = toupper(*p);
	    strcpy(argsystype,"-DSYSTYPE_");
	    strcat(argsystype,systype);
	    arv[n++] = argsystype;
	}
	else
	{
	...
	if ( (comp_target_root != 0) || (systype != 0) )
	{
	    strcpy(include_default,include_base);
	    strcat(include_default,"/usr/include");
	    arv[n++] = include_default;
	}
	for (i = 0; i < incs; i++) {
		strcpy(&argdirs[i][0],include_base);
		strcat(&argdirs[i][0],incdirs[i]);
		arv[n++] = &argdirs[i][0];
	};
	arv[n] = 0 ;
...
mktmpfl()
{
	...
	tmp1 = tempnam(NULL, "xr1");
	tmp2 = tempnam(NULL, "xr2");	/* holds output of "xpass" */
	tmp3 = tempnam(NULL, "xr3");	/* holds output of tmpscan() routine */
	tmp4 = tempnam(NULL, "xr4");	/* holds output of tempfile 3 */
