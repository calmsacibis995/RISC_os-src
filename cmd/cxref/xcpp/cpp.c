 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: cpp.c,v 1.6.2.3 90/05/09 15:35:40 wje Exp $"
...
# define WARN DROP
# define SAME 0
# define MAXINC 30      /* was 10. bumped for rel. 1.30 --uk 11/30/87 */
# define MAXFRE 14	/* max buffers of macro pushback */
# define MAXFRM 64	/* max number of formals/actuals to a macro */
# define MAXIDIRS 50	/* max number of -I's, added const --mad 7/23/86 */
                        /* bumped for rel. 1.30--uk 12/1/87  */
...
/* LFL 3-9-88 */
int pascal_file = 0;		/* True if LANGUAGE_PASCAL is defined */
#if tgp
...
STATIC	char	*dirs[MAXIDIRS];	/* -I and <> directories */
...
#define BACK  3
STATIC	int	ifno;
# define NPREDEF 50	/* was 20, bumped for kernel --mad 7/23/86 */
                        /* bumped for rel.1.30--uk 12/1/87 */
...
again:
	...
	case '/': 
	  ...
	  /* 3-9-88 LFL */
	  /* Handle pascal comments if appropriate */
	case '(':
	  if (*p != '*') break;
	  /* else fallthrough */
	case '{':
	  if (!pascal_file) break;
	  /* LFL 6-19-88 */
	  if (flslvl) break;	/* Don't bother with comments after #if 0 */
	  if (p[-1] == '{') --p;
	  incomment++;
	  if (!passcom) {
	    *inp++=' ';		/* 8-4-88 LFL - Hack to handle const{}a=1; */
	    dump();
	    ++flslvl;
	  }
	  for (;;) {		/* Loop until out of comment */
	    ++p;
	    if eob(p) {
	      if (!passcom)
		inp = p;
...
char *
dopragma(p)   /* process '#pragma' */
   char *p;
{  char *pin;
   char *psav,*space;
   pin=inp;
   space=psav=malloc(WORK_AREA);
   for(;;) { /* accumulate pragma until linefeed */
      outp=inp=p;
      p=cotoken(p);
      pin=inp;
      if (*pin=='\n') break;
	...
	*psav++=params; *psav++='\0';
	...
	if (np->value == psav-1) {
			...
			np->value += newspace-space;
                ...
                else {pperror("macro definition too long");exit(exfail);}
	...
	*--inp=SALT; outp=inp; ++flslvl; np=slookup(inp,p,0); --flslvl;
	if (np==defloc) {/* define */
		if (flslvl==0) {p=dodef(p); continue;}
             ...
             if ( flslvl==0) {p=dopragma(p);continue;}
		...
		/* remove this cause we don't want to want to pass it
		 * -- if we ever do remove this comment around the following
		 * 	code
		 *	while(*inp!='\n') p=cotoken(p);
		 */
...
subst(p,sp) register char *p; struct symtab *sp; {
	...
	if (0!=(params= *--vp&0xFF)) {/* definition calls for params */
		...
		/* For bug #2136 */
		maclin=lineno[ifno]; macfil=fnames[ifno]; /* LFL 3-9-88 */
		if (*inp=='(') {
			for (plvl=1; plvl!=0; ) {
				*ca++='\0';
				for (;;) {
	...
	char *argv[];
{
	...
	register char *p;
	char *tf,**cp2;
	int inc_std = 1; /* include standard directories in the search path */
...
# endif
				...
				case 'I':
					/*
					 * Leave room for two "standard"
					 * include directories and a NULL
					 */
					if (nd >= (MAXIDIRS-3))
					    pperror(
					      "excessive -I file (%s) ignored",
					      argv[i]);
					else if (argv[i][2] == '\0')
					    inc_std = 0; /* no std dirs */
					else dirs[nd++] = argv[i]+2;
					continue;
				case '\0': continue;
				case 'v': continue;
				case 'm':
					if (strcmp (argv[i], "-mips2") == 0)
					    continue;
#ifdef CXREF
				case 'F':
...
# if unix
#  if defined(CROSS) || defined(MOXIE)
	if (inc_std)
	  dirs[nd++] = "/usr/local/mips/include";
#  else
	if (inc_std) {
	  dirs[nd++] = "/usr/include";
	}
...
# endif
	...
	pascal_file = (lookup("LANGUAGE_PASCAL",0)->value != 0); /* 3-9-88 */
	if (pascal_file) {
	  (fastab+COFF)['{'] |= SB;
	  (fastab+COFF)['('] |= SB;
	}
