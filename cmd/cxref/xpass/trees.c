 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: trees.c,v 1.5.2.3 90/05/09 15:39:13 wje Exp $"

/* $Log:	trees.c,v $
 * Revision 1.5.2.3  90/05/09  15:39:13  wje
 * add restricted rights legend
 * 
 * Revision 1.5.2.2  89/12/10  22:27:29  wje
 * add RID 1.6 to branch RISCOS_4_50_FIXES
 * 
 * Revision 1.6  89/12/10  18:43:35  jay
 * replaced with files from 2.0 C compiler's lint, modified to support cxref.
 * closes bugs 1373
 * 
 * Revision 2000.7  89/03/26  14:48:28  bettina
 * 2.0G
 * 
 * Revision 1040.6  88/09/29  15:53:23  bettina
 * bootstraped 1.40
 * 
 * Revision 1.4  88/01/12  11:05:35  gb
 * fix #2126, restoring -float option after unprototyped function calls.
 * 
 * Revision 1.3  87/12/16  14:34:13  gb
 * fix bug #1956, spurious err on cast in a constant expression init.
 * 
 * Revision 1.2  87/12/09  11:42:24  gb
 * added $Log keyword
 *  */
# include "mfile1"
# include "messages.h"
...
/* !01		20mar85		tracking offsets into static area for   */
/*				offset field in IMPV, EXPV UCODES.	*/
/* !02		12apr85		ucode needs explicit conversions        */
/* !03		17may85		introduced volatile			*/
/* !04		25jun85		assignment operations were converting
				the right-hand side to the left-hand side
				before the operation.			*/
/* !05          22sep85         access field at its natural boundary    */
/* !06		22oct85		constant folding of 'casted' expressions  */
/*				was wrong.			        */
/* !07		19dec85		check for register variables, they look */
/*				auto variables.				*/
/* !08          26jan85         volatile attribute was causing conversions*/
/*				to take place where there not supposed to*/
...
/* Default is double precision.  Single Precision turn on with -single option.
*/
int floatflag = 0;	/* this one always reflects the user's -float choice */
int singflag = 0;	/* this way changes during func call arg processing */
...
# endif
	...
	if( actions&LVAL ){ /* check left descendent */
		...
		if( (q->in.op == UNARY MUL && q->in.left->in.op == PLUS &&
		     q->in.left->in.left->in.op == PCONV) &&
		    ((q->in.left->in.left->in.left->in.op == UNARY STCALL) ||
		      (q->in.left->in.left->in.left->in.op == STCALL)))  {
			UERROR( MESSAGE( 62 ) );	/* f().x = y*/
		}
	}
	...
	if( actions & OTHER ){
		...
		case NAME:
			...
			p->tn.prototype = sp->sprototype;
			p->in.type = sp->stype;
			p->fn.cdim = sp->dimoff;
			p->fn.csiz = sp->sizoff;
			/*p->tn.lval = sp->offset;			!01*/
			p->tn.rval = idname;
			/* special case: MOETY is really an ICON... */
			if( ISEQUAL(p->in.type,MOETY) ){
				p->tn.rval = NONAME;
				p->tn.lval = sp->offset;
				p->fn.cdim = 0;
				p->in.type = ENUMTY;
		...
		case STREF:
			...
			if( stab[i].sflags & SNONUNIQ &&
				(ISEQUAL(l->in.type,PTR+STRTY) ||	/*!03*/
				 ISEQUAL(l->in.type, PTR+UNIONTY)) &&
				(l->fn.csiz +1) >= 0 ){
...
# endif
				...
				register j;
				register align;
				if( !ISEQUAL(l->in.type, PTR+STRTY) &&	/*!03*/
			            !ISEQUAL(l->in.type, PTR+UNIONTY) )
			        {
...
# endif
		...
		case UNARY CALL:
                        valid_argument_expressions( l, p); 
			/* "illegal function" */
			...
			if( ISEQUAL(p->in.type, STRTY) || 
			    ISEQUAL(p->in.type, UNIONTY) ){
...
strargs( p ) register NODE *p;  { /* rewrite structure flavored arguments */
	...
	if( ISEQUAL(p->in.type, STRTY) || ISEQUAL(p->in.type,UNIONTY) ){
...
# endif
	...
	if( (k = j) < 0 ) UERROR( MESSAGE( 112 ) );
		...
		for( ; (kk = dimtab[k] ) >= 0; ++k ){
			...
			if( kk == i ) return( 1 );
			switch( NORM(stab[kk].stype) ){

			case STRTY:
			case UNIONTY:
				if( ISEQUAL(type, STRTY) ) continue;  /* no recursive looking for strs */
...
conval( p, o, q ) register NODE *p, *q; {
	...
	if( q ) val = q->tn.lval;
	u = ISUNSIGNED(p->in.type) || (q && ISUNSIGNED(q->in.type));
	if( u && q && ISEQUAL(UNSIGNED, q->in.type) && !ISUNSIGNED(p->in.type))
		p->in.type = UNSIGNED; /* q->in.type came from sizeof */
	if( u && (o==LE||o==LT||o==GE||o==GT)) o += (UGE-GE);

	if( p->tn.rval != NONAME && q && q->tn.rval != NONAME ) return(0);
	if( q && q->tn.rval != NONAME && o!=PLUS ) return(0);
	if( p->tn.rval != NONAME && o!=PLUS && o!=MINUS ) return(0);

	if( SZLONG > SZINT ){
		if( p->in.op == ICON && ( ISEQUAL(p->in.type,LONG) ||
		    (q && ISEQUAL(q->in.type,LONG)) ) ) {
				  p->in.type = LONG;
				  if( q ) q->in.type = LONG;
				}
		}
	...
	case UNARY MINUS:
		if( SZLONG > SZINT && (ISEQUAL(p->tn.type,UNSIGNED)) ) /* its an ICON from sizeof */
...
chkpun(p) register NODE *p; {
	...
	/* this is called when the op of p is ASSIGN, PASSIGN, RETURN, CAST, COLON, or relational */
	...
	/* in the case of ASSIGN or PASSIGN, any assignment of pointer to  */
        /*   integer is illegal  this falls out, because the LHS is never 0 */

	register NODE *q;
	register t1, t2;
	register d1, d2;

	t1 = p->in.left->in.type;					/*!03*/
	t2 = p->in.right->in.type;					/*!03*/

	if( ISEQUAL(t1, ENUMTY) || ISEQUAL(t2, ENUMTY) ) { 
	/* check for enumerations */
		...
		if( p->in.op == PASSIGN )  WERROR( MESSAGE( 138 ) );
		else WERROR( MESSAGE( 37 ), opst[p->in.op] );
		return;
		}
	...
	if( !ISPTR(q->in.type) && !ISARY(q->in.type) ){
		if( q->in.op != ICON || q->tn.lval != 0 ){
			...
			if( p->in.op == PASSIGN ) WERROR( MESSAGE( 138 ) );
                        else WERROR( MESSAGE( 53 ), opst[p->in.op] );
			}
		}
	else {
...
stref( p ) register NODE *p; {
	...
	if( !ISPTR(p->in.type)){
		p->in.type = PTR+UNIONTY;
     	...
     	t = INCREF(q->stype);	
	if( BTQ(p->in.type) ) /* inherit volatility */
	  t = SETBTV(t);
	d = q->dimoff;
	s = q->sizoff;
        attach_prototype(p, q->sprototype);
...
pconvert( p ) register NODE *p; {
	...
	if( ISARY( p->in.type) ){
		p->in.type = DECREF( p->in.type );
		++p->fn.cdim;
		p->in.mflags = (p->in.mflags & ~VREG);			/*!07*/
...
ptmatch(p)  register NODE *p; {
	...
	case PASSIGN:
        case ASSIGN:
		if( !ISPTV(t1) && ISPTV(t2) ) {
		/* pointer to volatile assigned to pointer to non-volatile */
		  if( o == PASSIGN ) UERROR( MESSAGE( 138 ) );
                  else UERROR( MESSAGE( 132 ));
		}
        case RETURN:
	case CAST:
		{  break; }
	...
	case COLON:
		/* "illegal types in :" */
		{  if( !ISEQUAL(t1, t2) ) UERROR( MESSAGE( 71 ));
		   break;
		   }
	...
	/* don't paint over volatile part of the right hand side */
	if( ISEQUAL(t1,ENUMTY) && ISEQUAL(t2,ENUMTY) && 
	    ( (o == ASSIGN) || (o == PASSIGN) ))
	  p->in.right = makety( p->in.right, t2, d, s );
	else p->in.right = makety( p->in.right, t, d, s );
	if( o!=MINUS && !logop(o) ){
...
int tdebug = 0;
#define shiftop(x) ( (x) == LS || (x) == RS || (x) == ARS )
NODE *
tymatch(p)  register NODE *p; {
	...
	/* rules are:
		...
		if any float or doubles, make double if singflag == 0
        	     ...
        	     if any doubles, make double
                     if a float and an integer constant, 
		       make double to preserve precision
	...
	t1 = NORM(p->in.left->in.type); /* remove volatile attribute      !08*/
	t2 = NORM(p->in.right->in.type);
	...
	if( ( t1 == CHAR || t1 == SHORT ) && o!= RETURN ) t1 = INT;	/*!04*/
	if( t2 == CHAR || t2 == SHORT ) t2 = INT;

	if((!singflag)&& (t1==DOUBLE || t1==FLOAT || t2==DOUBLE || t2==FLOAT)) 
	  t = DOUBLE;
	else if( t1==DOUBLE || t2==DOUBLE ) t=DOUBLE;
	else if( (t1==FLOAT && p->in.left->in.op==ICON)||
	         (t2==FLOAT && p->in.right->in.op==ICON) ) t=DOUBLE;
	...
	if( asgop(o) ){
		...
		tu = ISUNSIGNED(p->in.left->in.type) ? ENUNSIGN(t1) : t1;
		t = t1;
	} else 	tu = (u && UNSIGNABLE(t))?ENUNSIGN(t):t;
	...
	if( t != t1 ||  (ISUNSIGNED(tu) ^ ISUNSIGNED(p->in.left->in.type)) ) 
	   p->in.left = makety( p->in.left, tu, 0, (int)tu ); /*!02*/

	if( t != t2 || o==CAST ||
	    (ISUNSIGNED(tu) ^ ISUNSIGNED(p->in.right->in.type)) ) 
	   p->in.right = makety( p->in.right, tu, 0, (int)tu );   /*!02*/
...
makety( p, t, d, s ) register NODE *p; TWORD t; {
	...
	if( ISEQUAL(p->in.type,ENUMTY) && p->in.op == ICON ) econvert(p);
	if( ISEQUAL(t, p->in.type)){
	        p->in.type = t; /* in case t is volatile */
		p->fn.cdim = d;
		p->fn.csiz = s;
		return( p );
		}
...
# endif
	...
	if( p->in.op == ICON ){
		if( ISEQUAL(t,DOUBLE)|| ISEQUAL(t,FLOAT) ){
			p->in.op = FCON;
