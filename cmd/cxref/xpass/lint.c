 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: lint.c,v 1.5.2.3 90/05/09 15:37:17 wje Exp $"
/* lint.c
...
int	ALSTRUCT=	16;
#endif
int vflag = 1;  /* tell about unused arguments */	/* DAG -- spelling correction */
#if CXREF
...
/* defnam - define the current location as the name p->sname */	/* DAG -- was "defname" */
...
#if CXREF	/* DAG */
#if FLEXNAMES
	if( stab[r].sclass == REGISTER ) uerror( "cannot take address of %s", stab[r].sname );
#else
	if( stab[r].sclass == REGISTER ) uerror( "cannot take address of %.8s", stab[r].sname );
#endif
#else
 ...
 */
		...
		if( SZLONG > SZINT && aflag
		   ...
		   && t!=FLOAT && t!=DOUBLE ) 
#if CXREF	/* DAG */
			werror( "conversion from long may lose accuracy" );
#else
...
#if CXREF	/* DAG */
			werror( "conversion to long may sign-extend incorrectly" );
#else
...
strip(s) char *s;
{
	...
	for( p=x; *s; ++s ){
		...
		if ( p > &x[BUFSIZ] ) {
#else
		if ( p >= &x[LFNM] ) {	/* Robert Henry -- bug fix (was >) */
#endif
...
/* VARARGS1 */
ucw_write(op) enum Uopcode op; {;}
enum Datatype
ucu_ucodety( ty ) TWORD ty; { return Adt;}/* returns UCODE type of object */
eprint( p, down, a, b ) NODE *p; int *a, *b; {;}
branch(n){;}
defalign(n){;}
deflab(n){;}
bycode(t,i){;}
int st_idn_index_fext(x,y) { return 0;}
cisreg(t) TWORD t; {return(1);}  /* everything is a register variable! */	/* DAG -- spelling correction */
...
#if CXREF	/* DAG */
	outfp = stdout;
	while((i=getopt(argc,argv,"bdef:i:o:txI")) != EOF)
#else
...
#endif
/* DAG -- Gould just uses default alignment */
	}
#endif
#if CXREF	/* DAG */
	n = mainp1(argc, argv);
...
register int op;
register NODE *p; {
/* create a temporary node for storing the switch expression */

  register TWORD type = BTYPE(p->in.type);
    register NODE *q;
    extern NODE  *block();

    type = (type == ENUMTY) ? ENUMTY : (ISUNSIGNED(type) ? UNSIGNED : INT);
    /* int sized things are more efficient */
    if( type == ENUMTY ) /* special treatment of enums for check in chkpun*/
      q = block( op, NIL, NIL, p->in.type , 0 , p->fn.csiz );
    else
      q = block( op, NIL, NIL, type , 0 , type );
     q->tn.lval = 0;
    return ( q );
} /* mk_temp */
