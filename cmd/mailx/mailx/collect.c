 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: collect.c,v 1.6.1.2 90/05/09 16:38:48 wje Exp $"
...
# endif /* VMUNIX */
		...
		if (linebuf[0] != escape || rflag != NOSTR) {
			if (write(obuf->_file,linebuf,strlen(linebuf)) != strlen(linebuf))
				goto err;
			continue;
		}
		c = linebuf[1];
