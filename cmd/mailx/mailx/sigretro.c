 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sigretro.c,v 1.4.1.2 90/05/09 16:42:14 wje Exp $"
...
sigrelse(sig)
{
	 ...
	 */
	if (sigtable[sig].s_func == (sigtype) SIG_DFL)
		signal(sig, SIG_DFL);
	return(old);
}
...
top:
	 ...
	 */
	if (sigtable[sig].s_func == (sigtype) SIG_IGN)
		return;
	/*
