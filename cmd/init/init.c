 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: init.c,v 1.17.2.5 90/05/09 16:15:25 wje Exp $"
...
#define DEFAULT_NPROC 300
#endif DEFAULT_NPROC
#ifndef MIN_NPROC
#define MIN_NPROC 100
#endif MIN_NPROC
#ifndef MAX_NPROC
#define MAX_NPROC 10000
#endif MAX_NPROC
#define NPROC_NAME "_riscos_nproc"
#include <bsd43/sys/syscall.h>
#include <bsd43/sys/sysmips.h>
#include <sys/mipskopt.h>
#define mipskopt(x,y,z) syscall(BSD43_SYS_sysmips,BSD43_MIPS_KOPT,(x),(y),(z))
extern char *calloc();
...
#endif
char	*SYSTTY	=	"/dev/systty";	/* Real System Console */
...
#else
struct	termio	dflt_termio = {
	BRKINT|IGNPAR|ISTRIP|IXON|IXANY|ICRNL,		/* c_iflag */
	OPOST|ONLCR|TAB3,				/* c_oflag */
#if mips
	CS8|CREAD|B9600,				/* c_cflag */
	ISIG|ICANON|ECHO|ECHOK|ECHOE,			/* c_lflag */
	LDISC0,						/* c_line */
	{ CINTR,					/* c_cc[VINTR] */
	  CQUIT,					/* c_cc[VQUIT] */
	  CTRL(H),					/* c_cc[VERASE] */
	  ...
	  CDEL,						/* c_cc[V_RPRNT] */
	  CFLUSH,					/* c_cc[V_FLUSH] */
	  CWERASE,					/* c_cc[V_WERAS] */
	  CESC,						/* c_cc[V_LNEXT] */
	  CDEL,						/* c_cc[V_STATUS] */
	  CDEL,						/* c_cc[V_SAVED_EOF] */
	  CDEL,						/* c_cc[V_SAVED_EOL] */
	  CDEL,						/* c_cc[19] */
	  CDEL,						/* c_cc[20] */
	  CDEL,						/* c_cc[21] */
	  CDEL }					/* c_cc[22] */
...
char **argv;
{
	...
	extern int errno;
	char utmplock[84];
	int override_level,defaultlevel,initialize();
	FILE *fp;
	FILE *fdup();
	int chg_lvl_flag;
	extern char level();
	...
	extern int rsflag;
	extern int time_up;
	register struct PROC_TABLE *reset_proc;
...
#define TZ_LEN 20
	...
	if (getenv("TZ") == NULL) {
		if ((tz_fp=fopen("/etc/TZ","r")) == NULL) {
			/* Syslog() won't work when syslogd isn't up!
			 * I tried to log the stuff by opening up /dev/console
...
#undef TZ_LEN
	...
	/*
	 * If our pid is not SPECIALPID, then we are a user invoked init
	 * whose job it is to inform init to change levels.
	 */
	if ( (own_pid = getpid()) != SPECIALPID ) {
		userinit(argc,argv);
		exit(1);		/* userinit() shouldn't return ... */
	}

#ifdef RISCOS
	errno = 0;
	nproc = mipskopt(NPROC_NAME,0,KOPT_GET);
	if (errno != 0)
		nproc = DEFAULT_NPROC;
	if (nproc < MIN_NPROC)
		nproc = MIN_NPROC;
	proc_table = (struct PROC_TABLE *) 
			calloc(nproc,sizeof(struct PROC_TABLE));
	if (proc_table == NULL) {
		own_pid++;
		real_console("cannot allocate %d proc_table entries\n",
				nproc,0,0,0);
		exit(1);
	};
#endif RISCOS
	/*
	 * Init takes on optional single argument specifying run level.
	 * The only legal form for such an arg is [1-6sS].
	 # Anything confusing triggers SINGLE_USER.
	 */
	switch (argc) {
	case 1:
		override_level = 0;	/* No over-ride */
		break;
	case 2: 
		if (strlen(argv[1]) == 1) {
			switch (argv[1][0]) {
			case '1':	override_level = LVL1; break;
			case '2':	override_level = LVL2; break;
			case '3':	override_level = LVL3; break;
			case '4':	override_level = LVL4; break;
			case '5':	override_level = LVL5; break;
			case '6':	override_level = LVL6; break;
			case 's':	override_level = SINGLE_USER; break;
			case 'S':	override_level = SINGLE_USER; break;
			/* confused */
			default:	override_level = SINGLE_USER; break;
			}
		}
		else {
			/* confused */
			override_level = SINGLE_USER;
		}
		break;
	default:
		/* confused */
	...
	/*
	 * Reset syscon to systty at reboot in case it got hosed.
	 * (Fork child so init won't grab the tty as it's controlling tty.)
	 */
	openlog("INIT", LOG_CONS|LOG_NDELAY|LOG_NOWAIT, LOG_AUTH);
	signal(SIGCLD,SIG_DFL);
	reset_proc = efork(M_WAIT, NULLPROC, NOCLEANUP);
	signal(SIGCLD,childeath);
	if (reset_proc == NULLPROC) {
		reset_syscon();
		exit(1);
	}
	while (waitproc(reset_proc) == FAILURE);

	/*
	 * Scan /etc/inittab for a default level.
	 * (Also, execute any sysinit entries from inittab.)
	 */
	...
	/*
	 * The initial level is determined by the first of the following
	 * that applies.
	 *
	 *   (1) Can't open inittab.  The Only legal state is Single User.
	 *   (2) Argument passed to init.
	 *   (3) If no argument, then initdefault from /etc/inittab is used.
	 *   (4) And if neither of the above, then ask the operator.
	 */
	if (defaultlevel == -1) {
		new_state = SINGLE_USER;	/* Can't open /etc/inittab */
		defaultlevel = 0;
	}
	else if (override_level) {
		new_state = override_level;	/* Level from argument */
		defaultlevel = 0;
	}
	else if (defaultlevel > 0) {
		new_state = defaultlevel;	/* Level from inittab */
	}
	else {
		new_state = getlvl();		/* Ask the operator. */
	}
...
/* If the first character of the id was a #, it's a comment. */
						if (shcmd[0] == '#') {
#ifdef DUMPTAB
 ...
 * informed of the fact.  (Must be a process group leader in order for
 * SYSTTY to become our controlling tty so that we get signals.)
 */
		setpgrp();
...
/* program in the "ut_user" field. */
	...
	case RUN_LVL :
		...
		if ( oldu != NULL && program != NULL && *program == 'S')
 ...
 * that has the same device number as SYSTTY
 */
...
realcon()
{
	...
	if ( (stat(SYSCON, &sconbuf) != -1)
	   && (stat(SYSTTY, &conbuf) != -1)
	   && ((sconbuf.st_mode & S_IFMT) == S_IFCHR)
	   && ((conbuf.st_mode & S_IFMT) == S_IFCHR)
...
real_console(format,arg1,arg2,arg3,arg4)
#else	!RISCOS
console(format,arg1,arg2,arg3,arg4)
#endif	!RISCOS
char *format;
...
check_key()
{
	if (sysmips(MIPS_KEYLOCKED) == 1) {
		fprintf(stderr, "Machine key locked - can not change state\n");
		exit(1);
	}
}
...
#if	RISCOS
	syslog(LOG_DEBUG,format,arg1,arg2,arg3,arg4,arg5,arg6);
#else	!RISCOS
...
dump_table()
{
	...
	while(getcmd(&cmd,&cmd_string[0]) == TRUE) {
		printf("\nInittab entry id %.4s\n", cmd.c_id);
		printf("\tLevel  mask: 0x%x\n", cmd.c_levels);
		printf("\tAction mask: 0x%x\n", cmd.c_action);
		printf("\tCommand    : %s\n", cmd.c_command ? cmd.c_command : "(none)");
	}
}
#endif
