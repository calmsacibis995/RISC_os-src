 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: whodo.c,v 1.9.1.2 90/05/10 00:55:48 wje Exp $"
...
#endif /* u3b2 || mips */
#if mips
#define MAINSTORE	0		/* Address of physical location 0
					 * for /dev/mem driver.
					 */
#endif
...
char *argv[];
{
	...
	case 16384:	pte_to_pfn_shift = 10;
			...
			break;
	default:	fprintf(stderr,"whodo: cannot handle %d byte page size\n",
				page_size);
			exit(1);
	} /* switch */
#else
	page_size = NBPP;
		...
		char username[sizeof(ut->ut_user) + 1];
                ...
                printf("\n%-8.8s %-8.8s %2.1d:%2.2d\n",
                        ut->ut_line, ut->ut_user, tm->tm_hour, tm->tm_min);
		strncpy(username, ut->ut_user, sizeof(ut->ut_user));
                tpw = getpwnam(username);
		if (tpw == NULL) {
			fprintf(stderr, "%s: No password entry for user %s\n",
				ut->ut_user);
			continue;
		}
                uid = tpw->pw_uid;
...
register struct proc *p;
{
	...
	for(cp = (char *) &uarea; cp < cp1; i++, cp += page_size) {
#ifdef u3b2
		lseek(fd, (p->p_ubptbl[i].pgm.pg_pfn << 11) - MAINSTORE, 0);
#else
		/* mips */
		lseek(fd, pte_to_byte(p->p_ubptbl[i].pgi.pg_pde) - MAINSTORE, 0);
#endif u3b2
		if (read(fd, cp, cp1-cp > page_size ? page_size : cp1-cp) < 0) {
