 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: callers.c,v 1.5.2.3.1.2 90/08/03 11:14:07 hawkes Exp $"
...
register char *flds[], *dev[];
{
	...
	if (dcf == -1) {
...
#ifdef RISCOS
		{
			...
			if (ioctl(dcf, TCGETA, &termio) >= 0) {
				termio.c_lflag |= CLOCAL;
				ASSERT(ioctl(dcf, TCSETAW, &termio) >= 0,
				    "RETURN FROM generic open TCSETAW",
				    "", errno);
			};
		};
#endif RISCOS
...
#ifdef RISCOS
	{
		...
		if (ioctl(dcf, TCGETA, &termio) >= 0) {
			termio.c_lflag &= ~CLOCAL;
			ASSERT(ioctl(dcf, TCSETAW, &termio) >= 0,
			    "RETURN FROM generic open TCSETAW", "", errno);
		};
	};
#endif RISCOS
	/*
