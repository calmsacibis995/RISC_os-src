 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: cu.c,v 1.5.2.5 90/05/28 17:55:22 wje Exp $"
...
#ifdef RISCOS
					saved_cflag = _Lv.c_cflag;
#endif RISCOS
					_Lv.c_cflag = 0;
        				(void) ioctl (Cn, TCSETAW, &_Lv);
        				(void) sleep (2);
#ifdef RISCOS
					/* restore flags, so uugetty works */
					_Lv.c_cflag = saved_cflag;
					(void) ioctl (Cn, TCSETAF, &_Lv);
#endif RISCOS
				}
			return(YES);
		case '!':
