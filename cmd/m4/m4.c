 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: m4.c,v 1.6.2.2 90/05/09 16:34:47 wje Exp $"
...
register char 	***xargv;
{
	...
	while (*xargc > 1) {
		...
		case 'B':
			if (arg[2] != '\0') {
				bufsize = strtol(&arg[2], sptr, 10);
				if (&arg[2] != *sptr)
					break;
			}
			fprintf(stderr,
				"%s: option %c must be followed by an int\n",
				procnam,arg[1]);
			delexit(NOT_OK);
		case 'D':
			{
