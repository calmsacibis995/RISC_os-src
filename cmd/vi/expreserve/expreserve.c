 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: expreserve.c,v 1.7.2.2 90/05/10 00:52:51 wje Exp $"
...
#ifdef preSV_3
	while (fread((char *) &direntry, sizeof direntry, 1, tf) == 1) 
#else
...
format:
	...
	if (stat(savdir, &stbuf) < 0) {
		sprintf(mkbuf, "mkdir %s 2> /dev/null \n", savdir);	
		if (system(mkbuf) != 0) {
