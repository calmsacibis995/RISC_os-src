 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: exrecover.c,v 1.6.2.2 90/05/10 00:53:04 wje Exp $"
...
#undef	NULL
#endif
#undef	EOF		/* EOF and NULL effectively the same */
	...
	char *argv[];
{
	...
	if (nb[0] == '/') {
		ignore(unlink(nb));
		sprintf(rmcmd, "rmdir %s 2> /dev/null", mydir);
		system(rmcmd);
	}
	exit(0);
}
	...
	char *dirname;
{
	...
	ecount = 0;
#ifdef preSV_3
	while (fread((char *) &direntry, sizeof direntry, 1, dir) == 1)
#else
	...
	char *dirname;
{
...
#endif
	{
		 ...
		 * we are looking for.
		 */
		ignore(strcat(strcat(strcpy(nb, dirname), "/"), filname));
		if (yeah(nb)) {
			/*
