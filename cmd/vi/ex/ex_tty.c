 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ex_tty.c,v 1.10.1.2 90/05/10 00:50:21 wje Exp $"
	...
	char *type;
{
...
#else TIOCGETD
		struct	termio	termiobuf;

		ioctl(2, TCGETA, &termiobuf);
		ldisc = termiobuf.c_line;
		if (! value(vi_NOVICE) &&
		    termiobuf.c_line != LDISC0 &&
		    termiobuf.c_cc[V_SUSP] != 0) {
			sc[0] = termiobuf.c_cc[V_SUSP];
			sc[1] = 0;
			if (termiobuf.c_cc[V_SUSP] == CTRL(z)) {
				for (i = 0; i <= 4; i++)
					if (arrows[i].cap != NULL &&
					    arrows[i].cap[0] == CTRL(z))
						addmac(sc, NULL, NULL, arrows);
	...
	if (tparm(cursor_address, 2, 2) == 0 || 
	    tparm(cursor_address, 2, 2)[0] == 'O')	/* OOPS */
		cursor_address = 0;
	else
...
setsize()
{
	...
	if (ioctl(0, TIOCGWINSZ, &win) == 0) {
		if ((winsz.ws_row = win.ws_row) != 0) 
	  		lines = winsz.ws_row;
		if ((winsz.ws_col = win.ws_col) != 0)
