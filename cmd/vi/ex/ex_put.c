 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ex_put.c,v 1.7.1.2.1.2 90/08/09 15:59:04 hawkes Exp $"
	...
	register short c;
{
	...
	if (c & QUOTE)
		...
		putchar('^'), c = ctlof(c);
	else if (c >= 0200 && !isprint(c) && ! value(vi_8BIT)) {
		outchar('\\');
		outchar(((c >> 6) & 07) + '0');
		outchar(((c >> 3) & 07) + '0');
		outchar((c & 07) + '0');
...
ttcharoff()
{
	tty.c_cc[VQUIT] = '\377';
	tty.c_cc[V_SUSP] = '\377';
	tty.c_cc[V_DSUSP] = '\377';
	tty.c_cc[V_FLUSH] = '\377';
	tty.c_cc[V_LNEXT] = '\377';
#ifdef VSTART
	/*
