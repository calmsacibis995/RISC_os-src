 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ex_io.c,v 1.10.1.3.1.2 90/08/09 15:57:53 hawkes Exp $"
	...
	int comm;
{
	...
	if (endcmd(d)) {
		...
		case 'e':
			if (savedfile[0]) {
				fs_push (savedfile, lineDOT());
			}
			CP(savedfile, file);
			break;

		default:
			if (file[0] && !Delayed_pop) {
				fs_push (file, (c != 'E') ? lineDOT() : 0);
			}
			break;
		}
	}
...
flong:
		...
		case '^':
			...
			Delayed_pop = 1;
			if (fp == 0 || *fp == 0)
				error("No files in stack@to pop");
			Save_saved = savedfile[0];
			savedfile[0] = 0;
			goto filexp;
	...
	struct glob *gp;
{
	...
	if (pid == 0) {
		...
		open("/dev/null", 1);
		execl(svalue(vi_SHELL), "sh", "-c", genbuf, 0);
	...
	int c;
{
	...
	case S_IFREG:
		...
		if (i != sizeof(magic))
			break;
#ifdef RISCOS
		if (ISCOFF(magic))  /* this tests everyone's magic numbers*/
		    error(" Executable");
		else {
#endif
		switch (magic) {
		...
		default:
			...
			if ((magic & 0100200) && ! value(vi_8BIT) )
#endif
