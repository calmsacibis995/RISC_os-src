 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ex_cmds.c,v 1.8.1.3.1.2 90/08/06 14:50:36 hawkes Exp $"
...
/* print file stack */
	    ...
	    /* the ignchar() and eol() are to fix a bug when you type
	     * something after the pf.  The peekchar was 'f' and this
	     * was not gobbled up correctly, so was being passed on
	     * in the next loop interation and was being interpreted
	     * as a "file xxx" command.  To preserve the behavior
	     * as before (file prints out the current stuff), call
	     * showfile('f') which is from the bottom of the file cmd code
	     */
				ignchar();
				eol();
				flush();
			...
			case 'n':
				{
					...
					ignchar();  /* eat the peekchar */
					quick = exclam();
					fs_push (file, lineDOT());
					x = mhgetn();
					if (x > 0) {
					    fs_pushn (x);
					} else if (x < 0) {
					    ...
					    if (fs_pushs (file) != 0)
					    {
				    /* didn't find the named file on the stack
				     * clean up by poping the current stuff
				     * we just pushed, and restoring the
				     * file name
				     */
						fs_pop();
						CP(file,savedfile);
						error(
				"Not found|File not found on file stack");
					    }
					} /* if */
					eol ();  /* nothing else allowed after*/
					fs_pop ();
					if ( strcmp(savedfile,altfile) == 0)
					{
				/* we're in the same file, just set dot */
					    dot = altdot + zero;
					    continue;
					}
					else if ( edited )
					{
					    if (!quick) /* no exclam */
					    {
						ckaw();  /* chk for autowrite*/
						if ( chng &&  (dol > zero))
						{
				/* push the stuff back on */
						    fs_push(altfile,altdot);
						    error(
				"No write@since last change (:pn! overrides)"
							 );
						}
