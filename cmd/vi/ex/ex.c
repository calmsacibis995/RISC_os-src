 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ex.c,v 1.10.1.3 90/05/10 00:48:19 wje Exp $"
 ...
 * 					error("Version SVR3.2");
 */

char *Version = "Version SVR3.2";	/* variable used by ":ver" command */
	...
	register char *av[];
{
...
#endif
	int nosrc = 0;	/* default is to read /usr/lib/ex.rc file */
	...
	switch (editor) {
		...
		case VEDIT:
#ifdef TRACE
		while ((c = tmpgetopt(ac,av,"VS:Lc:T;t:r;lw:xRCn","+:")) != EOF)
#else
		while ((c = tmpgetopt(ac,av,"VLc:t:r;lw:xRCn","+:")) != EOF)
#endif
		{
			switch(argtyp) {
			  case OPTIONS:
...
#define	tracebuf	NULL
					...
					default:
#ifdef TRACE
			sprintf(usage,"Usage: %s [-n] [-l] [-L] [-r [file]] [-t tag] [-T [-S suffix]] \n", cmdnam);
#else
			sprintf(usage,"Usage: %s [-n] [-l] [-L] [-r [file]] [-t tag]\n", cmdnam);
#endif
	...
	case EDIT:
#ifdef TRACE
		while ((c = tmpgetopt(ac,av,"S:LTxC;r;n","")) != EOF)
#else
		while ((c = tmpgetopt(ac,av,"LxCr;n","")) != EOF)
#endif
		{
		  switch (c) {
			case 'n':
			...
			default:
#ifdef TRACE
				sprintf(usage,"Usage: %s [-n] [-L] [-r [file]] [-T [-S suffix]] file....\n", cmdnam);
#else
				sprintf(usage,"Usage: %s [-n] [-L] [-r [file]] file....\n", cmdnam);
#endif
				write(2, usage, strlen(usage));
				exit(1);
				}
		...
		case EX:
#ifdef TRACE
		while ((c = tmpgetopt(ac,av,"-VLsS:c:T;vt:r;RlxCn","+:")) != EOF)
#else
		while ((c = tmpgetopt(ac,av,"-VLsc:vt:r;RlxCn","+:")) != EOF)
#endif
		{
		switch (argtyp) {
			case OPTIONS:
...
#define	tracebuf	NULL
				...
				default:
#ifdef TRACE
			sprintf(usage,"Usage: %s [- | -s] [-n] [-l] [-L] [-R] [-r [file]] [-t tag] [-T [-S suffix]]\n", cmdnam);
#else
			sprintf(usage,"Usage: %s [- | -s] [-n] [-l] [-L] [-R] [-r [file]] [-t tag]\n", cmdnam);
#endif
			write(2,usage,strlen(usage));
	...
	if (setexit() == 0 && !fast) {
		...
		/* Read global .exrc ("/usr/lib/ex.rc") IF the user
		 * has not called program with -n (don't source the
		 * global .exrc file) - the default is to call the
		 * /usr/lib/ex.rc file.
		 */

		if (!nosrc)
			source("/usr/lib/ex.rc",1);

		/* 
		 * Either (EXINIT or $HOME/.exrc) and ./.exrc.
		 */
		...
		if ((globp = getenv("EXINIT")) && *globp)
			...
			if ((cp = getenv("HOME")) != 0 && *cp) {
				strcat(strcpy(genbuf, cp), "/.exrc");
				if ( stat(genbuf, &stat_buf) >= 0) {
					dev = stat_buf.st_dev;
					ino = stat_buf.st_ino;
					if (iownit(genbuf)) {
						source(genbuf, 1);
		...
		/*
		 *      MIPS UMIPS 3.0 change
		 * Allow local .exrc too, but only if it's not the same
		 * file as $HOME/.exrc.
		 *      AT&T SVR3.2 change
		 * Allow local .exrc if the "exrc" option was set.
		 *      MIPS riscos 4.50 change
		 * The riscos 4.50 repair to the SVR3.2 change is to create
		 * the global /usr/lib/ex.rc file and ship it with "set exrc"
		 * so that our original value-added ability is preserved.
		 * Can-o-worms for the sysadmin, but we have to let vi/ex
		 * work as a vanilla SVR3.2 program, too.
		 */
		if ( stat(".exrc", &stat_buf) >= 0
		&& ( dev != stat_buf.st_dev || ino != stat_buf.st_ino)) {
			if (iownit(".exrc")) {
				if (value(vi_EXRC) || value(vi_SOURCEANY))
					source(".exrc", 1);
			}
		}
	}
...
char *file;
{
	...
	if (stat(file, &sb) == 0 && ((value(vi_EXRC) || value(vi_SOURCEANY)) || sb.st_uid == getuid()))
		return(1);
	else
		return(0);
}
