 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: lpstat.c,v 1.7.2.2 90/05/09 16:32:53 wje Exp $"
...
char errmsg[100];
char *usage = "usage: lpstat [-alist] [-clist] [-d] [-olist] [-plist] [-r]\n                        [-s] [-t] [-ulist] [-v]";
...
/*	printf("%-*s %-*s %*ld   %s", IDSIZE, reqid, LOGMAX-1, o->o_logname,
	...
	printf("%s %s %*ld   %s", reqid, o->o_logname,
		OSIZE, o->o_size, dt);
...
char *argv[];
{
	...
	if(argc == 0) {
		if((d = opendir(MEMBER)) == NULL)
			fatal("MEMBER directory has disappeared!", 1);

		while (dp = readdir(d))
		   if(dp->d_name[0] != '.') {
			strncpy(dest, dp->d_name, DESTMAX);
			dest[DESTMAX] = '\0';
			putdline(dest);
		    }
		fclose(d);
...
char *argv[];
{
	...
	if(argc == 0) {
		if((d = opendir(CLASS)) == NULL)
			fatal("CLASS directory has disappeared!", 1);
		while (dp = readdir(d))
		   if(dp->d_name[0] != '.') {
			strncpy(cl, dp->d_name, DESTMAX);
			cl[DESTMAX] = '\0';
			putcline(cl);
		    }
		fclose(d);
