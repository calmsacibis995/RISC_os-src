 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: lpsched.c,v 1.5.2.4.1.1.1.2 90/11/29 16:11:18 beacker Exp $"
...
cinit()
{
	...
	while(dp = readdir(cd)) {
		if(dp->d_name[0] != '.') {
			...
			d->d_status = D_CLASS;
			insert(&class, d);
			sprintf(cfile, "%s/%s", CLASS, dp->d_name);
...
struct dest *p;
{
	...
	if(pid == 0) {
		...
		sprintf(errmsg,"Can't execute printer interface program (errno=%d)\n",errno);
	...
	while(wait(&status) != pid)	/* 1st child waits for 2nd child */
		;
#ifdef RISCOS
					/* wait for last buffer to drain */
					/* loops, just in case its timeout */
        buf.ic_cmd =  TCSBRK;
        buf.ic_timout = -1;
        buf.ic_len = sizeof(int);
        buf.ic_dp = (char *)&param;
        while (ioctl(fileno(stdout), I_STR, &buf) == -1 && errno == ETIME)
		buf.ic_len = sizeof(int);
#endif
...
pinit()
{
	...
	while(dp = readdir(md)) {
		if( dp->d_name[0] != '.') {
			...
			sprintf(memfile, "%s/%s", MEMBER, dp->d_name);
...
#ifdef DEBUG
	{
		fprintf(stderr, "Entering schedule loop\n");
   		fflush(stderr);
	}
#endif
	while(TRUE) {
