 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: inode.c,v 1.5.1.5 90/05/09 17:22:36 wje Locked $"
 ...
 *
 *	@(#)inode.c	1.4 88/05/12 4.0NFSSRC SMI	from UCB 5.2 7/17/85
 *	@(#) from SUN 1.5
 */
	...
	register struct inodesc *idesc;
{
	...
	DINODE dino;
#ifdef RISCOS
	long dbcnt;	/* Total number of remaining datablocks. */
	long dbcntpb;	/* Number of datablocks per indirect block. */
	int i;
#endif /* RISCOS */
...
#ifdef RISCOS
	dbcnt = ((dino.di_size + sblock.fs_bsize - 1) / sblock.fs_bsize) - NDADDR;
#endif /* RISCOS */
	idesc->id_numfrags = sblock.fs_frag;
	for (ap = &dino.di_ib[0], n = 1; n <= NIADDR; ap++, n++) {
		if (*ap) {
			idesc->id_blkno = *ap;
#ifdef RISCOS
			for (dbcntpb = NINDIR(&sblock), i = 1; i < n; i++)
				dbcntpb *= NINDIR(&sblock);
			if (dbcnt > dbcntpb) {
				ret = iblock(idesc, n, dbcntpb);
				dbcnt -= dbcntpb;
			} else {
				ret = iblock(idesc, n, dbcnt);
			}
#else
			ret = iblock(idesc, n,
...
#endif /* RISCOS */
{
	...
	int i, n, (*func)(), nif;
#ifdef RISCOS
	int dbcntpb;		/* Number of datablocks per indirect block. */
#else
	int sizepb;
#endif /* RISCOS */
	BUFAREA ib;
	...
	ilevel--;
#ifdef RISCOS
	for (dbcntpb = 1, i = 0; i < ilevel; i++)
		dbcntpb *= NINDIR(&sblock);
	nif = (dbcnt + dbcntpb - 1) / dbcntpb;
#else
...
#ifdef RISCOS
				if (dbcnt > dbcntpb) {
					n = iblock(idesc, ilevel, dbcntpb);
					dbcnt -= dbcntpb;
				} else {
					n = iblock(idesc, ilevel, dbcnt);
				}
#else
