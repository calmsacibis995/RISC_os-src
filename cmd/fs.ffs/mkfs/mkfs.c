 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: mkfs.c,v 1.5.1.4 90/05/09 15:57:23 wje Exp $"
 ...
 *
 * usage:
 *mkfs.ffs -N special size [ nsect ntrak bsize fsize cpg minfree rps nbpi opt ]
 */
 ...
 */
#ifdef RISCOS
#define ROTDELAY	7	/* taken from previous version */
#else
#define ROTDELAY	4
#endif
	...
	char *argv[];
{
	...
	time(&utime);
	if (argc < 2) {
		printf("usage: mkfs.ffs -N special size [ nsect ntrak bsize fsize cpg minfree rps nbpi ]\n");
		exit(1);
	}
	fsys = argv[0];
	fssize = atoi(argv[1]);
...
#define min(a,b) ((a < b) ? a : b)
	{
	  int round_size, new_fssize;

	  /* Since DESBLKSIZE is 8K, we take the min of this and
	   * what the user wants.  This has been done this way
	   * because even if the block size is 16K, we read ahead 8K.
	   */
	  round_size = min(DESBLKSIZE, sblock.fs_bsize) / DEV_BSIZE;
	  new_fssize = (fssize/round_size)*round_size;
	  if (new_fssize != fssize)
	    fprintf(stderr,
		    "mkfs.ffs: warning: resetting filesystem size to %d\n",
		    new_fssize);
	  fssize = new_fssize;
	}
#endif
...
next:
	...
	if ((sblock.fs_spc * sblock.fs_cpg) % NSPF(&sblock)) {
		printf("mkfs.ffs: nsect %d, ntrak %d, cpg %d is not tolerable\n",
