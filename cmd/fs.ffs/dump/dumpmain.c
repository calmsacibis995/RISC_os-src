 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: dumpmain.c,v 1.5.1.2 90/05/09 15:52:35 wje Exp $"
 ...
 */
/* @(#)dumpmain.c	5.6 (Berkeley) 2/23/87 */
	...
	char	*argv[];
{
	...
	case 'b':			/* blocks per tape write */
		if(argc > 1) {
			...
			if (ntrec <= 0) {
				fprintf(stderr,
					"Blocks/tape must be a positive integer\n");
				exit(2);
			}
#endif
		}
...
#if defined(RISCOS)
	{	/* call sprintf for correct functionality RISCOS */
		char buf[128];

		sprintf(buf, "estimated %ld tape blocks on %3.2f tape(s).\n",
			esize, fetapes);
		msg(buf);
		if (iflag)
		    exit(0);
...
/*
 * We don't currently need to do rawname(), because in UMIPS-V, the
 * cooked disk is faster.
 */
