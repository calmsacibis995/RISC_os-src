 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: vmsbackup.c,v 1.2.2.3 90/05/09 18:11:47 wje Exp $"
...
int	blocksize;
{
	...
	if (bhsize != sizeof(struct bbh)) {
		fprintf(stderr, "%s: Invalid header block size\n", progname);
		exit(1);
	}
	...
	while (i < bsize) {
		...
		default:
			fprintf(stderr, " %s: invalid record type\n", progname);
...
rdhead()
{
	...
	while ((i = read(fd, label, LABEL_SIZE)) != 0) {
		if (i != LABEL_SIZE) {
			fprintf(stderr, "%s: bad label record\n", progname);
			exit(1);
		}
