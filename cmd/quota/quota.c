 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: quota.c,v 1.2.1.2 90/05/09 18:22:02 wje Exp $"
 ...
 */
/* @(#)quota.c	1.3 88/08/03 4.0NFSSRC SMI: from UCB 5.4 2/24/86 */
...
#if defined(RISCOS)
	/* Under Tahoe semantics, there is no easy way to check if
	 * there are no quotas on the NFS fs.  The only way to remain
	 * consistent with Tahoe semantics is the read the FSTAB file
	 * and look for MNTOPT_RQ and the MOUNTED file for MNTOPT_RW
	 */
#endif
	...
	register struct dqblk *dqp;
{
	...
	} else if (dqp->dqb_bhardlimit && dqp->dqb_bwarn == 0) {
		printf("Out of block warnings on %s\n", mntp->mnt_dir);
	} else if (dqp->dqb_bsoftlimit &&
		 dqp->dqb_curblocks >= dqp->dqb_bsoftlimit) {
		printf("Too many blocks on %s\n", mntp->mnt_dir);
	}
	if (dqp->dqb_fhardlimit &&
	    dqp->dqb_curfiles >= dqp->dqb_fsoftlimit) {
		printf("File limit reached on %s\n", mntp->mnt_dir);
	} else if (dqp->dqb_fhardlimit && dqp->dqb_fwarn == 0) {
		printf("Out of file warnings on %s\n", mntp->mnt_dir);
	} else if (dqp->dqb_fsoftlimit &&
		 dqp->dqb_curfiles >= dqp->dqb_fsoftlimit) {
		printf("Too many files on %s\n", mntp->mnt_dir);
	}
#else /* !RISCOS */
	struct timeval tv;
	...
	register struct dqblk *dqp;
{
	...
	if (dqp->dqb_bsoftlimit && dqp->dqb_curblocks >= dqp->dqb_bsoftlimit) {
		sprintf(bwarn, "%d", (int)dqp->dqb_bwarn);
	} else {
		bwarn[0] = '\0';
	}
	if (dqp->dqb_fsoftlimit && dqp->dqb_curfiles >= dqp->dqb_fsoftlimit) {
		sprintf(fwarn, "%d", (int)dqp->dqb_fwarn);
	} else {
		fwarn[0] = '\0';
	}
...
#if defined(RISCOS) && defined(TAHOE_QUOTA)
	printf("%-12.12s %7d%7d%7d%12d%7d%7d%7d%12d\n",
#else
...
#if defined(RISCOS) && defined(TAHOE_QUOTA)
		dqp->dqb_bwarn = gq_rslt.gqr_rquota.rq_bwarn;
		dqp->dqb_fwarn = gq_rslt.gqr_rquota.rq_fwarn;
#else /* RISCOS */
		dqp->dqb_btimelimit =
