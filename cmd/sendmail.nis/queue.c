...
# ifndef QUEUE
#ifndef lint
static char sccsid[] = 	"@(#)queue.c	1.2 88/05/02 4.0NFSSRC SMI(no queueing)"; /* from UCB 5.21 4/17/86 */
						/* @(#) from SUN 1.24  */
#endif
# else QUEUE

#ifndef lint
static char sccsid[] = 	"@(#)queue.c	1.2 88/05/02 4.0NFSSRC SMI(with queueing)"; /* from UCB 5.21 4/17/86 */
						/* @(#) from SUN 1.24  */
#endif
...
# endif DEBUG
	...
	*/
	...
	if (e->e_df == NULL)
	{
		...
		(*e->e_putbody)(dfp, ProgMailer, e, "queueup");
		(void) fclose(dfp);
		e->e_putbody = putbody;
	}
		...
		while (i != 0 && fgets(lbuf, sizeof lbuf, cf) != NULL)
		{
			...
			switch (lbuf[0])
			{
			  ...
			  case 'R':
				...
				for (sp = &lbuf[1]; *sp; sp++)
				{
					if (strncasecmp(sp, OnlyRunRecip,
...
**
**	Returns:
**		Non-zero if we were unable to read the qf file
