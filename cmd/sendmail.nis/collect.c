#ifndef lint
static char sccsid[] = 	"@(#)collect.c	1.2 88/04/24 4.0NFSSRC SMI"; /* from UCB 5.2 6/8/85 */
#endif
...
SCCSID(@(#)collect.c 1.14 87/09/02 SMI); /* from UCB 5.2 6/8/85 */
...
# endif NOTUNIX
	...
	*/
	...
	do
	{
		...
		/*
		 * If we overflew the header buffer, then 
		 * skip the rest
		 */
		if ( (p - buf) >= MAXFIELD ) {
			char junk[128];

			while (	sfgets(junk, sizeof(junk), InChannel))
				if (junk[0] != ' ' || 
				    junk[0] != '\t') break;
		}
...
# endif DEBUG
	...
	*/
	...
	{
	   /*
	    * restore the timeout value if we saved it 
	    */
