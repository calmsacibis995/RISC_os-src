#ifndef lint
static char sccsid[] = 	"@(#)deliver.c	1.2 88/04/25 4.0NFSSRC SMI"; /* from UCB 5.10 3/2/86 */
#endif
...
# ifdef MXDOMAIN
		...
		mx = getmxrr(host, MAXMXHOSTS, MyHostName);
		if (mx->mx_number < 0)
		{
			/*
	...
	*/
	...
	if (e->e_dfp == NULL)
	{
		if (e->e_df != NULL)
		{
			...
			if (e->e_dfp == NULL)
				syserr("Cannot open df in %s, euid=%d", 
				  where, geteuid() );
		}
		else
...
# endif SIGCHLD
	...
	else if (pid == 0)
	{
		...
		putline("\n", f, ProgMailer);
		(*CurEnv->e_putbody)(f, ProgMailer, CurEnv, "mailfile");
		putline("\n", f, ProgMailer);
		(void) fclose(f);
		(void) fflush(stdout);
...
# endif DEBUG
# ifdef LOG
	    syslog(LOG_INFO, "%s: owner-%s=%s", CurEnv->e_id,
	       qq->q_user, aliaslookup(obuf) );
# endif LOG
