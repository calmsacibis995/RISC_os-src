#ifndef lint
static char sccsid[] = 	"@(#)savemail.c	1.2 88/05/02 4.0NFSSRC SMI"; /* from UCB 5.7 12/7/85 */
						/* @(#) from SUN 1.18  */
#endif
		...
		switch (state)
		{
		  ...
		  case ESM_USRTMP:
			...
			*/
			...
			putline("\n", fp, ProgMailer);
			(*e->e_putbody)(fp, ProgMailer, e, "savemail");
			putline("\n", fp, ProgMailer);
			(void) fflush(fp);
...
static char	*saveHost;	/* saved value of 's' macro for Received: */
...
# endif DEBUG
	...
	saveHost = macvalue('s', CurEnv);
	define('s', NULL, CurEnv);		/* local:no host name */
	...
	register ENVELOPE *e;
{
	...
	*/
	define('s', saveHost, e->e_parent);	/* restore host name */
	...
	else if (e->e_parent->e_dfp != NULL)
	{
		if (SendBody)
		{
			...
			putline("\n", fp, m);
			putbody(fp, m, e->e_parent,"errbody");
		}
		else
		{
			putline("\n", fp, m);
