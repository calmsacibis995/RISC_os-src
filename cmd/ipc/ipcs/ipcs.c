 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ipcs.c,v 1.5.2.2 90/05/09 16:17:27 wje Exp $"
...
/*
 * The MIPS version of ipcs uses the standard nlist() from libmld.a and
 * not the one in nnlist.c in this directory.
 * There is a bug in the MIPS nlist which sets n_type not n_value for
 * found namelist elements.  It is ifdef'ed in here with MIPS_NLIST_BUG.
 */
#define MIPS_NLIST_BUG
...
#else
			if(oflg || bflg || tflg || !qflg || !nl[MSG].n_type)
#endif
				printf("%s%s%s%s%s%s\n", chdr,
					cflg ? chdr2 : "",
					oflg ? " NATTCH" : "",
...
#ifdef MIPS_NLIST_BUG
			if(bflg || tflg || (!qflg || !nl[MSG].n_type) &&
				(!mflg || !nl[SHM].n_type))
#else
