 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: at.c,v 1.6.2.2 90/05/09 15:30:20 wje Exp $"
...
char **argv;
{
	...
	if (argc < 2) {
		fprintf(stderr,"Usage: at time [ date ] [ +increment ]\n");
		fprintf(stderr,"       at -r job... \n");
...
copy()
{
	...
	while ((c = getc(pfp)) != EOF) {
		...
		case 'd':	/* fork off a child with submitter's permissions,  */
				/* otherwise, when IFS=/, /bin/pwd would be parsed */
