 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: crontab.c,v 1.5.2.2 90/05/09 15:31:10 wje Exp $"
...
char **argv;
{
	...
	if (errflg || argc >2 || (argc == 2 && (rflag || lflag))) 
		crabort(BADUSAGE);
	pp = getuser(getuid());
	if(pp == NULL) {
		if (per_errno==2)
	...
	else if (access(argv[1],04) || (fp=fopen(argv[1],"r"))==NULL) crabort(BADOPEN);
	else copycron(fp);
	sendmsg(ADD,login);
	if(per_errno == 2)
