 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: cron.c,v 1.14.2.8 90/05/09 17:22:05 wje Locked $"
...
#define NOPERIODIC	"cron file named periodic MUST appear in cron spool directory.\nSee crontab(1) and cron(1M) for details."
...
struct	queue
{
	...
	qd = {100, 2, 60, 0},		/* default values for queue defs */
	qt[NQUEUE];
...
static char *spec_root = "periodic";	/* special file, owned by root
				    to run periodic shell scripts; it is
				    named "periodic" and lives in the
				    usr-spool-cron-crontab directory; it
				    is massaged by this program */
int file_opened = 0;	/* is ACCTFILE opened onto stdout/stderr yet? */
			/* this is set only ONCE, when file is opened */
...
char *envinit[]={
	homedir,
	logname,
	"PATH=/usr/net:/bin:/usr/bin:/etc:/usr/etc:/usr/ucb:/usr/new",
	"SHELL=/bin/sh",
	tzone,
	0};
extern char **environ;
...
char **argv;
{
	...
	if (argc >= 2 && strcmp(argv[1],"nofork")==0) dofork = 0;
	if (argc >= 3 && strcmp(argv[2],"nofork")==0) dofork = 0;
begin:
	...
	if (argc <= 3) {
		register int newrun;
		newrun = maxrun;
		if (argc >= 2 && strncmp(argv[1],"maxrun=",7)==0) {
			newrun = atoi(&argv[1][7]);
		}
		if (argc == 3 && strncmp(argv[2],"maxrun=",7)==0) {
			newrun = atoi(&argv[2][7]);
		}
		if (newrun > 0 && newrun < 10000)	/*reasonable */
			maxrun = newrun;
	}
	rt = (struct runinfo *) xmalloc(maxrun*sizeof(struct runinfo));
	bzero(rt,maxrun*sizeof(struct runinfo));
	...
	msg("*** cron started ***   pid = %d maxrun = %d",getpid(),maxrun);
	...
	while (TRUE) {			/* MAIN LOOP	*/
		...
		if (time((char *)0) < next_event->time) {
			/* Neat bug! cron thinks it woke up exactly */
			/* on time. If it wakes up early it forks */
			/* and tries to run it but decides it isn't quite */
			/* time yet so he sleeps until it is time. */
			/* Meanwhile, back at the ranch papa still has */
			/* the same job on his queue and he will respawn */
			/* it again. To fix this just check the time */
			/* to be sure cron should run the program */
			continue;
		}
		ex(next_event);
...
initialize(firstpass)
{
	...
	close(fileno(stderr));
	dup(1);
	file_opened = 0xf;	/* ACCTFILE now exists */
				/* it will be opened and closed as
				   needed, rather than remaining open*/
...
read_dirs()
{
	...
	dirp = NULL;
	if (chdir(CRONDIR) == -1) crabort(BADCD,1);
	cwd = CRON;
	dirp = opendir(".");
	if(dirp == NULL) 
		crabort(NOREADDIR,1);
	...
	closedir(dirp);
	dirp = NULL;
	if (chdir(ATDIR) == -1) crabort(BADCD,1);
	cwd = AT;
	dirp = opendir(".");
	if(dirp == NULL) 
		msg("cannot chdir to at directory");
	dscan(dirp,mod_atjob);
	closedir(dirp);
	dirp = NULL;
}
...
int format;
{
	...
	char *temp,*i,*strrchr();
	int special;
	char mailto[MAXNAMLEN];

#ifdef TESTING
	return;
#else

	if (strcmp(usrname,spec_root)==0)  {	/* periodic file? */
		strcpy(mailto,"root");	/* mail to root   */
		special = 0xf;			/* special mesg	  */
	}  else  {
		strcpy(mailto,usrname);		/* else mail to usr */
		special = 0;
	}
	temp = xmalloc(strlen(MAIL)+strlen(mailto)+2);
	pipe = popen(strcat(strcat(strcpy(temp,MAIL)," "),mailto),"w");
	if (pipe!=NULL) {
		if (format == 1) {
			if (special)
				fprintf(pipe,"Your periodic crontab file has an error in it.\n");
			else
...
/*  child process    */
	...
	if ((maxfiles=ulimit(4,NULL)) < 0) maxfiles = 20;
	...
	if (strcmp((e->u)->name,spec_root)==0)
		strcat(logname,"root");
	else
...
struct	runinfo	*pr;
{
	...
	int	fd;
	char	lin[5+UNAMESIZE+CTLINESIZE];
	struct	usr	*p;
	struct	stat	buf;
	...
	if(!stat(pr->outfile,&buf)) {
		if(buf.st_size > 0) {
			if((fd=open(pr->outfile,O_WRONLY)) == -1)
				...
				sprintf(lin,"%s %s < \"%s\"\n",
					MAIL,(strcmp(p->name,spec_root)==0)?"root":p->name, pr->outfile);
				if((pr->pid = fork()) == 0) {
					execl("/bin/sh","sh","-c",lin,0);
					exit(127);
				}
				pr->que = ZOMB;
				running++;
...
msg_wait()
{
	...
	if(pmsg->etype != NULL) {
#ifdef DEBUG
	fprintf(stderr,"in msg_wait - msg type %d\n", pmsg->etype);
#endif
		switch(pmsg->etype) {
		case AT:
...
#if	RISCOS
	openlog("cron", LOG_CONS, LOG_DAEMON);
#endif	RISCOS
	if(clean)
		if(unlink(FIFO) < 0)	/* FIFO vanishes when cron finishes */
#if	RISCOS
			syslog("could not unlink FIFO");
	/* write error msg to syslog (or console if syslogd not running) */
	syslog(LOG_NOTICE, "aborted: %s\n", mssg);
	closelog(); 
#else	!RISCOS
...
char *fmt;
{
	...
	t = time((long *) 0);
#ifndef DEBUG
	if (file_opened)	/* does ACCTFILE exist yet? */
	{			/* if so, it must be opened */
#ifdef RISCOS
		if (freopen(ACCTFILE,"a",stdout) == NULL)
#else
		if (freopen(ACCTFILE,"a",stdout) < 0)
#endif
		{
			fprintf(stderr,"msg: Problems with logfile %s\n",ACCTFILE);
		}  else {
			close(fileno(stderr));
			dup(1);	/* associate stderr with logfile again */
		}
	}
#endif
				/* if not, write directly to stderr */
	fprintf(stderr,"! ");	/* stderr is duped onto ACCTFILE */
	fprintf(stderr,fmt,a,b);
	fprintf(stderr," %s",ctime(&t));
	fflush(stdout);
#ifndef DEBUG
	if (file_opened) close(stdout);
#endif
}
