 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sort.c,v 1.6.2.2 90/05/09 19:02:33 wje Exp $"
		...
		if(**++argv == '-') for(arg = *argv;;) {
			...
			case 'o':
				...
				if(--argc > 0)
					...
					diag("can't identify output file","");
					exit(1);
				}
				break;
		...
		*lp-- = cp;
		...
		while ((char *)lp > cp + 1 ) {
			...
			if ( cp + 2 < (char *) lp ) {
				...
				if(f == NULL) diag("warning: missing NEWLINE added at EOF", "");
				else diag("warning: missing NEWLINE added at end of input file ", f);
				*++cp = '\n';
				*++cp = '\0';
				*lp-- = ++cp;
				keep = 0;
