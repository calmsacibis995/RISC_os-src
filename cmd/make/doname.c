 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: doname.c,v 1.3.2.2 90/05/09 16:43:52 wje Exp $"
/*
 * ke	3/13/85	Added DPATH extensions to do path searching for implicit
 *		rules.
 * ke	3/14/85 Added a (CHARSTAR) cast to suppress a warning.
 * mh	2/4/86 Add mechanism to recognize not ".a" archives
 */
 ...
 */
	...
	for(suffp = lp->depp ; suffp!=0 ; suffp = suffp->nextdep)
	{
		...
		if(suffix(p->namep , pnamep , prefix))
		{
			...
			found = 1;
			if(savenamep) {
			    archlen = strlen(archname);
			    if (archlen >= 2 && archname[archlen - 2] == '.')
				pnamep = &(archname[archlen - 2]);
			    else
				pnamep = ".a";
			}
searchdir:
			...
			for(suffp1=lp1->depp ; suffp1!=0 ; suffp1 = suffp1->nextdep)
			{
				...
				/*
				 * If no slashes in name, and DPATH set and
				 * non-null, search for the dependency
				 */
				if (!any(sourcename, SLASH) && 
			         ((ddir = findfl(sourcename, "DPATH")) != (CHARSTAR)-1))
					copstr(sourcename, ddir);
...
/*
 *  search all directories in DPATH for dependenices (unless name
 *	contains a slash -- prevents endless recursion)
 *  see findfl()
 */
depdir(name)
register CHARSTAR name;
{
	...
	if(any(name, SLASH))		/* don't look at path if slash */
	{
		srchdir(name, NO, NULL);	/* just use name */
		return;
	}
	cp = varptr("DPATH");
	if(cp->varval == 0)
	...
	do
	{
		p = execat(p, name, tname);
		srchdir(tname, NO, NULL);
	} while (p);
}
