 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: main.c,v 1.5.2.2 90/05/09 16:44:25 wje Exp $"
/*
 * ke	3/7/85	Added ifdef for 4.2bsd file system stuff.  Also sys/types.h
 *		and sys/dir.h now get included defs.
 * mh	12/26/85 made MAKEARGS variable for the '=' command line args.
 */
...
CHARSTAR argv[];
{
	...
	/*
	 * If the name of the command is "makerules", just print out
	 * the rules.
	 */

	bname = (char *) strrchr(argv[0], '/');
	if (bname == 0) {
		bname = argv[0];
	} else {
		bname++;
	}
	if (strcmp(bname, "makerules") == 0) {
		makerules();
		exit(0);
	}
 ...
 */
	...
	strcpy(args,"MAKEARGS=");	/* set to the command line args */
	for(i=1; i<argc; ++i)
...
/*
 * The subroutine makerules() just prints out the rules list.
 */
