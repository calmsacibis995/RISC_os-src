 ...
 * |-----------------------------------------------------------|
 */
/* $Header: gram.y,v 1.3.2.2 90/05/09 16:44:19 wje Exp $ */
/* 
 * mh	12/26/85 allow macros and IPATH for include files
 * mh	3/2/86 make \# do #
 */
 ...
 */
		subst(pfile, tempfile);	/* Substitute for macros in includes */
		fstack(tempfile, &fin, &yylineno);
		goto again;
	}
	if(isalpha(c) || isdigit(c) || c==BLANK || c==DOT || c==USCORE)
...
register int *oldlno;
{
	...
	if(access(newname, 4) != 0) {
		pfile = findfl (newname, "IPATH");    
		if (pfile == (CHARSTAR) -1) {
		    char sname[128];

		    /* try the sccs version of the name */
		    copstr (sname, newname);
		    trysccs (sname);
		    pfile = findfl (sname, "IPATH");    
		    if (pfile == (CHARSTAR) -1)
			pfile = newname;
		    else {
			untrysccs(pfile);
 ...
 *	old yylineno;
 */
	strcat(sfiles[morefiles].sfname, pfile);
	sfiles[morefiles].sfilep = *oldfp;
	sfiles[morefiles++].syylno = *oldlno;
	yylineno = 0;
	if((*oldfp=fopen(pfile, "r")) == NULL)
		fatal1("Cannot open %s", pfile);
}
