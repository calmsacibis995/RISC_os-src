 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: mail.c,v 1.17.1.5 90/05/09 17:24:33 wje Locked $"
...
/*
	...
	and
		#define USE_SENDMAIL for sendmail
...
#define USE_SENDMAIL 1	/* switch for sendmail */
...
#if USE_SENDMAIL
char	optflg[50] = "-bm"; /* flags passed in (via mailx) for sendmail */
#endif
struct	utsname utsn;
int	changed;	/* > 0 says mailfile has changed */
char	curlock[MAXNAMLEN+1];
...
int	flgl = 0;	/* 1 says 'l' option specified */
int	flgF = 0;	/* 1 = Installing/Removing  Forwarding */
int	flgd = 0;	/* 1 says 'd' option specified */
...
int	delfrom = 0;	/* 1 says new From line needs to be created */
...
char	*my_name = NULL;	/* user's name who invoked this command */
...
char	**argv;
{
	...
	*/
	...
	if (my_name == NULL) {
		if (((my_name = getenv("LOGNAME")) == NULL)
		    || (strlen(my_name) == 0)) {
			my_name = getlogin();
		}
...
char	**argv;
{
	...
	*/
	...
	/*
		...
		This is done unless -e was specified and -F was not
	*/
	if (!flge || flgF) {
		lock(mailfile);
	}
	...
	*/
	if (areforwarding(mailfile)) {
		...
		*/
		...
		printf("Your mail is being forwarded to %s", senditto);
		return;
	}
	if (flge) {
		unlock();
...
copyback()
{
	...
	*/
	if (stbuf.st_uid != getuid() && stbuf.st_uid != geteuid()) {
		errmsg(E_OWNR,"");
		done(0);
	}
...
/*
	...
	senditto is used as work area, and if forwarding then
	senditto is set to the list of users
...
char *s;
{
	...
	fread(senditto, (unsigned)(sizeof(frwrd) - 1), 1, fd);
	if (strncmp(senditto, frwrd, sizeof(frwrd) - 1) == SAME) {
		fgets(senditto, sizeof(senditto), fd);
		fclose(fd);
		return(TRUE);
	}
	senditto[0] = '\0';	/* lets be nice */
	fclose(fd);
	return(FALSE);
}
...
char **argv;
{
	...
	char	buf[1024],num_newlines;
	int	in_headers = 1;
	int	saw_from;

	for (i = 1; i < argc; ++i) {
	        if (argv[i][0] == '-') {
		        if (argv[i][1] == '\0') errmsg(E_SYNTAX,"Hyphens MAY NOT be followed by spaces");
		        if (i > 1) errmsg(E_SYNTAX,"Options MUST PRECEDE persons");
		        done(0);
	        }
		/*
	...
	*/
	...
	while ((n=getline(LSIZE,stdin)) > 0) {
		...
		*/
		if (newline && 
		    fromflg == 0 &&
		    in_headers &&
		    (strncmp(line, from, from_len) == SAME ||
		     strncmp(line, ">From ", 6) == SAME)) {
			fromlevel++;
			saw_from = 1;
		} else {
			fromflg = 1;
	...
	/*
	 *	assure that the message ends in at least two newlines.
	 */

	if (num_newlines < 2)
		if (!wtmpf("\n\n", (2 - num_newlines))) done(0);
...
#ifdef USE_SENDMAIL
	/*
	 * Use sendmail if /usr/lib/sendmail.ok and /usr/lib/sendmail exist.
	 */
	if (! flgd) {
	        int	n;
		char	cmd[10240];
		...
		let[1].adr--; /* discard extra trailing newline */
		if (stat("/usr/lib/sendmail.ok", &stb) == 0 &&
		    access("/usr/lib/sendmail", A_EXIST) != CERROR){
			sprintf(cmd, "/usr/lib/sendmail %s", optflg);
			ac = argc;
			ap = argv;
			while (--ac > 0)
				sprintf(cmd + strlen(cmd), " '%s'", *++ap);
			sprintf(cmd + strlen(cmd), " 2>/dev/null");
#ifdef DEBUG
		        printf("%s\n", cmd);
#endif
			if (pipletr(0,cmd,FALSE) == TRUE) {
				return(TRUE);
			}
		}
		let[1].adr++; /* restore extra trailing newline */
	};
#endif
...
#ifdef USE_SENDMAIL
	if (local) sprintf(cmd, "exec rmail -d %s", rsys);
	if ((strcmp(thissys, rsys) == SAME) && (!local)) {
		local++;
		sprintf(cmd, "exec rmail -d %s", name+1);
	}
#else USE_SENDMAIL
...
#ifdef USE_SENDMAIL
				int	sendmail_sent = FALSE;
				char    cmd[10240];
				struct stat stb;

				let[n+1].adr--; /* discard extra 	*/
						/* trailing newline	*/
				if (stat("/usr/lib/sendmail.ok", &stb) == 0 &&
				    access("/usr/lib/sendmail", A_EXIST)
						 != CERROR){
				       sprintf(cmd, 
			    	       "/usr/lib/sendmail %s '%s' 2>/dev/null",
					       optflg, user);
#ifdef DEBUG
				       printf("%s\n", cmd);
#endif
				       if (pipletr(n,cmd,FALSE) == TRUE) {
					    sendmail_sent = TRUE;
				       }
				}
				let[n+1].adr++; /* restore extra	*/
						/* trailing newline	*/
				if (! sendmail_sent)
#endif USE_SENDMAIL
				    send(n, user, level + 1);
...
char	buf[128], *cp, work[128], wuser[128];
	...
	if (dflag < 2) {
		...
		if (strlen(work) > 0) {
			if (replying) {
				sprintf(buf,"mail %s %s", senditto,work);
				printf("%s\n",buf);
				systm(buf);
				return;
			}
...
char	**arglst;
{
	...
	*/
	...
	if (ismail) {
	  	for (i=1; i<argcnt; i++)
			if (arglst[i][0]=='-' && arglst[i][1]=='d') {
			  	flgd = 1;
	  			break;
	  		}
		options = (flgd ? "f:hr:opqmiestwF:dl" : "f:hropqmiestwF:dl");
	} else {
		options = "f:hr:opqmiestwF:dl";
	}
	while ((c = getopt(argcnt, arglst, options)) != EOF) switch(c) {
		/*
			deliver mail locally for sendmail
		*/
		case 'd':
			ismail = FALSE;
			flgd = 1;
...
#ifdef USE_SENDMAIL
		/*
		  	internal flag for /usr/lib/sendmail
		*/
		case 'm':
			strcat(optflg, " -m");
			optcnt++;
...
char	userval[HSIZE+1];
	...
	for (ln = start; ln != stop; ln = forward ? ln + 1 : ln - 1) {
		...
		if (fgets(wline, HSIZE, file) == NULL) {
...
char *file;
{
	...
	*/
	...
	if (strlen(cptr) > MAXNAMLEN-sizeof(maillock)) {
		fprintf(stderr,"%s: Cannot lock file as length of basename '%s' is > MAXNAMLEN-sizeof(maillock) characters\n",program,cptr);
		errmsg(E_LOCK,"");
		done(0);
	}
	...
	*/
	...
	for (i = 0; i < 10; i++) {
		...
		if (flgl) {
			printf("mailbox locked -- retry %d of 10...\n", i + 1);
		}
		sleep(5*(i+1));
        ...
        char *msg;
{
       ...
       static int f = -1;
        ...
        if (addr.sin_family == 0) {
                ...
                struct servent *sp = getservbyname("mbiff", "udp");
