 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: cpio.c,v 1.18.1.6.1.1.1.2 90/10/05 09:59:40 beacker Exp $"
...
#define PATHSIZE MAXPATHLEN	/* maximum PATH length */
#define IN	'i'		/* copy in */
...
static
	int cerr;      /* character count or error returned from */
		       /* symlink, readlink			 */
...
char **argv;
{
	...
	orig_umask = umask(0);
	umask(0);
	while( (ans = getopt( argc, argv, "aBC:ifopcdHklLmrSsbtuvVM:6eI:O:")) != EOF ) {
		switch( ans ) {
...
#ifdef BROKEN
			/*
			 * This code was added by someone (I think SGI)
			 * to have -o be followed by a set of directory
			 * or filenames, so it acts like tar.  This isn't
			 * documented and it doesn't work, so it isn't
			 * worth fixing now.
			 */
		...
		while(getname()) {
			...
			for(filesz=mklong(Hdr.h_filesize); filesz>0; filesz-= CPIOBSZ){
				...
				if (A_link) {
				    ...
				    if ((cerr = readlink(Hdr.h_name,Buf,MAXPATHLEN)) < 0){
					fperr("cpio: Cannot readlink %s\n", Hdr.h_name);
					continue;
				    }
				    else {
					Buf[cerr] = NULL;
...
#endif
	  	new_stat('z',EERROR,Fullname,&Xstatb);
...
copyfile:
			...
			if(Ofile) {
				...
				zchmod( EWARN, Fullname, Hdr.h_mode);
			        set_time(Fullname, mklong(Hdr.h_mtime), 
				         mklong(Hdr.h_mtime));
			    }
...
{
	errusage("%s\n\t%s %s\n\t%s %s\n\t%s %s\n\t%s %s\n",
		    "-o[acvVBHL] [-Cbufsize] [-Mmessage] <name-list >collection",
	Err.source, "-o[acvVBHL] -Ocollection [-Cbufsize] [-Mmessage] <name-list",
	Err.source, "-i[bcdkmrsStuvVfB6HL] [-Cbufsize] [-Mmessage] [pattern ...] <collection",
	Err.source, "-i[bcdkmrsStuvVfB6HL] -Icollection [-Cbufsize] [-Mmessage] [pattern ...]",
	Err.source, "-p[adlmruvVHL] directory <name-list");
}
...
#endif
		    if (new_stat('l',EWARN,namep,&Statb) < 0) {
			_errmsg("UXlstat1", EWARN,
...
#endif
		   if (new_stat('z',EWARN,namep,&Statb) < 0) {
			continue;
		   }
		}
...
#endif
		if (new_stat('s',0,namep,&Xstatb)==-1) {
...
ret:
	...
	if (!Lflag) 
#ifdef 0
	    cerr = lstat(namep, &Xstatb);
#endif
	    cerr = new_stat('l',0,namep,&Xstatb);
	else 
#ifdef 0
	    cerr = stat(namep, &Xstatb);
#endif
	    cerr = new_stat('s',0,namep,&Xstatb);
	if (cerr == 0) {
...
#include <pwd.h>
	...
	static char tbuf[32];
	static char save_name[100];	/* big enough for a few years */
	char *ctime();
