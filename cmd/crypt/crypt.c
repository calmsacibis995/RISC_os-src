 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: crypt.c,v 1.5.2.3 90/05/09 15:32:04 wje Exp $"
	...
	if (argc < 2){
		...
		while ((c = getopt(argc, argv, "pk")) != EOF)
			...
			/* notify editor that exec has succeeded */
				...
				break;
			case 'k':
				key_env = getenv("CRYPTKEY");
				if (key_env == NULL) {
					fprintf(stderr,"environment CRYPTKEY is not set\n");
					exit(1);
				}
				strncpy(key, key_env, 8);
				setup(key);
				kflag = 1;
				break;
			case '?':
