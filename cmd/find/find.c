 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: find.c,v 1.11.2.2 90/05/09 15:50:54 wje Exp $"
...
#define	STAT(name,stbp) \
	(followf ? stat(name, stbp) : lstat(name, stbp))
...
localf()
{
	...
	if (Statb.st_dev == cur_dev)
		return 1;
	if (nfs_fstyp <= 0)
		nfs_fstyp = sysfs(GETFSIND, FSID_NFS);
	if (statfs(Pathname, &sfb, sizeof sfb, 0) < 0)
		return 0;
	return (sfb.f_fstyp != nfs_fstyp);
}
...
char *name, *fname;
{
	...
	int	dir = 0, /* open directory */
		...
		cdval = 0;
#ifdef mips	/* was sgi */
	register DIR *dirp;		/* pointer to "directory" file */
	register struct dirent *dp;	/* pointer to abstract entry */
#else
