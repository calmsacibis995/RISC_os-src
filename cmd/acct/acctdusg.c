 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: acctdusg.c,v 1.1.2.2.1.1.1.2 90/10/23 13:42:40 beacker Exp $"
...
char	*afile, *nfile;
#ifdef RISCOS
int pflag=0;	/* user supplied passwd file, don't use VIS */
struct passwd *pw;
#endif
...
#ifdef RISCOS
/* like makdlst, but takes a struct passwd * vs a line of passwd 
 * this is for use of VIS
 */
mkpwdlst(p)
struct passwd *p;
{
	...
	strncpy(dp->dsk_name,p->pw_name, NAMESZ);
	dp->dsk_uid = p->pw_uid;
	dp->dsk_dirsz = strlen(p->pw_dir);
	if((dp->dsk_logdir = calloc(dp->dsk_dirsz + 1, 1)) == NULL)
		goto nocore;

	strncpy(dp->dsk_logdir, p->pw_dir, dp->dsk_dirsz);
