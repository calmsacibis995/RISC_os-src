...
# |-----------------------------------------------------------|
#
# $Header: runacct.sh,v 1.3.1.4 90/05/09 17:20:13 wje Locked $
...
closewtmp	# fixup pgm to handle people logged in now
cp ${_wtmp} ${_nite}/wtmp.${_date}
acctwtmp "runacct" >> ${_nite}/wtmp.${_date}
nulladm ${_wtmp}
utmp2wtmp	# re-initialize wtmp from utmp records 
...
# this is the way I think AT&T wants to do it.
cp ctacct.${_date} tmpdayt
for _i in ptacct*.${_date}
do
    acctmerg ${_i} < tmpdayt > daytacct
    cp daytacct tmpdayt
done
rm -f tmpdayt
