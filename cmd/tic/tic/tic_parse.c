 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: tic_parse.c,v 1.2.1.3 90/05/09 19:41:12 wje Exp $"
...
short	Strings[];
{
	...
	while (other_names)
	{
		...
		*other_names++ = '\0';
	    ...
	    {
		if (other_names)
		    warning("'%s': bad term name found in list.", cur_name);
		continue;
	    }
		...
		if (!check_only) {
			if (stat(linkname, &statbuf) >= 0  &&
				...
				warning("'%s' defined in more than one entry.", cur_name);
				fprintf(stderr, "Entry being used is '%s'.\n",
					    (unsigned) term_names + string_table);
			}
			unlink(linkname);
