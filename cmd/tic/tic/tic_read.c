 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: tic_read.c,v 1.2.1.2 90/05/09 19:41:47 wje Exp $"
 ...
 *
 *  $Log:	tic_read.c,v $
 * Revision 1.2.1.2  90/05/09  19:41:47  wje
 * add restricted rights legend
 * 
 * Revision 1.2.1.1  89/11/27  04:42:15  wje
 * Branch RISCOS_4_50_FIXES off of trunk.
 * 
 * Revision 1.2  89/09/08  09:52:28  vicki
 * Removed ident string.
 * 
 * Revision 1.1  89/09/08  09:48:28  vicki
 * Initial revision
 * 
 * Revision 1.1  89/09/07  13:01:17  vicki
 * Initial revision
 * 
...
struct _str_struct	*sptr;
{
	...
	else
	{
		...
		if (bytebuf[1] == 0377)
			...
			UN[i] = -2;
		    else if (bytebuf[0] == 0377)	/* -1 == not there */
			UN[i] = -1;
		    else
	...
	if (beencalled)	/* beencalled is non-zero only if we've been called */
	{
	...
	while (num_strings > 0)
	{
	    ...
	    if (swapping)
	    {
		numread = read(fd, byte, 2*min(num_strings, OFFSET_BUFSIZE));
		if (numread <= 0)
		{
		    close(fd);
		    return(-1);
		...
		{
		    if (byte[i][0] == 0377 &&  byte[i][1] == 0377) /* -1 */
			US[i + cur_string] = 0;
		    else if (byte[i][0] == 0376 &&  byte[i][1] == 0377) /* -2 */
			US[i + cur_string] = (char *) -1;
		    else
			US[i + cur_string] = (byte[i][0] + 256*byte[i][1])
					     + pst;
		}
	    }
	    else
	    {
		numread = read(fd, number, 2*min(num_strings, OFFSET_BUFSIZE));
		if (numread <= 0)
		{
		    close(fd);
		    return(-1);
