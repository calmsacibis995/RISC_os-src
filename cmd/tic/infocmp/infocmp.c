 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: infocmp.c,v 1.2.1.3 90/05/09 19:40:18 wje Exp $"
...
*/
void allocvariables(argc, firstoptind)
int argc, firstoptind;
{
...
sortnames()
{
	...
	{
	qsort ((char *) ibool, (unsigned) numbools, sizeof (struct boolstruct)  , boolcompare);
	qsort ((char *) num, (unsigned) numnums, sizeof (struct numstruct) , numcompare);
	qsort ((char *) str, (unsigned) numstrs, sizeof (struct strstruct) , strcompare);
	}
}

/*
...
char *addko (output, input, koptr)
char *output, *input, *koptr;
{
...
int n;
{
	...
	{
	if (Aflag && Bflag)
	    (void) printf ("comparing %s (TERMINFO=%s) to %s (TERMINFO=%s).\n",
	...
	if (verbose)
	    (void) fprintf (trace, "%s: %d:%d,changed=%d,seen=%d.\n",
	...
	if (use)
		...
		if ((numval > -1) && (numval != num[i].secondval))
		    {
		    (void) fprintf (stderr,
			"%s: use= order dependency found:\n", progname);
		    (void) fprintf (stderr, "    %s: %s has %d, %s has %d.\n",
	...
	if (use)
		...
		if (!EQUAL (strval, str[i].secondval))
		    {
		    (void) fprintf (stderr, "use= order dependency found:\n");
		    (void) fprintf (stderr, "    %s: %s has '",
			    str[i].capname, str[i].secondname);
		    PR (stderr, str[i].secondval);
		    (void) fprintf (stderr, "', %s has '", nterm);
		    PR (stderr, strval);
		    (void) fprintf (stderr, "'.\n");
		    }
		}
...
*/
void dorelative (firstoptind, argc, argv)
register int firstoptind;
register int argc;
register char **argv;
{
    register int i;
	...
	if (used[i - firstoptind])
	    ...
	    (void) fprintf (stderr,
		"%s: 'use=%s' did not add anything to the description.\n",
		progname, argv[i]);
}

void setenv (termNinfo)
register char *termNinfo;
{
    extern char **environ;
...
char **argv;
{
	...
	switch (c)
		...
		break;
	    case 'V':
		(void) printf ("%s: version %s\n", progname, "infocmp.c 1.10");
		exit (0);
	    case '?':
	    usage:
		(void) fprintf (stderr,
			"usage: %s [-ducn] [-ILC] [-1Vv] [-s d|i|l|c] [-A directory] [-B directory] term-names ...\n",
			progname);
		(void) fprintf (stderr, "\t-d\tprint differences (the default for >1 term-name)\n");
	...
	{
	(void) fprintf (stderr,
	    "%s: must have at least two terminal names for a comparison to be done.\n",
	    progname);
	goto usage;
	}
