 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: exportfs.c,v 1.2.1.4 90/05/09 23:14:59 wje Locked $"

/* @(#)exportfs.c	1.2 88/06/15 4.0NFSSRC Copyr 1988 Sun Micro */
/*
...
#	define pstreq(a, b, blen) \
	...
	if (streq(opstr, ro_opt)) {
		ops->flags |= EX_RDONLY;
#ifdef RISCOS
	} else if (streq(opstr, rw_opt)) {
		/* Do nothing.  This added for backwards compatibility.
		 * By adding this, we can make the option: -rw
		 * perform the same task as putting nothing.
		 */
#ifdef NOHIDE
	} else if (streq(opstr, nohide_opt)) {
		ops->flags |= EX_SUBMOUNT;
#endif
#endif
...
#ifdef RISCOS	/* to remain backwards compatible with risc/os 4.0 */
	} else if (pstreq(opstr, rootid_opt, sizeof(rootid_opt) - 1)) {
		char *val = &opstr[sizeof(rootid_opt)];
		if (isdigit(*val))
			ops->anon = atoi(val);
		else {
