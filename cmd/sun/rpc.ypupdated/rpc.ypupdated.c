 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: rpc.ypupdated.c,v 1.2.1.4 90/05/09 19:24:26 wje Exp $"
	...
	char *argv[];
{
	...
	default:
#ifdef RISCOS
		syslog(LOG_ERR, "%s: warning -- options ignored\n", cmd);
#else
	...
	if (chdir(YPDIR) < 0) {
#ifdef RISCOS
		syslog(LOG_ERR,"%s: can't chdir to ", cmd);
		syslog(LOG_ERR,"%s\n",YPDIR);
#else
...
#ifdef RISCOS
		syslog(LOG_ERR, "%s: couldn't register service\n", cmd);
#else
...
#ifdef RISCOS
		syslog(LOG_ERR,"svc_freeargs failed\n");
#else
	...
	char *data;
{
	...
	if (pid < 0) {
#ifdef RISCOS
		syslog(LOG_ERR,"openpipes failed\n");
#else
