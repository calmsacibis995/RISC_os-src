 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: on.c,v 1.1.1.5 90/05/09 19:15:42 wje Exp $"
#ifndef lint
...
#ifdef RISCOS
/* rex_ttysize is NOT the same as ttysize in RISC/os, darn it! */
...
#ifdef RISCOS
	if ((size.ts_cols==WindowSize.ts_cols) && (size.ts_lines == size.ts_lines)) return;
#else
     if (bcmp(&size,&WindowSize,sizeof size)==0) return;
#endif
#ifdef RISCOS
     WindowSize.ts_lines = size.ts_lines;
     WindowSize.ts_cols = size.ts_cols;
#else
     /* this is wrong, but it is as it came from sun!*/
     WindowSize = size;
#endif
...
#ifdef TIOCGSIZE
#ifdef RISCOS
	     err = ioctl(0, TIOCGSIZE, &OurSize) < 0;
	     WindowSize.ts_cols = OurSize.ts_cols;
	     WindowSize.ts_lines = OurSize.ts_lines;
#else
