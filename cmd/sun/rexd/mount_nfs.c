 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: mount_nfs.c,v 1.1.1.4 90/05/09 19:15:35 wje Exp $"
#ifndef lint
static char sccsid[] = 	"@(#)mount_nfs.c	1.3 88/07/26 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
	...
	char *fsname, *dir;
{
	...
	*p++ = 0;
...
#ifdef RISCOS
		syslog(LOG_ERR, "%s not in hosts database\n", fsname);
#else
	...
	if ((client = clntudp_create(&sin, MOUNTPROG, MOUNTVERS,
	    timeout, &s)) == NULL) {
#ifdef RISCOS
		syslog(LOG_ERR,"Can't create client handle\n");
#else
	...
	if (rpc_stat != RPC_SUCCESS) {
#ifdef RISCOS
		syslog(LOG_ERR, "umount: RPC call failed\n");
#else
	...
	char *deldir;
{
	...
	while (rename(MOUNTED, from)) {
		if (++count > MAXTRIES) {
#ifdef RISCOS
			syslog(LOG_ERR, "newmtab: cat get %s lock\r\n", MOUNTED);
#else
	...
	if (fromp == NULL || top == NULL) {
		rename(from, MOUNTED);
#ifdef RISCOS
		syslog(LOG_ERR, "newmtab: can't open %s or %s\r\n", from, to);
#else
...
#ifdef RISCOS
		syslog(LOG_ERR, "newmtab can not rename %s to %s\r\n", 
			to, MOUNTED);
#else
