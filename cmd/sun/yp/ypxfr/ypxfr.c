 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: ypxfr.c,v 1.2.1.2 90/05/09 19:35:58 wje Exp $"
#ifndef lint
...
#include <dbm.h>
#undef NULL
/* this turns off a typedef in yp_prot.h that is already satisfied by ndbm.h*/
#define DATUM
...
\t[-s <domainname>] [-S] [-c] [-C tid prot ipadd port] map\n\n\
where\n\
	...
	char *name;
{
	...
	if (debug) fprintf(stderr,"ypxfr:[b] Last Modified '%s'\n",number);
