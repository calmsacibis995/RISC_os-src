 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: makedbm.c,v 1.2.1.2 90/05/09 19:28:37 wje Exp $"
/*      @(#)ypdefs.h	1.2 88/06/02 4.0NFSSRC SMI   */
#ifndef lint
static char sccsid[] = 	"@(#)makedbm.c	1.1 88/03/07 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
	...
	char **argv;
{
	...
	while (fgets(buf, sizeof(buf), infp) != NULL) {
		...
		while (1) {
			if (p == NULL || *p == NULL) {
#ifdef RISCOS
				fprintf(stderr, "makedbm: improper file format\n");
#else
		...
		if (lower_case_keys) 
			for (i=0; i<key.dsize; i++) {
				ic = *(key.dptr+i);
				if (isascii(ic) && isupper(ic)) ic = tolower(ic);
				*(key.dptr+i) = ic;
			} 
		tmp = fetch(key);
		if (tmp.dptr == NULL) {
