 ...
 * |-----------------------------------------------------------|
 */
/* $Header: ypbind.c,v 1.1.1.2.1.1.1.4 90/11/07 15:06:19 beacker Exp $ */
#ifndef lint
...
#ifdef RISCOS
#define INIT_DEFAULT
int oldhup,oldterm,oldcld;  /* signal saves */
int init_deflt = 0;
#endif
...
/* this is an option for RISC/os */
	...
	if (silent) {
		...
		if (pid == -1) {
#ifdef RISCOS
		    /* we're going to close everything right away, so
		     * the normal case must put openlog call after the
		     * closes.  For this exit case, we still want the
		     * log to show what went wrong.  So, we do an
		     * extra openlog here.
		     */
		...
		oldterm = (int)signal(SIGTERM,SIG_IGN);
		oldhup = (int)signal(SIGHUP,SIG_IGN);
		oldcld = (int)signal(SIGCHLD,SIG_IGN);
 		t = open("/dev/tty", 2);
 		if (t >= 0) {
...
#ifdef RISCOS
		syslog(LOG_ERR,	
		    "Can't catch broadcast process exit signal.\n");
#else
		(void) fprintf(stderr,
...
#ifdef RISCOS
                syslog(LOG_ERR,
                  "Cannot create socket for pinging.\n");
#else
                (void) fprintf(stderr,
...
#ifdef RISCOS
		syslog(LOG_ERR, "can't create tcp service.\n");
#else
...
#ifdef RISCOS
		syslog(LOG_ERR,"can't register tcp service.\n");
#else
		(void) fprintf(stderr,
...
#ifdef RISCOS
		syslog(LOG_ERR,"can't register udp service.\n");
#else
		(void) fprintf(stderr,
...
tryagain:
		...
		if (current_domain->bindfile != -1) {
			...
			write(current_domain->bindfile, &response,
				sizeof(response));
			/* BOZO, you left this line out, rendering this
			 * time saving piece totally ineffective!
			 */
			flock(current_domain->bindfile,LOCK_UN);
		} else {
			/* 
...
#ifdef RISCOS
	        syslog(LOG_ERR,"Can't respond to rpc request.\n");
#else
		(void) fprintf(stderr,
...
#ifdef RISCOS
		syslog(LOG_ERR,"ypbind_get_binding can't free args.\n");
#else
		(void) fprintf(stderr,
...
#ifdef RISCOS
			syslog(LOG_ERR,"broadcaster fork failure.\n");
#else
			(void) fprintf(stderr,
	...
	unsigned short vers;
{
...
#ifdef RISCOS
		syslog(LOG_ERR,"Set domain request to host %s, ",
			inet_ntoa(addr));
		syslog(LOG_ERR,"from host %s, failed (bad port).\n",
	...
	if (rqstp->rq_cred.oa_flavor == AUTH_UNIX) {
		if (((struct authunix_parms *)rqstp->rq_clntcred)->aup_uid != 0) {
			...
			syslog(LOG_ERR,"Set domain request to host %s,",
					inet_ntoa(addr));
			syslog(LOG_ERR," from host %s, failed (not root).\n", 
					inet_ntoa(who->sin_addr));
#else
			fprintf(stderr,"ypbind: Set domain request to host %s,",
					inet_ntoa(addr));
		...
		syslog(LOG_ERR, "Set domain request to host %s,",
				inet_ntoa(addr));
		syslog(LOG_ERR," from host %s, failed (credentials).\n", 
				inet_ntoa(who->sin_addr));
#else
		fprintf(stderr, "ypbind: Set domain request to host %s,",
				inet_ntoa(addr));
...
#ifdef RISCOS
			openlog("ypbind",LOG_PID,LOG_DAEMON);
			syslog(LOG_ERR,"current domain not found.");
#endif
			abort();
		}
...
#ifdef RISCOS
			openlog("ypbind",LOG_PID,LOG_DAEMON);
			syslog(LOG_ERR,"current domain not found for OLDVERS.");
#endif
			abort();
		}
