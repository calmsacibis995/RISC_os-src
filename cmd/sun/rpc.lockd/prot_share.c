 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: prot_share.c,v 1.2.1.2.1.1.1.2 90/11/17 11:57:22 beacker Exp $"
/* @(#)nfs.cmds:nfs/lockd/prot_share.c	1.2 */
/*
 ...
 */
	hv = 0;
/*
	for (i = req->share.fh.n_len, c = req->share.fh.n_bytes; --i; )
*/
	i = req->share.fh.n_len;
	if (i > 20)
		i = 20;
	for (c = req->share.fh.n_bytes; --i; )
		hv += *c++;
	hv %= SH_HASH;
 ...
 */
			if (sxh->sxxp == NULL) {
				...
				if (sxh->fwd != NULL)
					sxh->fwd->bck = sxh->bck;
				if ((char *)sxh != NULL) free((char *)sxh);
			}
		}
