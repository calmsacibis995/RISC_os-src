...
/* @(#)nfs.cmds:nfs/lockd/flk_reclox.c 1.2 */
/*
...
#ifdef RISCOS
        new->lck.lox.filocks.sysid = lckdat->lck.lox.filocks.sysid;
#else
	...
	while (flp = nflp) {
		...
		if (debug)
			...
			printf("result from SAMEOWNER(&(flp->set, ld))=%d\n",
				SAMEOWNER(&(flp->lck.lox.lld),
					  &(ld->lck.lox.lld),
					  flp->lck.lox.filocks.sysid,
					  ld->lck.lox.filocks.sysid));
#else
			printf("result from SAMEOWNER(&(flp->set, ld))=%d\n",
...
#ifdef RISCOS
struct reclock *
blocked_reclox(flp, lckdat, lckdat_sysid, insrt)
        struct reclock *flp;
        struct flock  *lckdat;
        long   lckdat_sysid;
...
#endif
{
        ...
        *insrt = NULL;
		...
		if (f->lck.lox.lld.l_len && f->lck.lox.lld.l_len != MAXEND) {
			      ...
			      flp->lck.lox.filocks.sysid, lckdat_sysid))
#else
                	if( SAMEOWNER(&(f->lck.lox.lld), lckdat) )
#endif
			  {
	...
	for (; f != NULL; f = f->next) {
		if (lckdat->l_len && lckdat->l_len != MAXEND) {
                	...
                	if (lckdat->l_start <= END(&f->lck.lox.lld) 
				       ...
				       f->lck.lox.filocks.sysid, lckdat_sysid))
#else
                  		&& ( !SAMEOWNER(&(f->lck.lox.lld), lckdat) )
#endif
                  		&& (f->lck.lox.lld.l_type == F_WRLCK
