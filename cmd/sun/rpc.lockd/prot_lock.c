 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: prot_lock.c,v 1.2.1.2.1.1.1.2 90/11/17 11:53:00 beacker Exp $"
/* @(#)nfs.cmds:nfs/lockd/prot_lock.c	1.11 */
/*
...
extern char		*xmalloc();
#ifdef RISCOS
extern char		hostname[MAXNAMLEN+1];	/* used by remote_data() */
#else
	...
	struct filock *b;
{
	...
	if (same_type(&(a->lck.lox.lld), &b->set) &&
		...
		SAMEOWNER(&(a->lck.lox.lld), &b->set,
			  a->lck.lox.filocks.sysid, b->sysid))
#else
	if (same_type(&(a->lck.lox.lld), &b->set) &&
		same_bound(&(a->lck.lox.lld), &b->set) &&
		SAMEOWNER(&(a->lck.lox.lld), &b->set))
