 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: prot_pnlm.c,v 1.2.1.2.1.1.1.2 90/11/17 11:55:36 beacker Exp $"
/* @(#)nfs.cmds:nfs/lockd/prot_pnlm.c	1.10 */
/*
 * prot_pnlm.c
 * consists of all procedures called by nlm_prog
 */
	...
	struct reclock *a;
{
	...
	if (act == nlmreply) { /* reply to nlm_transp */
		...
		valid = 1;
#ifdef NOTUSE
		if ((rpc_err = call_rpc(name, NLM_PROG, NLM_VERS, proc,
	...
	remote_result *(*cont)();
{
	...
	if ((msgp = search_msg(reply)) != NULL) {	/* found */
		...
		add_reply(msgp, resp);
                ...
                if (msgp->reply) {
		  /*
		   * add_reply dequeue's the reply message if it's the last
		   * message on the queue to send to klm. This is a performance
		   * optimization to process it immediately.
		   * So we MUST check to see if this happened prior to
		   * following the reply message pointer.
		   */
                   if (msgp->reply->lstat == blocking)
                        dequeue(msgp);
		 }
	...
	int retransmit;
{
	...
	if (proc == NLM_LOCK_MSG || proc == NLM_UNLOCK_MSG ||
		 ...
		 */
		if ((rpc_err = call_rpc(name, NLM_PROG, NLM_VERS, func, xdr_arg,
...
#ifdef RISCOS
	/*
	 * Because the protocol requires some unique identifier to allow the
	 * remote machine to distinguish between lock requests AND the BSD 4.3
	 * flock record does NOT contain the system identifier maintained by
	 * System V, it is necessary to maintain the following field will be
	 * to make the requests unique.
	 */
	to->alock.lox.filocks.sysid=(long)nlm_transp->xp_raddr.sin_addr.s_addr;
	if (debug)
		printf("nlm_transp->xp_rtaddr.sind_addr.s_addr= %x\n",
		       nlm_transp->xp_raddr.sin_addr.s_addr);
#else
	to->alock.lox.lld.l_sysid = (long)nlm_transp->xp_rtaddr.buf;
	if (debug)
