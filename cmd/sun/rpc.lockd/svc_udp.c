/*
 * svc_udp.c 1.1 88/08/05 NFSSRC4.0 1.3 88/01/05 Copyr 1984 Sun Micro
 *
 * Server side for UDP/IP based RPC.
...
extern int errno;
#ifdef RISCOS
static int cache_set(SVCXPRT *xprt, u_long replylen);
...
struct svcudp_data {
	...
	char *	su_cache;	/* pointer to cached data, NULL if no cache */
};
	...
	u_int sendsz, recvsz;
{
	...
	if (xprt == NULL) {
#ifdef RISCOS
		syslog(LOG_ERR, "svcudp_create: out of memory\n");
#else
 ...
 * Copies reply buffers into fifo cache
 * Might want to copy pointer instead of buffer
 */
