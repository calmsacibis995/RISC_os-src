...
#ident	"$Header: prot_libr.c,v 1.2.1.2.1.1.1.2 90/11/17 11:52:24 beacker Exp $"

/*
 * @(#)nfs.cmds:nfs/lockd/prot_libr.c 1.6
 * prot_libr.c
...
#ifdef RISCOS
char hostname[MAXNAMLEN+1];		/* for generating oh */
#else
...
#ifdef RISCOS
	(void) gethostname(hostname, MAXNAMLEN+1);
#else
...
#ifdef RISCOS
		printf(" rel=%d w_flag=%d type=%d pid=%d class=%d granted=%d LockID=%d ",
			a->rel, a->w_flag, a->lck.lox.lld.l_type,
			a->lck.lox.lld.l_pid, a->lck.lox.class,
		        a->lck.lox.granted, a->lck.lox.LockID);
#else
