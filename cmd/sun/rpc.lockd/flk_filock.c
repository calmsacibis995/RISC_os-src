...
#ident	"$Header: flk_filock.c,v 1.1.1.1.1.1.1.1 90/11/17 11:40:56 beacker Exp $"

/* @(#)nfs.cmds:nfs/lockd/flk_filock.c 1.2 */
/*
...
#ifdef RISCOS
struct filock *
insflck(lck_list, lckdat, lckdat_sysid, fl)
...
#ifdef RISCOS
int
flckadj(lck_list, insrtp, ld, ld_sysid)
	struct filock 	**lck_list;
	...
	while (flp = nflp) {
		...
		if (debug) 
			...
			printf("result from SAMEOWNER(&(flp->set), ld) %d\n",
			 SAMEOWNER(&(flp->set), ld, flp->sysid, ld_sysid));
#else
			printf("result from SAMEOWNER(&(flp->set), ld) %d\n",
				SAMEOWNER(&(flp->set), ld));
#endif
#ifdef RISCOS
		if(SAMEOWNER(&(flp->set), ld, flp->sysid, ld_sysid))
#else
...
#ifdef RISCOS
struct filock *
blocked(flp, lckdat, lckdat_sysid, insrt)
	struct filock *flp;
	struct flock  *lckdat;
	long   lckdat_sysid;
	struct filock **insrt;
...
#ifdef RISCOS
			if(SAMEOWNER(&(f->set),lckdat,f->sysid,lckdat_sysid) )
#else
...
#ifdef RISCOS
int
deadflck(flp, lckdat, lckdat_sysid)
	struct filock *flp;
	struct flock *lckdat;
	long   lckdat_sysid;
#else
...
#ifdef RISCOS
	blcksysid = blck->sysid;
#else
	blcksysid = blck->set.l_sysid;
#endif
	do {
#ifdef RISCOS
		if (blckpid == lckdat->l_pid && blcksysid == lckdat_sysid)
#else
		if (blckpid == lckdat->l_pid && blcksysid == lckdat->l_sysid)
#endif
			return 1;
		/*
