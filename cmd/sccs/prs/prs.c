 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: prs.c,v 1.8.2.2 90/05/09 18:46:51 wje Exp $"
	...
	for(lp = spec; *lp != 0; lp++) {
		if(lp[0] == ':' && lp[1] != 0 && lp[2] == ':') {
			...
			case 256*'N'+'P':	/* :PN: Full pathname of File */
				...
				dname(Dir);
				if(( p = getcwd(Olddir,sizeof(Olddir))) == NULL)
					fatal("getcwd() failed (prs2)");
				if(chdir(Dir) != 0)
					fatal("cannot change directory (prs3)");
				if(( p = getcwd(Pname,sizeof(Pname))) == NULL )
					fatal("getcwd() failed (prs2)");
				if(chdir(Olddir) != 0)
					fatal("cannot change directory (prs3)");
				printf("%s/",Pname);
				...
				remq(pkt,ser);
 				...
 				if (pkt->p_apply) {
 				   if ((ap = &pkt->p_apply[ser])->a_code == APPLY) {
 					addq(pkt,ser,iod == INS ? YES : NO,
 						iod,ap->a_reason & USER);
 				   } else {
 					addq(pkt,ser,iod == INS ? NO : NULL,
 						iod,ap->a_reason & USER);
 				   }
 				} else {
 					addq(pkt,ser,iod == INS ? NO : NULL,
