 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: admin.c,v 1.6.2.3 90/05/09 18:44:03 wje Exp $"
...
char	*afile;
{
	...
	*/
	uname(&un);
	uuname = un.nodename;
	if (!HADH && lockit(copy(auxf(afile,'z'),Zhold),2,getpid(),uuname))
	...
	if (fexists) { /* modifying */
		...
		/* Modify checksum if corrupted */
	        ...
	        if (strlen(gpkt.p_line) > 8 && gpkt.p_line[0] == '\001'
		         && gpkt.p_line[1] == '\150' ) 
		{
			gpkt.p_line[7] = '\012';
		   	gpkt.p_line[8] = '\000';
		...
		if (strlen(sname(afile)) > NAME_MAX) {
