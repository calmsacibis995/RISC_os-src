...
/* | All Rights Reserved.                            | */
/* --------------------------------------------------- */
/* $Header: awk.g.y,v 1.5.2.2 90/01/11 17:50:40 wje Exp $ */
...
pattern:
	...
	| pattern and pattern %prec AND
		{ $$ = op2(AND, op2(NE,$1,nullnode), op2(NE,$3,nullnode)); }
	| reg_expr bor pattern %prec BOR
		{ $$ = op2(BOR, op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($1), 0)),nullnode), op2(NE, $3, nullnode)); }
	| reg_expr and pattern %prec AND
		{ $$ = op2(AND, op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($1), 0)),nullnode), op2(NE, $3, nullnode)); }
	| pattern bor reg_expr %prec BOR
		{ $$ = op2(BOR, op2(NE, $1, nullnode), op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($3), 0)),nullnode)); }
	| pattern and reg_expr %prec AND
		{ $$ = op2(AND, op2(NE, $1, nullnode), op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($3), 0)),nullnode)); }
	| reg_expr bor reg_expr %prec BOR
		{ $$ = op2(BOR, op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($1), 0)),nullnode), op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($3), 0)),nullnode)); }
	| reg_expr and reg_expr %prec AND
		{ $$ = op2(AND, op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($1), 0)),nullnode), op2(NE, op3(MATCH, 0, rectonode(), makedfa(reparse($3), 0)),nullnode)); }
