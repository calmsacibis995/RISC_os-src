 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: jobs.c,v 1.2.2.2 90/05/09 18:58:07 wje Exp $"
     	...
     	int	bg;
{
	j_default_pg = getpid();
	if (! bg)
	  	ioctl(1,TIOCSPGRP,&j_default_pg);
...
#else RISCOS
	static int	ldisc = NTTYDISC;	/* BSD_SYS ioctl brain damage */
#endif RISCOS
