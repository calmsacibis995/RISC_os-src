 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: xec.c,v 1.9.2.3 90/05/09 19:01:41 wje Exp $"
...
int	*pf1, *pf2;
{
	...
	if ((t = argt) && execbrk == 0)
	{
		...
		/* Research includes login as part of the shell */	
...
#if defined(RISCOS) || (! defined(BSD_SYS) && ! defined(RES))
						case SYSNEWGRP:	
							if (flags & rshflg)	
								failed(com[0], restricted);	
							else	
...
#if !defined(RISCOS) && !defined(SYMLINK) && (defined(BSD_SYS) || defined(BRL) && !defined(pdp11))
...
#include	<BRL/umasks.h>
						...
						case SYSTST:
							...
							break;
#endif	/* ! defined(RES) || defined(RISCOS) (DAG -- bug fix: SYSTST not in RES) */
...
#endif
				{
					sigset(SIGINT, SIG_IGN);	/* DAG */
					sigset(SIGQUIT, SIG_IGN);	/* DAG */
