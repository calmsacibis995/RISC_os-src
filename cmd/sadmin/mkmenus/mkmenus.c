 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: mkmenus.c,v 1.8.2.3 90/05/09 18:36:42 wje Exp $"
...
char	*fname;
{
	...
	*/
	...
	if( !direct ) {
		...
		struct stat	statbuf;
#ifdef mips	/* was sgi */
		dircnt = scandir(dir, &direct, select, alphasort);
#else
		dirp = Fopen( dir, "r" );
...
#ifdef mips	/* was sgi */
	for( ;  dircnt--;  cnt++ ) {
		if( direct[cnt]->d_ino != 0  &&  direct[cnt]->d_name[0] != '.' ) {
			strncpy( fname, direct[cnt]->d_name, sizeof direct[cnt]->d_name );
