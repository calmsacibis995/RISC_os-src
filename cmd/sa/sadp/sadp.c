 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sadp.c,v 1.7.2.3 90/05/09 18:26:42 wje Exp $"
...
/* mips uses buf instead of iobuf */
...
int idcnt;
struct buf dkp[NDRIVE]; /* mips doesn't use iobuf to head lists */
...
#ifdef RISCOS
	machine_setup(setup,X_ID,X_IDEQ);
	if (strlen(DevName) > 0 &&
...
/*      open /dev/kmem  */
	...
	if (check_kernel_id (f,
			(long)setup[X_ID_STRING].n_value, "/unix") > 0) {
		extmsg("/unix does not match %s\n");
	};
#endif RISCOS
#if pdp11 || vax
	if (dflg == 0){
...
/*      get storage from memory for sysbuf pool ,
	...
	if (dflg == 0){
		for (i=0;i<1;i++){
			if (setup[i+DKTAB].n_value != 0 ){
				dev = i;
				break;
			}
		}
		if (i == 1)
			extmsg("dkip is not defined");
		for (i=0;i<NDRIVE;i++)
			drvlist[i] =1;
	}
	else 
		if (setup[dev+DKTAB].n_value == 0){
			fprintf(stderr,"Device %s is not defined\n",devnm[dev]);
			exit(2);
		}

/*      set upper bound on number on drives */
	ub = 2;		/* 2 drives for now on mips */
	if (dflg && dn >= ub) { 
...
#if pdp11 || vax || u3b2 || mips
#if u3b2
		lseek(f,(long)setup[dev+ID].n_value,0);
#else
#if mips
		lseek(f,(long)setup[dev+DK].n_value,0);
#else
...
#else
#if mips
		lseek(f,(long)setup[DKTAB+dev].n_value,0);
#else
		lseek(f,(long)setup[HPU+dev].n_value,0);
#endif
#endif
#if mips
		if( read(f,dkp,(sizeof (struct buf)* ub)) == -1){
#else
