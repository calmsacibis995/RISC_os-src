 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: sar.c,v 1.9.1.3 90/05/09 18:27:52 wje Exp $"
...
#endif
	nx = (struct sa *) calloc(recsz,1);
	ox = (struct sa *) calloc(recsz,1);
	if (nx == NULL ||
	    ox == NULL) {
		fprintf(stderr,"sar:  cannot allocate dynamic memory\n");
		exit(1);
	};
...
prpass(){
	...
	while(read(fin, nx, (unsigned)recsz) > 0) {
		...
		memcpy(ox,nx,recsz);		/*  Age the data	*/
		if(isec > 0)
			while(tnext <= trec)
				tnext += isec;
	}
...
prthdg()
{
	...
	case 'p':
		tsttab();
		printf(" %7s %5s %5s %6s %6s %7s %5s %6s %7s %7s\n",
			"vflt/s",
			"%dmnd",
			"%swap",
			"%cache",
...
prtopt()
{
	...
	case 'b':
		...
		printf(" %7.0f %7.0f %7.0f %7.0f %7.0f %7.0f %7.0f %7.0f\n",
			...
			(nx->si.lread - ox->si.lread) == 0.0 ? 0.0 :
			...
			(nx->si.lwrite - ox->si.lwrite) == 0.0 ? 0.0 :
...
/* new freemem for 3b2 */
	...
	case 'p':
		tsttab();
		printf(" %7.2f %5.2f %5.2f %6.2f %6.2f %7.2f %5.2f %6.2f %7.2f %7.2f\n",
			(float)(nx->mi.vfault - ox->mi.vfault) / tdiff * HZ,
    			(float)(nx->mi.vfault - ox->mi.vfault == 0 ? 0.0 :
(float)(nx->mi.demand - ox->mi.demand)/(nx->mi.vfault - ox->mi.vfault) * 100.0),
    			(float)(nx->mi.vfault - ox->mi.vfault == 0 ? 0.0 :
(float)(nx->mi.swap - ox->mi.swap)/(nx->mi.vfault - ox->mi.vfault) * 100.0),
    			(float)(nx->mi.vfault - ox->mi.vfault == 0 ? 0.0 :
(float)(nx->mi.cache - ox->mi.cache)/(nx->mi.vfault - ox->mi.vfault) * 100.0),
    			(float)(nx->mi.vfault - ox->mi.vfault == 0 ? 0.0 :
(float)(nx->mi.file - ox->mi.file)/(nx->mi.vfault - ox->mi.vfault) * 100.0),
			(float)(nx->mi.pfault - ox->mi.pfault) / tdiff * HZ,
    			(float)(nx->mi.pfault - ox->mi.pfault == 0 ? 0.0 :
(float)(nx->mi.cw - ox->mi.cw)/(nx->mi.pfault - ox->mi.pfault) * 100.0),
    			(float)(nx->mi.pfault - ox->mi.pfault == 0 ? 0.0 :
(float)(nx->mi.steal - ox->mi.steal)/(nx->mi.pfault - ox->mi.pfault) * 100.0),
			(float)(nx->mi.file - ox->mi.file) / tdiff * HZ,
			(float)(nx->mi.freedpgs - ox->mi.freedpgs) / tdiff * HZ);
		ax.mi.vfault += nx->mi.vfault - ox->mi.vfault;
		ax.mi.demand += nx->mi.demand - ox->mi.demand;
		ax.mi.cache += nx->mi.cache - ox->mi.cache;
		ax.mi.file += nx->mi.file - ox->mi.file;
		ax.mi.swap += nx->mi.swap - ox->mi.swap;
		ax.mi.pfault += nx->mi.pfault - ox->mi.pfault;
		ax.mi.cw += nx->mi.cw - ox->mi.cw;
		ax.mi.steal += nx->mi.steal - ox->mi.steal;
...
#if	vax || u3b || u3b5 || u3b2 || mips
	case 'p':
		printf("Average  %7.2f %5.2f %5.2f %6.2f %6.2f %7.2f %5.2f %6.2f %7.2f %7.2f\n",
			(float)(ax.mi.vfault / tdiff * HZ),
			(float)(ax.mi.demand)/ax.mi.vfault * 100.0,
			(float)(ax.mi.swap)/ax.mi.vfault * 100.0,
			(float)(ax.mi.cache)/ax.mi.vfault * 100.0,
			(float)(ax.mi.file)/ax.mi.vfault * 100.0,
			(float)(ax.mi.pfault / tdiff * HZ),
			(float)(ax.mi.cw)/ax.mi.pfault * 100.0,
			(float)(ax.mi.steal)/ax.mi.pfault * 100.0,
